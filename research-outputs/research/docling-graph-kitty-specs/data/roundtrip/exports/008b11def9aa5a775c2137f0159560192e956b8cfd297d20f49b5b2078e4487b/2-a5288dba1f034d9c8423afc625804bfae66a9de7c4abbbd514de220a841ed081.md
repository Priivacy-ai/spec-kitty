...
**Mission** : `079-post-555-release-hardening` **Branch contract** : planning\_base = `main` · merge\_target = `main` **Spec** : [spec.md](spec.md) `/private/tmp/311/spec-kitty`
...
`git init` /commit call path from `init.py` ; remove `--no-git` flag | WP01 | [P] | `.agents/skills/` seeding from `init.py` `init` next-steps output (name `spec-kitty next` + agent action verbs) | WP01 | [P] | `init --help` text to describe the new model accurately | WP01 | [P] | `_split_wp_sections()` at top-level non-WP `##` headings | WP02 | [P] | `PLANNING_ARTIFACT` in `compute.py` ; assign to `lane-planning` `branch_naming.py` : `lane_branch_name(..., "lane-planning")` → planning branch | WP03 | [P] | `worktree.py` + `workspace_context.py` : route `lane-planning` → main repo checkout; add `get_next_feature_number` display-only docstring | WP03 | [P] | `resolver.py:174-182` `execution_mode` special-case | WP03 | [P] | `implement.py` : uniform lane lookup + rewrite `--help` docstring as internal-infrastructure | WP03 | [P] | `mission_id` in `mission_creation.py` ; persist to `meta.json` ; lock write; mint 079's own `mission_id` `mission_id` to `MissionIdentity` dataclass; update `resolve_mission_identity()` `mission_id` to `emit_mission_created()` payload in `events.py` + `emitter.py` `FileLock` scope in `refresh_tokens()` to full read→network→persist transaction | WP05 | [P] | `README.md` canonical workflow line + mermaid diagram | WP06 | [P] | `spec-kitty implement WP##` with `spec-kitty agent action implement` ) | WP06 | [P] | `docs/` canonical-path mentions in first ~5 paragraphs of ~12 files | WP06 | [P] | `.kittify/metadata.yaml` version to match `pyproject.toml` `validate_metadata_yaml_version_sync()` to `scripts/release/validate_release.py`
...
| T025   | Update slash-command source templates (replace   |
|--------|--------------------------------------------------|
...
| WP07   | —                                                                                    |
|--------|--------------------------------------------------------------------------------------|
| T031   | Regression tests for Track 7 (T7.1–T7.4 from test-contracts.md)                      |
| T032   | Dogfood acceptance: run quickstart V-7 walkthrough; verify all RG-1..RG-8 gates pass |
...
**Dependencies** : none **Estimated prompt size** : ~380 lines **Prompt file** : [tasks/WP01-init-coherence.md](tasks/WP01-init-coherence.md)
...
**Dependencies** : none **Estimated prompt size** : ~200 lines **Prompt file** : [tasks/WP02-dependency-parser-hotfix.md](tasks/WP02-dependency-parser-hotfix.md)
...
**Dependencies** : none (T007 from WP02 is independent; lane computation and dependency parsing are separate subsystems) **Estimated prompt size** : ~430 lines **Prompt file** : [tasks/WP03-planning-artifact-lane-unification.md](tasks/WP03-planning-artifact-lane-unification.md)
...
**Dependencies** : none **Estimated prompt size** : ~320 lines **Prompt file** : [tasks/WP04-mission-identity-phase-1.md](tasks/WP04-mission-identity-phase-1.md)
...
**Dependencies** : none **Estimated prompt size** : ~300 lines **Prompt file** : [tasks/WP05-auth-refresh-race-fix.md](tasks/WP05-auth-refresh-race-fix.md)
...
**Dependencies** : none — owned files (README.md, templates, docs) are independent of all other WPs. **Estimated prompt size** : ~300 lines **Prompt file** : [tasks/WP06-implement-de-emphasis.md](tasks/WP06-implement-de-emphasis.md)
...
**Dependencies** : Depends on WP01, WP02, WP03, WP04, WP05, WP06 all being merged into `main` first. **Estimated prompt size** : ~340 lines **Prompt file** : [tasks/WP07-version-coherence.md](tasks/WP07-version-coherence.md)
...
WP01–WP06: **fully parallel** , no file-ownership conflicts. WP07: **sequential final gate** , runs after all prior WPs are merged.
