...
`tests/architectural/` that pins a codebase invariant via an allowlist/baseline that shrinks over time. | "lint test", "gate test" (imprecise) | `composite_key` ) | `(enclosing_qualname, normalized_token_line)` key from `tests/architectural/_ratchet_keys.py` that survives benign line drift. | "line key", "file:line anchor" | `(rel_path, int_line)` pair (or `path:NNN` string) used as a comparand; banned by the #2077 guard. | — | `(rel, N)` tuple in a module-level seed constant, then feeding `N` through a loop variable into `composite_key(source, N)` — evading the int-to-line-sink detector. This is the #2564 hole. | — | `runtime.next.runtime_bridge` that forwards to a relocated seam module so `monkeypatch.setattr(runtime_bridge, …)` keeps working post-#2531. | "shim" (ambiguous with removed `specify_cli/next` ) | `read_text()` s a module and asserts a substring is/ isn't present, instead of exercising behaviour. | "wiring test" | `status.events.jsonl` , HTTP response body, rendered output, config on disk). | — | `SHARD_GROUPS` mapping ( `tests/_arch_shard_map.py` ) that assigns test roots to CI shard groups. | — | `pytest` . | — |

| Canonical term        | Meaning      | Avoid   |
|-----------------------|--------------|---------|
| Architectural ratchet | A test under |         |

| Content-addressed key(   |
|--------------------------|

| Positional line-anchor   | A raw   |
|--------------------------|---------|

| Seed-tuple laundering   | Storing a raw   |
|-------------------------|-----------------|

| Compat delegate   | A thin re-export in   |
|-------------------|-----------------------|

| Source-as-text test   | A test that   |
|-----------------------|---------------|

| Observable contract   | A persisted artifact a test can assert on without patching the unit under test (event in   |
|-----------------------|--------------------------------------------------------------------------------------------|

| Shard registry   | The   |
|------------------|-------|

| Suite job   | A CI job whose steps invoke   |
|-------------|-------------------------------|
...
`tests/architectural/test_no_dead_symbols.py` + `_symbol_key.py` ) MUST recognise first-party `module.attr` dynamic access (e.g. `_runtime_bridge_module().<name>` ) as a live reference, so a symbol reachable only via dynamic access is classified live, not dead. | #2559 | Planned | `runtime.next.runtime_bridge` façade symbols currently on the permanent allowlist because the gate could not see their dynamic access ( `get_or_start_run` , `query_current_state` , `answer_decision_via_runtime` , `QueryModeValidationError` ) MUST be removed from the permanent allowlist and pass as genuinely-live reachability checks. | #2559 | Planned | `runtime.next.runtime_bridge` compat-delegate surface MUST be retired: each thin delegate whose only role is re-export forwarding is deleted, and every test that patches it via `monkeypatch.setattr(runtime_bridge, <name>)` / `"runtime.next.runtime_bridge.<name>"` is repointed at the owning seam module. | #2561 | Planned | `category_b_grandfathered_legacy` dead-symbol carry-over MUST be burned down by the subset that FR-001 reclassifies as genuinely-dead; symbols that remain live-by-dynamic-access or are load-bearing `doctrine.*` re-exports MUST NOT be deleted, and the allowlist count MUST decrease from its current baseline (193, not the historical 237). | #2293 | Planned |

| ID     | Requirement          | Source   | Status   |
|--------|----------------------|----------|----------|
| FR-001 | The dead-code gate ( |          |          |

| FR-002   | After FR-001, at least the 4   |
|----------|--------------------------------|
...
`test_ratchet_positional_anchor_ban.py` MUST flag a raw `(rel_path, int)` tuple stored in a module-level allowlist seed even when the `int` reaches `composite_key(source, N)` via an intermediate loop/local variable, and the residual raw seed tuples in `test_no_write_side_rederivation.py` / `test_trio_seam_only.py` MUST be converted to content-addressed keys. ( `test_no_write_side_rederivation.py` was verified already content-addressed — WP06 records-and-skips it; only `test_trio_seam_only.py` converts.) | #2564 (P1) | Planned | `status/test_models.py` MUST assert the exact frozenset of `Lane` member names (not `len(Lane) == N` ) and be renamed off "…nine\_values". This is the exemplar pattern for the systematic sweep (FR-014). | #2076 (CT5) | Planned | `read_text()` -based guard in `status/test_agent_status_emit_aggregate_wiring.py` MUST assert a persisted event / rendered output with no `@patch` on the module under test. Its three siblings ( `test_dashboard/test_api_handler.py` , `agent/glossary/test_event_emission.py` , `sync/tracker/test_service.py` ) MUST be audited and re-pointed **only where a genuine source-as-text/wiring twin is confirmed** (they are largely already observable-outcome); no false shared helper is manufactured across the four distinct seams. | #2075 (CT4) | Planned | `tests/_factories/make_mission()` MUST be a thin wrapper over `create_mission_core()` applying test-specific overrides on production-shaped meta, so one schema authority flows a new required field to every consumer. If `create_mission_core()` lacks a side-effect-free / no-coordination-branch entrypoint, adding that entrypoint is in scope; forking the schema is not. | #2074 (CT3) | Planned | `main` (the historical "17 quarantined" is stale — ~1 quarantine mark remains, `tests/retrospective/test_summary_tolerance.py` ) and the genuinely-quarantined residue triaged (re-enable, fix-root, or record a tracked reason). Cross-reference #2309 for reaper-family ownership; do NOT drive the count to 0 or re-enable a #2309-owned test. | #2295 | Planned |

| ID     | Requirement                                                                 | Source   | Status   |
|--------|-----------------------------------------------------------------------------|----------|----------|
| FR-005 | The seed-tuple laundering hole in the positional-anchor ban MUST be closed: |          |          |

| FR-006   | The flagship stale golden-count MUST be corrected:   |
|----------|------------------------------------------------------|

| FR-007   | The confirmed source-as-text wiring test MUST be re-pointed to an observable contract: the   |
|----------|----------------------------------------------------------------------------------------------|

| FR-008   | A production-delegating mission factory MUST be introduced:   |
|----------|---------------------------------------------------------------|
...
| NFR-002 | Every requirement's definition of done MUST be non-fakeable, asserting an observable artifact rather than a restatement.                  | Ratchet re-key:                                                                                                                                 |          |

| NFR-003   | No production behaviour change. The mission edits tests, test tooling, and CI workflow wiring only; the sole production edits permitted are the FR-003 delegate deletions and the FR-008   |
|-----------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|

| NFR-004   | New and changed code passes   |
|-----------|-------------------------------|
...
| C-008   | Lanes topology (no coordination branch).WPs implement in per-lane worktrees (                                                                                                                                                                                                                                                                                                                                             |         |
...
- **runtime\_bridge façade** — `src/runtime/next/runtime_bridge.py` and its post-#2531 seam modules ( `runtime_bridge_{cores,io,identity,engine,composition}` ).
...
Already shipped (do not re-scope): CT1 file:line→composite\_key re-key (#2072/#2547/#2548), CT2 security xfail de-theater (#2073), dead-code scanner rewrite (#2546/#2556), #2077 positional-anchor recurrence guard, tasks.py degod (#2308). Routed out (C-001/C-002): #2463, #2603, #2604, #2465, #2059, #2057, #2026, #2056, #2532, #2560, #2595, #2600, #2309, #2342, #2323, #2499 (optional consolidation — deferred unless the operator explicitly pulls it).