...
**Problem
**: Spec-kitty used "agent" to mean the CLI tooling (Claude Code, OpenCode, etc.). New specifications uses "agent" to mean a role-based identity with capabilities and behavioral rules. Using "agent" for both creates collisions.
...
**Migration**: Canonical glossary language uses `tool` terminology. Current implementation still contains mixed legacy
`agent` identifiers and command groups, so migration remains in progress with compatibility mapping.
