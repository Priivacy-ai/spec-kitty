...
3. **Signature change (decided)**: thread `mission` into `_check_composed_action_guard()` as a **keyword-only parameter** and pass it from `_dispatch_via_composition`. The fail-closed behavior for unknown research actions belongs at the same guard chokepoint — do NOT introduce a sibling helper. Update the existing software-dev call sites to pass `mission="software-dev"` (no behavior change for them; their existing branches still match on `action` alone).
...
3. Tests:
   ...
   - `test_fast_path_does_not_load_frozen_template` — **C-007 strict, no patches/monkeypatches/timing**. The test is purely behavioral: call `_should_dispatch_via_composition(mission='research', step_id=<action>, run_dir=<path-that-does-not-exist>)`. Assert it returns `True`. Rationale: the fast path returns based on `_COMPOSED_ACTIONS_BY_MISSION` lookup alone; if the implementation accidentally called the frozen-template loader, the loader would fall through on a non-existent `run_dir` and the gate would return `False` (or raise). The test asserting `True` against a missing `run_dir` is structural proof that the fast-path didn't do I/O — no patches required.
...
- 2026-04-26T13:12:19Z – claude:opus-4.7:reviewer-renata:reviewer – shell_pid=65939 – Review passed: all 9 reviewer gates green. C-007 grep zero hits; fail-closed default reachable + asserted; helpers match v1 mission-events.jsonl shape; signature mission= keyword threaded; 84/84 tests pass; mypy strict + ruff clean; software-dev branches byte-identical.
