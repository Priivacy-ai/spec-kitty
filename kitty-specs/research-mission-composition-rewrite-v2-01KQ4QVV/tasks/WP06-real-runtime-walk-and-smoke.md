...
3. Tests:
   ...
   - **`test_research_advances_one_composed_step`** — Set up a realistic feature_dir with the artifacts `scoping`'s guard requires (just `spec.md`). Call into the runtime to advance one step. **Do NOT assert directly on `_dispatch_via_composition` being called** (that would require patching it, which C-007 forbids). Instead, assert the observable behavior the dispatch produces: (a) run state advances past the initial step (read status events), (b) the invocation trail's recorded action equals the research-native step ID (`scoping`, not a profile default verb), (c) the invocation lifecycle is paired (started + done), (d) no legacy-fallback events are present in the status events log. These observations together prove the composed path fired.
...
- 2026-04-26T13:36:13Z – claude:opus-4.7:reviewer-renata:reviewer – shell_pid=75841 – Review passed: 8/8 P0 adversarial gates clean. Independent dogfood smoke (rev-smoke-01KQ4ZYD) advanced scoping->methodology, 6 paired invocations. v1 P0 closed reproducibly. C-007 grep zero hits. 5/5 new tests pass. 84/84 regression sweep green. mypy --strict + ruff clean. --force used to bypass false-positive cleanliness check on WP06 owned files (quickstart.md, smoke-evidence.md) which the WP06 frontmatter declares as owned and expected on lane branch.
