...
- **Affected surfaces**: `src/specify_cli/charter_runtime/freshness/computer.py` (`_compute_synthesized_drg` — add the parity read; **campsite**: extract the `built_in_only` branch + the hash-compare tail so the function stays ≤15 / ≤6 returns); `src/charter/activation/consistency_check.py` (**campsite**: pre-extract sub-checks from `_check_reference_id_parity` (complexity 12) before it becomes a read-path dependency); `tests/specify_cli/charter_runtime/` incl. the SC-002 end-to-end seam test (activate → stale → reconcile → fresh) and an NFR-002 preserve-regression (unchanged bundle → unchanged hash).
