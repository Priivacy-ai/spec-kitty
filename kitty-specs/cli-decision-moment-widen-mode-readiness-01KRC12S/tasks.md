...
| T005 | Re-run the acceptance test set and the broader CLI slice; confirm 4 additional previously-failing tests now pass with no new regressions. | WP02 | [D] |

**Total**: 5 subtasks across 2 work packages.
...
**Goal**: Fix the 4 failing `test_plan_widen.py` tests by hardening their `_setup_repo` helper to satisfy the `assert_initialized(require_specs=True)` gate. No production code changes.
...
**Goal**: Fix the 4 pre-existing failures in `tests/specify_cli/cli/commands/test_specify_widen.py` (same root cause: helper does not bootstrap `.kittify/config.yaml` + git, so `_enforce_initialized(require_specs=False)` exits the test before exercising the specify-widen path). Covers the **specify** surface of FR-003. No production code changes.
**Priority**: P0 (release blocker per missions-list.md Mission 1, FR-003)
**Estimated prompt size**: ~80 lines (2 subtasks × ~40 lines each)
**Independent test**: `uv run pytest tests/specify_cli/cli/commands/test_specify_widen.py -q` exits zero.
**Dependencies**: WP01 (reuse the same fixture pattern; in particular, the `git init` + minimal `config.yaml` approach validated there).
**Owned files**: `tests/specify_cli/cli/commands/test_specify_widen.py` (test helper only).
