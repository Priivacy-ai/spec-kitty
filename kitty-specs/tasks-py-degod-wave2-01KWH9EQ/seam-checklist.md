...
| symbol | tasks.py binding (line) | routed-via-`_tasks`? | interception/identity test id | monkeypatch sites swept |
|---|---|---|---|---|
| `_MoveTaskCoordRouter` | 267 (re-import) | yes: `emit_status_transition_transactional` (`commit_status`), `commit_for_mission` (`commit_artifact`) | `test_move_task_orchestration.py:33` direct import from `tasks` (binding identity) + `:221` `feature_write_dir` inheritance pin; interception via `test_atomic_status_commits_unit.py` `@patch("…tasks.emit_status_transition_transactional")`/`@patch("…tasks.commit_for_mission")` (×7 sites) + `test_tasks.py` paired patches (×12) — all bite through the `_tasks.` route; construction stays interceptable because `_default_move_task_ports` (tasks.py) builds via the `tasks.<Router>` binding | none found (`CoordRouter` grep: 0 monkeypatch sites) |
| `_MapReqCoordRouter` | 265 (re-import) | yes: `commit_for_mission` (`commit_artifact`, threads `target_branch`) | `test_tasks_core_backed_orchestration.py:41` direct import from `tasks` (binding identity) + `:189`→`:193` T030 pin: constructs `_MapReqCoordRouter("wip-lane")` under `@patch("…tasks.commit_for_mission")` and asserts the mock receives `target_branch="wip-lane"` — interception proven through the `_tasks.` route; `test_map_requirements.py:676` `@patch("…tasks.commit_for_mission")` bites likewise | none found |
| `_MarkStatusCoordRouter` | 266 (re-import) | yes: `commit_for_mission` (`commit_artifact`, NO threaded `target_branch`) | `test_tasks_coreless_orchestration.py:55` direct import from `tasks` (binding identity) + `:206`→`:209` T033 pin: constructs the router under `@patch("…tasks.commit_for_mission")` and asserts interception + the no-`target_branch` call shape; `test_tasks_canonical_cleanup.py` ×2 + `test_tasks_mark_status.py` seam patches bite likewise | none found |
