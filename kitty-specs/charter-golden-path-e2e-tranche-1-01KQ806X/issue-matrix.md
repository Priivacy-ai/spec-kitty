...
| F5 | `next --result success` for `step_id=discovery → action=research` does NOT create `.kittify/events/profile-invocations/` | `spec-kitty next` | `tests/e2e/test_charter_epic_golden_path.py:674-729` (FR-016 paired-records assertion is gated on `pi_dir.is_dir()`; tight `action == issued_step_id` comparison applies only when records ARE present) | deferred-with-followup | Follow-up: file Priivacy-ai/spec-kitty issue titled "next --result success on the first composed action (`step_id=discovery`, `action=research`) does not write profile-invocation lifecycle records; the `step_id != action` mismatch + missing `pi_dir` together suggest the legacy single-dispatch path is taken instead of the composition path that writes profile-invocation records". When fixed, remove the gate at `test_charter_epic_golden_path.py:687`. |
...
3. File one issue for F5; note the test gate that needs removing when it lands.
