...
- This WP's changes correctly bumped the DRG's edge count from 900 to 916 (16 new `requires` edges from the 7 targeted profiles to `directive:DIRECTIVE_051` / `tactic:dependency-hygiene` / `tactic:supply-chain-install-safety`), and the fixture `tests/doctrine/fixtures/graph-identity.baseline.json` was correctly regenerated to reflect this.
...
5. `agent_profile.graph.yaml` (T012): all new edges use well-formed `<kind>:<id>` URNs (`agent_profile:X` → `directive:DIRECTIVE_051` / `tactic:dependency-hygiene` / `tactic:supply-chain-install-safety`), scoped only to the 7 targeted profiles, targets are pre-existing WP01 nodes (no orphans) — **PASS**
