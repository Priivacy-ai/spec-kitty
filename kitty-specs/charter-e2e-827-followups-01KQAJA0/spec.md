...
| NFR-003 | All affected verification commands **must** be green on the merging branch before PR closeout: `uv lock --check`; `uv run pytest tests/architectural -q` (must include the new `test_uv_lock_pin_drift.py`); `PWHEADLESS=1 uv run pytest tests/e2e/test_charter_epic_golden_path.py -q`; `uv run pytest tests/contract/test_cross_repo_consumers.py -q`; `uv run pytest tests/next -q`; `uv run pytest tests/specify_cli/cli/commands/agent -q`; `uv run pytest tests/integration -k 'dossier or move_task or dirty or transition' -q`; `uv run pytest tests/integration -k 'specify or plan or auto_commit or mission' -q`. | 100% pass on the listed commands | Draft |
