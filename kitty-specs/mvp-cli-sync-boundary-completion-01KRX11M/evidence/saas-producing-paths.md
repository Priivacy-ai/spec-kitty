...
| `src/specify_cli/cli/commands/agent/mission.py:1578` — `finalize-tasks` | Emits `WPCreated` (`emit_wp_created_local`) and `TasksCompleted` (`emit_artifact_phase`); triggers dossier body-upload (`trigger_feature_dossier_sync_if_enabled`). | **Yes (follow-up FR-002 / FR-009 gate)** — guarded by `is_saas_sync_enabled() and not validate_only`, `run_preflight(repo_root=repo_root, require_auth=False)` runs immediately after `locate_project_root()` and refuses with exit 2 on D-3 mismatch, orphan owner record, or legacy rows in scope. Offline / CI invocations with the SaaS-sync feature flag unset are unaffected. |
| `src/specify_cli/cli/commands/agent/tasks.py:2491` — `agent tasks ...` subcommand calling `trigger_feature_dossier_sync_if_enabled` | Triggers dossier body-upload pipeline. | **Indirect** — direct enqueue is not gated at the `tasks` entry point itself, but the body-upload queue rows it produces only ship via `sync now`, which IS gated. WP01's preflight contract treats `sync now` as the SaaS egress chokepoint, and `finalize-tasks` (the primary path into `agent tasks`) is now itself gated above. |
| `src/specify_cli/cli/commands/agent/workflow.py:742` — `workflow ...` subcommand calling `trigger_feature_dossier_sync_if_enabled` | Triggers dossier body-upload pipeline. | **Indirect** — same reasoning as the `tasks` row: enqueue not gated, egress (`sync now`) is. |
