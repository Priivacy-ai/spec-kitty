...
- **SC-001**: 100% of `_flip_phase` writes are *enforced against* the placement port — the port resolves the PRIMARY home and a fail-closed equality assert refuses any mismatch; a synthetic non-PRIMARY caller fails closed (proven by a red-first test). The write target itself remains the caller's canonicalized `feature_dir` by design (FR-001(c): enforcement is a runtime assert, not resolution delegation — a migration writer must never silently retarget the mission directory it was handed). Wording corrected during the landing pass: the prior phrasing ("write-target *resolution* goes through the port") described delegation the design deliberately does not do.
