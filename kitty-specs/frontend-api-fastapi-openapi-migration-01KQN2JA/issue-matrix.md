...
|------|-------|---------|--------------|
| Glossary service extraction | `src/specify_cli/dashboard/handlers/glossary.py` is still the canonical implementation; `src/dashboard/api/routers/glossary.py` is transport-only | `fixed` | Fixed by mission `api-surface-completion-services-aliases-async-01KQSXDA` (WP02): `GlossaryService` extracted to `src/specify_cli/glossary/service.py`; `GlossaryHealthResponse` and `GlossaryTermRecord` TypedDicts moved to `src/specify_cli/glossary/types.py`; all `# TODO(follow-up)` markers referencing #954 removed. Parity tests pass. |
| Lint service extraction | Same shape as glossary — `src/specify_cli/dashboard/handlers/lint.py` is still authoritative | `fixed` | Fixed by mission `api-surface-completion-services-aliases-async-01KQSXDA` (WP03): `LintService` extracted to `src/specify_cli/charter_lint/service.py`; `DecayWatchTileResponse` TypedDict moved to `src/specify_cli/charter_lint/types.py`; all `# TODO(follow-up)` markers referencing #955 removed. Parity tests pass. |
...
- 6 mission-introduced rows: 2 `fixed`, 4 `deferred-with-followup`.
