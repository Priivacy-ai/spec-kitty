...
- **C-002/C-007 bound this explicitly**: C-002 forbids any `template_set`-style migration of
  ...
  SC-006's `git diff --name-only main...HEAD` check applies exactly as spec.md states it — any
  path other than C-007's three named files is a violation — with exactly one explicit carve-out.
  This mission's own `kitty-specs/mission-types-empty-action-sequence-01M0RMCA/**` artifacts
  ...
  not a C-002/C-003/C-007 violation — that is the only carve-out from SC-006's diff check; any path
  outside both C-007's three named files and this mission's own
  `kitty-specs/mission-types-empty-action-sequence-01M0RMCA/**` directory is a violation.
