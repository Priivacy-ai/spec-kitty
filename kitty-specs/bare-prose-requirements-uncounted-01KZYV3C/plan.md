...
**Complexity ceiling** — one surface flagged by `radon cc`
(`_validate_requirement_mapping`, complexity 16 > 15 ceiling) — see "Campsite-Clean
Scope" below (PLAN-GOV-001) and its post-implementation correction: the repo's actually
**enforced** gate is ruff's `C901` check (`pyproject.toml:305-306`, ceiling 15), which
measures this same function at complexity **12** — never in violation by the enforced
tool. `radon` is a different tool with a different mccabe-variant count and was never
the repo's binding gate. The behaviour-preserving extraction still landed in the
campsite-clean FIRST commit (WP02), justified on tidy-first grounds ahead of IC-03's
functional addition, not because the enforced gate was actually red. The Complexity
Tracking table below (for
...
**Correction (post-implementation, recorded 2026-08-14)**: every "9/368 = 2.45%" /
"139/368 = 37.77%" figure below (and throughout this plan) was measured at
design/plan time against an *earlier, broader prototype* that lacked the shipped
function's per-line declared-shape skip rule. The shipped `find_bare_prose_requirement_ids`,
independently re-verified twice, measures **1/368 = 0.27%** against this same corpus
(sole hit: `egress-refusal-consolidation-3110-01KYW895/spec.md`, a foreign-id citation,
zero true positives) — see the docstring's "Reconciliation" note in
`src/specify_cli/requirement_mapping.py` for the full account. The 9/368 and 139/368
figures below are retained, unedited, as the historical basis for the document-scoped
vs. section-scoped design decision (C-006) they informed at plan time; read every
occurrence below as that earlier prototype's measurement, not the shipped function's.
...
**Complexity debt finding (verified, not by inspection alone) — and its
post-implementation correction.** `_validate_requirement_mapping`
(`src/specify_cli/cli/commands/agent/mission_finalize.py:621-677`) measures at
**cyclomatic complexity 16** per `radon cc -s
src/specify_cli/cli/commands/agent/mission_finalize.py -n A`, which reports `F 621:0
_validate_requirement_mapping - C (16)` — one over the charter's stated ceiling of 15
**by that tool**. **Correction (pre-merge squad, recorded 2026-08-14; recurs
`reviews/plan.confirmed.yaml`'s PLAN-GOV-001 severity-4 finding, whose own remediation
— "re-verify with `ruff check --select C901`" — was never carried out before this
correction)**: the repo's actually **enforced** complexity gate is ruff's `C901` check
(`pyproject.toml:305-306`, `max-complexity = 15`), not `radon`. Measured directly against
the pre-refactor function with `ruff check --select C901`, this same function's
complexity is **12** — under the enforced ceiling; it was **never in violation by the
enforced tool**. `radon` is a different tool computing a different mccabe-variant count
and was never this repo's binding gate. IC-03 below schedules exactly this function to
gain a new branch (surfacing `bare_prose_requirement_ids` alongside the existing
missing/unknown/unmapped fields); even accounting for that added branch, the campsite
clean was tidy-first hygiene, not a violation remediation. Per Standing Order 2, a
distinct, behaviour-preserving FIRST commit ahead of a functionally-changed surface is
still good practice independent of whether the enforced gate happened to be red — the
extraction is folded into the campsite-clean FIRST commit on those grounds, not
deferred:
...
- **Verification**: re-run `ruff check --select C901` (the enforced gate) on both
  extracted helpers and the orchestrator after the split, confirming each is
  comfortably under 15, before IC-03's commit is authored. **Post-implementation
  measurement**: `_classify_wp_requirement_refs` and `_emit_requirement_mapping_report`
  both land well under ceiling (`radon cc`: B(6)/B(10)) and the reduced orchestrator at
  A(5) — all comfortably clear regardless of which tool measures them.
...
- `tests/next/test_runtime_bridge_unit.py` — the CLI-native guard family
  ...
  `_check_cli_guards` integration tests. **Exception, recorded post-implementation
  (operator ruling 2026-08-14, DIRECTIVE_041)**: exactly two pre-existing tests in this
  file — `test_requirement_mapping_3394_repro_shape_does_not_block` and
  `test_requirement_mapping_f4_repro_shape_stays_non_blocking_but_logs_advisory` — were
  re-pinned (commit `1b5b86e0f`) because #3396 deliberately supersedes #3395's
  advisory-only decision for exactly this mixed-declaration shape (one of the two IS
  this mission's own target repro). No other file in this bullet list was touched.
...
One surface (`_validate_requirement_mapping`) was flagged by `radon cc` at 16 > 15 (see
"Charter Check" and "Campsite-Clean Scope," PLAN-GOV-001) — but **post-implementation
correction (2026-08-14)**: the enforced gate (ruff `C901`, `pyproject.toml:305-306`,
ceiling 15) measures the same function at 12, so it was never actually in violation by
the enforced tool. No ceiling violation, by the enforced tool, was ever shipped or
needed remediation. This table, which is for violations accepted *with* a justification,
is left empty, honestly, rather than populated to satisfy a template requirement.
