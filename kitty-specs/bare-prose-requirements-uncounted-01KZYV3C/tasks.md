...
(`requirement_mapping.py` ~lines 43-53). **Corrected post-execution (WP03 rework, post-review,
issue #3396 — see `tracer-design-decisions.md`)**: re-verifying the shipped function
against the live corpus found **1/368 = 0.27%**, not 9/368 — the 9/368 and 139/368
figures were an earlier, broader prototype's measurement (taken without the shipped
function's per-line declared-shape skip rule), not this function's own rate. The
shipped docstring records the accurate 1/368 rate as this function's own measured rate,
and retains 9/368/139/368 explicitly labeled as the earlier prototype's historical
figures.
...
still pass, unmodified in their pinned assertions. **Recorded exception, discovered
during this task's own execution and ratified by operator ruling 2026-08-14
(DIRECTIVE_041)**: two tests in `tests/next/test_runtime_bridge_unit.py` —
`test_requirement_mapping_3394_repro_shape_does_not_block` and
`test_requirement_mapping_f4_repro_shape_stays_non_blocking_but_logs_advisory` — pinned
the exact pre-#3396 non-blocking behavior Story 1 exists to newly block; #3396
supersedes #3395's advisory-only decision for that shape, so both were re-pinned and
renamed (commit `1b5b86e0f`), not left broken or silently weakened. `tests/runtime/
test_bridge_cores.py` needed no such exception — it stayed unmodified in its pinned
assertions.
...
`requirement_mapping.py`'s module docstring alongside the shipped **1/368** figure
(WP03's T015; corrected post-execution — 9/368 was an earlier, broader prototype's
figure, not this function's own rate, see T015's correction note above), following
FR-005's own re-verification precedent.
...
(the plan-time expected fixture size; **corrected post-execution**: re-verified against
the shipped detector at this WP's own execution time per T037's own instruction, the
actual fixture is **1 flagged spec**, not 9 — the 9-spec figure was the earlier, broader
design-time prototype's count, superseded by the shipped detector's own 1/368 rate; see
WP03's T015 correction note) and the shrink-only, **non-vacuous** ratchet test (charter
Standing Order 5: concrete
...
T037 Snapshot the fixture specs' per-spec detector signatures into
...
unverified. **Result, recorded post-execution**: re-verification against the shipped
detector found **1 flagged spec** (`egress-refusal-consolidation-3110-01KYW895`), not
9 — the plan-time "9-spec" expectation was the earlier, broader design-time prototype's
count; the committed fixture (`tests/fixtures/bare_prose_corpus_baseline.json`) reflects
the shipped detector's actual 1-spec result, per this subtask's own
re-verify-don't-copy-paste instruction.
...
T039 Add assertion (3) in the same module: for each fixture spec (the committed fixture
holds 1 spec, per T037's corrected result above — the plan-time "9 fixture specs"
phrasing described the earlier design-time prototype's expected count, not the shipped
detector's), the live result is **non-empty** (`assert live_ids`, not only the subset
check) — the concrete floor that (1)+(2) alone do not provide.
...
- T037-T040: This is deliberately **not** a live-scored percentage re-run at CI time —
  it never recomputes the flagged rate (**1/368**, the shipped detector's own measured
  figure — corrected post-execution from the plan-time "9/368" design-time prototype
  figure, see WP03's T015 correction note); it only asks whether the flagged *set* grew
  and is still non-empty where it should be. A future, unrelated mission adding a new
