...
Ratchet**: Implement IC-06 (plan.md): commit the 9-spec baseline signature (plan-time
expected size) and the shrink-only, **non-vacuous** ratchet test (charter Standing
Order 5: concrete floor + self-mutation test + shrink-only allowlist — "a gate-unmask
cannot self-validate"). **Corrected post-execution**: T037's own re-verify-don't-copy
instruction found the shipped detector flags **1 spec**, not 9 — the 9-spec figure was
the earlier, broader design-time prototype's count; the committed fixture holds the
shipped detector's actual 1-spec result.
...
- This is deliberately **not** a live-scored percentage re-run at CI time — never
  recompute the flagged rate (shipped: **1/368**, corrected post-execution from the
  plan-time 9/368 design figure — see WP03's T015 correction note) in the test; only
  ask whether the flagged *set* grew and is still non-empty where it should be.
...
- **Steps**: Run the live, fully-wired `find_bare_prose_requirement_ids` against
  `kitty-specs/*/spec.md`; snapshot the flagged specs' per-spec detector signatures
  ...
  spec.md's plan-time figure, copy-pasted unverified). **Result**: re-verification found
  **1 flagged spec** (`egress-refusal-consolidation-3110-01KYW895`), not the plan-time
  "9 flagged specs" expectation — that 9-spec figure described the earlier, broader
  design-time prototype; the committed fixture reflects the shipped detector's actual
  1-spec result, exactly as this subtask's own instruction required.
...
- **Steps**: For each fixture spec (the committed fixture holds 1 spec — see T037's
  corrected result; the plan-time "9 fixture specs" phrasing described the earlier
  design-time prototype's expected count, not the shipped detector's), assert the LIVE
  result is **non-empty** (`assert live_ids`, not only the subset check).
