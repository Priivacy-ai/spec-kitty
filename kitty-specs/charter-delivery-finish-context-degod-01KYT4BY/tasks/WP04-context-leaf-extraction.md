...
Begin the #2532 decomposition of `src/charter/activation/context.py` (3243 LOC): capture the **non-trivial parity baseline** (after WP01/WP02/WP03), then extract the **low-risk pure/leaf seams** and **dissolve the `profile_sections` import cycle** — with byte-identical output. This WP continues the existing `context_renderers/` package convention; it does NOT change behaviour.
...
- **Cycle**: `context_renderers/profile_sections.py:160-165` function-locally imports 4 symbols from `charter.activation.context` (`_render_fetch_stanza` — already moved by WP01 —, `_budget_estimate`, `_diagnose_catalog_miss`, `_PROFILE_INLINE_BODY_LIMIT_CHARS`). After this WP moves the remaining 3 to leaf homes, replace those with top-level imports and delete both `# noqa: PLC0415`.
...
- **FR-009**: keep every private symbol tests import from `charter.activation.context` importable — via a re-export shim in `context.py` (the shim binds the function object by reference, so caches stay single-source). The full test-import list is in the architect map ([`../notes/`] / data-model.md).
...
Move `_diagnose_catalog_miss` + its private helper `_available_catalog_ids` to `src/charter/activation/context_renderers/catalog_diagnosis.py` (deps: `charter.activation._catalog_miss` only). Declare `__all__`.
