...
| FR-039 | The NEW per-kind readers (`_read_activated_directives`, `_read_activated_tactics`, etc.) treat an empty YAML list as an explicit empty frozenset (zero artifacts available); they must NOT include the `and raw` guard that collapses `[]` to all built-ins. The **existing** `_read_activated_kinds` and `_read_activated_mission_types` readers retain their current two-state behavior (`[]` → all built-ins) for backward compatibility — those keys are grandfathered and their readers are NOT changed. The upgrade command's default-pack write is the mechanism that prevents newly-upgraded projects from inadvertently having an empty per-kind activation set. | Must | Proposed |
