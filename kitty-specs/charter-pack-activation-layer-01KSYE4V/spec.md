...
| FR-040 | A new `src/charter/invocation_context.py` module is created defining `ProjectContext`, `OperationalContext`, and `ContextPreconditionError`; no new `specify_cli.*` package is needed — callers import directly from `charter.invocation_context`; `CharterPackManager` and all wiring sites introduced in this mission accept `ProjectContext` as their primary context parameter; guard methods (`require_repo_root()`, `require_pack_context()`) are called at method entry for any method that uses those fields | Must | Proposed |
