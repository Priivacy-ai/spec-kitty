...
authoritative_surface: src/charter/activation/scope.py
...
**Ownership coordination with WP07:** WP07 owns `src/charter/activation/context.py` and intentionally does NOT change `build_charter_context`'s signature. WP09 introduces a NEW wrapper module `src/charter/activation/scope_router.py` that resolves the scope and calls into `build_charter_context`. The prompt builder calls `scope_router.build_with_scope(...)` instead of calling `build_charter_context` directly. This avoids cross-WP file ownership conflict on `context.py`.
...
**Validation:** `pytest tests/integration/test_monorepo_charter_scope.py tests/charter/test_charter_scope.py -v` MUST FAIL on planning base (ImportError — `charter.activation.scope` doesn't exist yet). Commit RED.
...
### T046 — Create `src/charter/activation/scope.py`

**File:** `src/charter/activation/scope.py` (new)
...
### T047 — Create `src/charter/activation/scope_router.py`

**File:** `src/charter/activation/scope_router.py` (new)
...
- ✅ **`tests/contract/test_example_round_trip.py` case `charter.activation.scope.CharterScopeConfig` flips from SKIPPED to PASSED** — WP03 cycle-2 remediation left this case skip-decorated pending the WP09 model. Landing `CharterScopeConfig` (with its real field shape + validators in `src/charter/activation/scope.py`) MUST turn the case green. This is a binding pre-approval acceptance criterion: the reviewer rejects if the case still shows `SKIPPED` after WP09 lands.
...
5. **WP07 (already merged at this point) changed `context.py` and the scope_router import breaks** — circular import risk. Mitigation: `scope_router.py` imports `charter.activation.context.build_charter_context` and `charter.activation.scope.CharterScope`; both are leaf modules. No circular dependency. Verify by `python -c "from charter.activation.scope_router import build_with_scope"`.
...
pytest tests/integration/test_monorepo_charter_scope.py tests/charter/test_charter_scope.py -v
# EXPECTED: ImportError (charter.activation.scope doesn't exist)
...
- Confirm `src/charter/activation/scope.py` declares `__all__` per WP02's convention (C-007).
...
- Confirm `src/charter/activation/scope_router.py` is a thin wrapper (no business logic beyond scope resolution + delegation).
- Confirm `build_charter_context`'s signature is UNCHANGED in `src/charter/activation/context.py` (WP07 ownership boundary).
