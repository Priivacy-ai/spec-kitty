...
- `grep -rn "from specify_cli\.status\.\|import specify_cli\.status\." src/ --include=*.py | grep -v "src/specify_cli/status/"` returns only the exempted plumbing lines (target: 0 non-exempt).
