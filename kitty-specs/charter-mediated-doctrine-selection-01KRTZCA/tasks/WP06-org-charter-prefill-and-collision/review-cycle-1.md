...
- **Layer rule preserved:** `org_charter.py` imports only `from charter.activation.activations import ActivationEntry`, no `from doctrine.*` direct imports. Zero `from specify_cli` imports in `src/charter/`.
...
The +283 lines in `src/charter/activation/context.py` are **legitimate scope for WP06**, not overreach. Rationale:
...
Implementer report claimed ruff was clean, but `ruff check src/specify_cli/doctrine/ src/charter/activation/context.py tests/specify_cli/doctrine/` reports one finding. Per the charter's Code Quality directives, ruff must be clean on touched paths.
...
```bash
ruff check src/specify_cli/doctrine/ src/charter/activation/context.py tests/specify_cli/doctrine/
```
