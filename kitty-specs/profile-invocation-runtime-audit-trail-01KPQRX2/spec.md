...
- A governance context block injected into `advise`/`ask`/`do` responses, assembled from the DRG using the `(profile, action)` pair resolved for the invocation.
...
- EC-2: The DRG is unavailable or corrupt — governance context assembly fails with a structured error; the operator is directed to run `charter bundle validate`; no `InvocationRecord` is written.
- EC-3: `profile-invocation complete` is called with an `invocation_id` that has already been closed — the command returns a structured warning (already closed); the original record is not mutated.
- EC-4: `profile-invocation complete` is called with an `invocation_id` that does not exist in the local log — the command returns a structured error; no write occurs.
...
| FR-012 | The governance context block returned by `advise`, `ask`, and `do` SHALL be assembled from the DRG using the resolved `(profile_id, action)` pair via the existing `charter context` context-loading mechanism. The governance context hash SHALL be recorded in the `InvocationRecord`. | Accepted |
| FR-013 | `InvocationRecord` events SHALL be written to `.kittify/events/profile-invocations/<profile_id>-<invocation_id>.jsonl` as append-only JSONL. | Accepted |
