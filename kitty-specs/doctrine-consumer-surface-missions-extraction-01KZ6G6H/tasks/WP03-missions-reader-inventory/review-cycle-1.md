...
2. `asset_root.parent / "scripts"` (line 109) and `asset_root.parent / "AGENTS.md"` (line 114) are
   a **third `.parent`-coordination site** alongside R-08 and R-14. Repointing R-09 to
   `packs/built-in/missions` makes `.parent` yield `packs/built-in`, and neither
   `packs/built-in/scripts` nor `packs/built-in/AGENTS.md` exists — both silently stop staging.
...
The neutrality lint's default scan roots recurse `src/doctrine` — which is how the doctrine
mission prompts, `templates/`, and `mission.yaml` files are currently covered. Post-move that
content is no longer under `src/doctrine`, and `packs/built-in/missions` is not a scan root, so
the lint **silently stops scanning it** while continuing to pass. Note the function already
enumerates the *other* missions tree explicitly (`src/specify_cli/missions`, via
`_iter_mission_scan_roots`) — so the doctrine tree needs the same explicit treatment once it
leaves `src/doctrine`. This is a gate-coverage loss with no red test, i.e. exactly the class
NFR-001 cannot catch. `repoint`.
...
7. **`src/specify_cli/template/manager.py`** — R-12/R-13 name two sites; there is a third at
   lines 122-127, `get_local_repo_root._is_template_root`, which tests
   `(path / "src" / "doctrine" / "missions").is_dir()`. Post-move that stays `True` against the
   data-less directory, so a checkout with no mission data is still accepted as a valid template
   root — the predicate is weakened rather than broken. File already owned; the row should name it.
8. **`src/doctrine/drg/migration/extractor.py`** is missing from the "would silently resolve to
...
`src/doctrine/mission-steps`. Post-move `.parent` becomes `packs/built-in/missions`, still
unequal. So this is a **pre-existing latent mismatch, not a move regression** — which is exactly
why it belongs in your "Bugs found, not fixed" section rather than the repoint set. Recording it
also keeps a future reader from "fixing" `_builtin_root.parent` as part of WP05 and changing
behaviour under cover of the relocation.
...
1. Add `repoint` rows for **B-1, B-2, B-3, B-4**, and add all four to WP05's `owned_files`.
...
4. Correct the five false "no independent path construction" claims in Part 4, name the third
   `manager.py` site, and add the extractor to the dangerous-class list.
5. Add the Part 4b latent mismatch to "Bugs found, not fixed".
6. Drop the hard-coded count from the `research.md` pointer.
