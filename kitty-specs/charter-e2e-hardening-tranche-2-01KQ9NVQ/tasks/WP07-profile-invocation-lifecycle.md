...
- 2026-04-28T13:28:57Z – claude:opus:reviewer-renata:reviewer – shell_pid=11239 – WP07 approved: invocation/errors.py adds InvalidOutcomeError; record.py adds CANONICAL_OUTCOMES frozenset, NEXT_RESULT_TO_OUTCOME mapping, coerce_outcome() helper preserving canonical Literal[done|failed|abandoned]; writer.py wires coerce_outcome before write_completed I/O. New integration test tests/integration/test_profile_invocation_lifecycle.py walks scoping action via decide_next_via_runtime issue+advance, asserts paired started/completed share invocation_id, action matches issued step, outcome=='done'. Test passes (1/1, 2.46s). Owned-file boundary respected; no runtime_bridge.py edit needed (composed actions already route through ProfileInvocationExecutor). --force used to bypass gitignored dossier snapshot.
