...
├── implementation-baseline.json # atomic frozen target-tip + WP01-start diff anchors
...
- **Risks / required content**: One named independent reviewer (squad or operator) answers the spec's exact six questions; operator review is optional, not a second gate. Immediately before WP01 edits, fetch/check target and atomically freeze its tip with implementation base; all later WPs re-run the identical audit at that same target snapshot with manifest arithmetic—never a mid-mission refetch. A different incorporated target forces the fresh-branch/planning-replay/WP01-restart procedure. Reject ownerless deferrals/TBDs; dry-run M1 with zero new decisions; validate guard and docs-contract CI. C-001 compares both frozen target-tip planning base and implementation base from `implementation-baseline.json`, including committed and working-tree changes.
