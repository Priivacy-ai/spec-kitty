...
`inventory.md` (frozen base `2621a56d06b9ae4e7da07ee206879c30a4d8b363`, tree `26e6fdd2…`, TSV `d8a09ef1…`, 48,964 rows,
...
archive = single fixed exclusion root), `DM-01M0P6C8C7Q6SPBT412V39RPN0` (amends `DM-01M0NMS9WPH33EPFCJQRTQVNSA`: four
fixed exclusion roots — `kitty-specs/`, `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`,
`.kittify/missions/`), `DM-01M0NMSD60JYG7K7V5MJCKJ3P8` (ephemeral manifest) · **Updated**: 2026-08-23
...
- **Base capture** (methodology §4.1): fetch, require the target tip incorporated, atomically persist the wave's own
  ...
  ':(top,exclude).kittify/migrations/mission-state/quarantine/' ':(top,exclude)kitty-ops/'
  ':(top,exclude).kittify/missions/'`, `ls-tree --full-tree` + four-root prefix drop, symlink-target and
  normalised-content passes, `mode=inventory`) at that tip; write the wave's `occurrence_map` (TSV SHA-256 + counts +
  exact owned hit set derived by the ordered OC/CR rule tables, plus any finer predicate the wave adds above an
  existing rule); pin the previous wave's closing manifest as the guard baseline. **Every wave's opening audit
  re-derives ownership by the rule tables** — rows created after the frozen base (e.g. this program's own ADR under
  `docs/adr/`, ~48 hits → OC-29/M5) fall to the owner the rules give them; M1's guard baseline is M1's own opening
  manifest, not the frozen-base `d8a09ef1…`.
...
- **Archive gate** (executable form; methodology §3.5): no **pre-existing** path under any of the four fixed exclusion
  roots (`kitty-specs/`, `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`, `.kittify/missions/`) is
  edited, renamed, or deleted by the wave; runtime may keep appending new records to the latter three. Test form: for
  each root, `git diff --name-status $(git merge-base <target-branch> <wave-tip>) <wave-tip> -- <root>` contains only
  `A` entries — for `kitty-specs/`, all under the wave's own `kitty-specs/<wave-slug>-<mid8>/`; for the three record
  roots, only new runtime-appended records. A whole-range base→result diff on the target branch is **not** the test
  (unrelated missions mutate their own archive directories or append their own records). The terminal and wave-local
  audits still exclude all four roots in full. Referrers outside the four roots that cite an archive path containing
  the token are re-cited by `mission_id`/mid8 or a token-free path (M5); the archive paths themselves never change.
- **Guard** (methodology §2.1, re-keyed 2026-08-23): tree-independent shrink-only fingerprint — per audited path, the
  ...
  of the wave's own CRs; `test_transition_guard_shrink_only` in every merge gate; the baseline store lives **untracked
  only** (never inside any of the four fixed exclusion roots, even the wave's own mission dir — a tracked store there
  would force M6's deletion of it to show as a `D` entry under `kitty-specs/`, violating M6's own archive gate, which
  forbids any edit/rename/delete of a pre-existing archive path); M6 deletes the store. (`match_sha256` embeds the
  tree OID and is never compared across trees.)
...
- **Exclusion-set re-derivation (2026-08-23, `DM-01M0P6C8C7Q6SPBT412V39RPN0`)**: the immutable historical-record
  exclusion set grew from the single `kitty-specs/` root to four fixed roots (`kitty-specs/`,
  `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`, `.kittify/missions/`), removing 83 content + 3
  pathname rows from M5's owned set: OC-33 284→262 (−22, `kitty-ops/` portion), OC-34 320→259 (−61, the
  `.kittify/migrations/mission-state/quarantine/` (54) and `.kittify/missions/` (7) portions), OC-49 3→0 (all 3 were
  quarantine pathnames). The TSV, its SHA-256, and all counts changed accordingly (`inventory.md` §8 amendment; new
  TSV `d8a09ef1…`, 48,964 rows). Sums after this re-derivation: M1 302 · M2 13,344 · M3 111 · M4 564 · M5 34,643 ·
  M6 0 = **48,964**.
...
| `purpose` | Rewrite/rename every remaining current-tree prose/history/ADR/docs/evidence occurrence, filename, and referrer outside the four fixed exclusion roots; regenerate serialized docs data; leave all four roots byte-identical for pre-existing paths (I5). |
| `depends_on` | M4 |
| `inputs` | I4 tree; `inventory.md` OC-26/29/30/31/32/33/34/35/47 (OC-49 has zero surviving rows at this base) plus rows created after the frozen base that the rule tables assign to M5 (e.g. this program's ADR, OC-29); R10; `methodology.md` §1.3(5), §3.3 (M5 row), §3.5; `tests/architectural/test_marker_job_completeness.py:82-85` (reads `docs/reports/test-sanitation/**/raw/wp07-route-manifest.yaml`, OC-30); resolved operator decision `DM-01M0P6C8C7Q6SPBT412V39RPN0` (option 1: serialized historical records keyed to archive slugs / retired profile IDs — `.kittify/migrations/mission-state/quarantine/**`, `kitty-ops/*.jsonl`, `.kittify/missions/**/retrospective.yaml` — join the fixed exclusion set) |
| `outputs` | canonical ADR bodies/titles/files under `docs/adr/**` (incl. `2026-08-22-2-…` and `2026-07-15-1-…`, renamed), docs prose/reference/guides/architecture/context/changelog, plans/investigations, `docs/reports/**` (test-sanitation census/dispositions), `research-outputs/**` (`kitty-ops/**` is now a fixed exclusion root, not an M5 output), `.kittify/memory|evidence/**` and `.kittify/migrations/**` outside the quarantine root (`.kittify/missions/**` is now a fixed exclusion root), `metadata.yaml`, root docs (`AGENTS.md`, `CLAUDE.md`, `README.md`, `CONTRIBUTING.md`, `CHANGELOG.md`), renamed docs pathnames (OC-47) with every referrer updated; regenerated retrieval index/page inventory/ownership manifest/toc/redirect maps (OC-26) via their generators (`freshen_adr_inventory`, docs tooling); OC-30 `docs/reports/test-sanitation/**` (27,990 rows) is **rename-in-place of historical evidence** — its producer ran on a historical suite, `audit.py` there is code-in-prose, path citations inside it will point at M2-renamed tests by design; archive referrers re-cited by `mission_id`/mid8 or token-free path; serialized historical records per resolved `DM-01M0P6C8C7Q6SPBT412V39RPN0` (excluded, not M5 work); wave `occurrence_map` |
| `base_capture` | §0; guard baseline = M4 closing manifest |
| `occurrence_map` | OC-26 611 · OC-29 815 · OC-30 27,990 · OC-31 2,960 · OC-32 1,660 · OC-33 262 · OC-34 259 · OC-35 14 · OC-47 72 = **34,643** |
| `retires_oc` | OC-26, OC-29, OC-30, OC-31, OC-32, OC-33, OC-34, OC-35, OC-47, OC-49 (OC-49's rule survives with 0 rows at this base — see `inventory.md` §3) |
| `introduces_compatibility` | none (prose has no compatibility channel) |
| `removes_compatibility` | none |
| `owned_files_or_surfaces` | everything in `retires_oc` plus the generators' outputs; never a pre-existing path under any of the four fixed exclusion roots |
| `tests` | `test_prose_history_closure_outside_archive`, `freshen_adr_inventory --check`, docs link/freshness/SEO gates with the link check scoped by a `:(exclude)<root>` pathspec per fixed exclusion root (the archive cites renamed paths by design), `test_marker_job_completeness` (OC-30 consumer) green, `test_serialized_surfaces_canonical_writers` (docs-data part), `test_transition_guard_shrink_only`, `test_archive_root_byte_identical` |
| `merge_gate` | pre-edit rename/re-cite map + OC-30/OC-31 sampled-diff review approved **before the first prose edit** (new bounded gate, see below); no M5-owned (or earlier-wave-owned) row remains at the closing audit except recorded quotation-fidelity exclusions; no dangling renamed reference outside the four fixed exclusion roots; generated docs data fresh; archive gate in its §0 test form (per root: `git diff --name-status $(git merge-base <target> <tip>) <tip> -- <root>` → only `A` entries — under M5's own mission dir for `kitty-specs/`, runtime-appended records for the other three) |
| `rollback` | before M6 lands revert the wave (renames are reversible); after → forward-fix |
| `change_mode` | `bulk_edit` |
| `invariant_after` | **I5** |
| `local_design_questions` | **0** — `DM-01M0P6C8C7Q6SPBT412V39RPN0` (disposition of tracked serialized runtime records keyed to immutable archive slugs or retired profile IDs) is **resolved** (option 1: exclude alongside `kitty-specs/`); it was the only M5 question; no history exemption beyond the four fixed roots otherwise; re-cite rule fixed |
...
**M5 pre-edit dry-run gate.** M5 (34,643 rows, including the 27,990-row OC-30 in-place rewrite) currently has no
...
| `purpose` | Remove every compatibility alias/key/path/control/fixture/baseline/allowlist and transition-guard record, replace detector literals with numeric-byte construction, and prove both exact zero audits over `HEAD` with the four fixed exclusion roots (I6; 4.0 boundary). |
| `depends_on` | M5 |
...
| `retires_oc` | none from the frozen base (all 43 populated OCs are retired by M1–M5; OC-49's rule survives but carries zero rows at this base — see `inventory.md` §3) |
...
| `merge_gate` | `python scripts/audit_retired_term_zero.py --commit <final-commit-oid> --mode terminal --json -` run at the repository toplevel exits 0 (content raw rc 1 + empty stdout with `':(top)' ':(top,exclude)kitty-specs/' ':(top,exclude).kittify/migrations/mission-state/quarantine/' ':(top,exclude)kitty-ops/' ':(top,exclude).kittify/missions/'`; pathname raw rc 0 + zero matches after the four-root drop on `--full-tree`; symlink targets zero; normalised-content pass zero; exit 1 = hits, 2 = audit/git/input error incl. non-toplevel cwd); attestation binds the resolved commit + tree OIDs; required external check `terminology-zero-current-tree` on the merge/publish result tree — no earlier working-tree or parent-commit zero result authorizes merge/publication; an entrypoint that omits any of the four fixed exclusion roots or adds any other fails; no program-introduced baseline/allowlist/skip/X survives; CR registry empty; archive gate |
...
**Resolution note (`DM-01M0P6C8` resolved, 2026-08-23; supersedes an earlier "contingent on deferral" note).** M6's
terminal audit contract above uses the **finalized four-root exclusion set**. The disposition question this note
previously flagged as open — `DM-01M0P6C8C7Q6SPBT412V39RPN0` (disposition of tracked serialized runtime records
outside `kitty-specs/`: quarantine `status.events.jsonl`, `kitty-ops/*.jsonl`,
`.kittify/missions/**/retrospective.yaml`) — resolved to **Option 1**: exclude alongside `kitty-specs/`, amending
`DM-01M0NMS9WPH33EPFCJQRTQVNSA` from a single fixed root to four (`kitty-specs/`,
`.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`, `.kittify/missions/`). This table's `purpose` and
`merge_gate` fields above, `inventory.md` §8, and `contracts/inventory-schema.md` already reflect the four-root
argv/drop. I6's exclusion set is final; issue #3684 tracked the amendment this resolution addresses.

## 2. Assignment tables (T014)

### 2.1 One row per OC — exactly one M1–M6 primary owner

Membership = all rows of `inventory-hits.tsv` (SHA-256 `d8a09ef1…`, four-fixed-root regeneration) whose
`occurrence_class_id` equals the OC; counts, files and ID spans are `inventory.md` §3. `OC-05` and `OC-50` are
declared ids with zero rows (unused placeholders); `OC-49`'s rule also carries zero rows at this base (its three
frozen-base rows were all under the newly-fixed `.kittify/migrations/mission-state/quarantine/` root).

| OC | S | Seam (short) | Rows | Files | ID span | Owner |
|---|---|---|---|---|---|---|
| OC-01 | S2 | glossary authorities (docs context, project YAML, built-in pack, contextive) | 221 | 8 | `H-C-000395`…`H-C-038516` | **M1** |
...
| OC-04 | S6 | project overlay root `.kittify/doctrine/**`, overrides | 55 | 13 | `H-C-000290`…`H-C-000622` | **M3** |
| OC-06 | S4 | built-in agent assets | 171 | 69 | `H-C-034905`…`H-C-035172` | **M4** |
| OC-07 | S4 | built-in mission prompts/step contracts | 51 | 22 | `H-C-035028`…`H-C-035078` | **M4** |
| OC-08 | S6 | pack structure/manifests/assets, `packs/internal` | 42 | 13 | `H-C-034897`…`H-C-035184` | **M3** |
| OC-09 | S4 | skill sources `src/doctrine/skills/**` | 253 | 35 | `H-C-038044`…`H-C-038296` | **M4** |
| OC-10 | S4 | host agent-dir prompts | 12 | 12 | `H-C-000001`…`H-C-000634` | **M4** |
| OC-11 | S8 | generated profile/skill API docs | 75 | 24 | `H-C-001464`…`H-C-001691` | **M4** |
| OC-12 | S1 | CLI routes/help/errors | 633 | 39 | `H-C-038650`…`H-C-039282` | **M2** |
| OC-13 | S10 | tracker ownership seam | 40 | 4 | `H-C-039813`…`H-C-039852` | **M2** |
| OC-14 | S7 | old package build/distribution | 56 | 2 | `H-C-037712`…`H-C-037951` | **M2** |
| OC-15 | S8 | old package schemas/templates | 48 | 23 | `H-C-037965`…`H-C-038371` | **M2** |
| OC-16 | S7 | `src/doctrine/**` code | 815 | 91 | `H-C-037218`…`H-C-038389` | **M2** |
| OC-17 | S7 | `src/charter/**` consumers | 1,657 | 95 | `H-C-035561`…`H-C-037217` | **M2** |
| OC-18 | S8 | generated manifests | 54 | 2 | `H-C-000202`…`H-C-038564` | **M2** |
| OC-19 | S7 | specify_cli doctrine modules | 312 | 27 | `H-C-038584`…`H-C-039563` | **M2** |
| OC-20 | S7 | other specify_cli consumers | 413 | 90 | `H-C-038565`…`H-C-039962` | **M2** |
| OC-21 | S7 | kernel/runtime/glossary/mission_runtime | 110 | 21 | `H-C-038390`…`H-C-038499` | **M2** |
| OC-22 | S7 | test fixtures/controls/baselines/allowlists | 641 | 28 | `H-C-040012`…`H-C-045123` | **M2** (control machinery itself deleted by M6 as later-created/retargeted coordinates) |
| OC-23 | S7 | architectural gate tests | 1,566 | 70 | `H-C-039999`…`H-C-041702` | **M2** |
| OC-24 | S7 | test code | 6,076 | 641 | `H-C-039963`…`H-C-048245` | **M2** |
| OC-25 | S9 | CI workflows | 173 | 13 | `H-C-000007`…`H-C-000179` | **M2** |
| OC-26 | S8 | serialized docs data (index, inventory, manifest, toc, redirects) | 611 | 9 | `H-C-001692`…`H-C-035512` | **M5** |
| OC-27 | S9 | scripts | 51 | 8 | `H-C-035487`…`H-C-035560` | **M2** |
| OC-28 | S9 | root build/lint/config metadata | 67 | 6 | `H-C-000180`…`H-C-035486` | **M2** |
| OC-29 | S3 | ADRs | 815 | 65 | `H-C-000649`…`H-C-001463` | **M5** |
| OC-30 | S3 | test-sanitation evidence reports | 27,990 | 33 | `H-C-006905`…`H-C-034894` | **M5** |
| OC-31 | S3 | plans/investigations | 2,960 | 190 | `H-C-003945`…`H-C-006904` | **M5** |
| OC-32 | S3 | docs prose | 1,660 | 115 | `H-C-001517`…`H-C-034896` | **M5** |
| OC-33 | S3 | research-outputs (`kitty-ops/` is now a fixed exclusion root; its 22 rows left this class 2026-08-23) | 262 | 36 | `H-C-035215`…`H-C-035476` | **M5** |
| OC-34 | S3 | memory/evidence/mission-state history (`.kittify/migrations/mission-state/quarantine/` (54) and `.kittify/missions/` (7) are now fixed exclusion roots; those 61 rows left this class 2026-08-23) | 259 | 21 | `H-C-000294`…`H-C-000571` | **M5** |
| OC-35 | S3 | root repository docs | 14 | 1 | `H-C-000635`…`H-C-000648` | **M5** |
| OC-40 | S2 | pathname `docs/context/doctrine.md` | 1 | 1 | `H-P-000037`…`H-P-000037` | **M1** |
| OC-41 | S4 | skill source pathnames (`src/doctrine/skills/**` tree) | 83 | 83 | `H-P-000193`…`H-P-000275` | **M2** (moved from the M4 default: the tree is `doctrine` package data resolved by `src/specify_cli/skills/registry.py:44,60,67`, shipped by `pyproject.toml:180-185`, counted by `release.yml:226` — M2 relocates it; skill IDs/content OC-09 stay M4) |
| OC-42 | S7 | `src/doctrine/**` pathnames | 181 | 181 | `H-P-000097`…`H-P-000360` | **M2** |
| OC-43 | S7 | test pathnames | 332 | 332 | `H-P-000388`…`H-P-000719` | **M2** |
| OC-44 | S7 | code pathnames | 30 | 30 | `H-P-000094`…`H-P-000387` | **M2** |
| OC-45 | S6 | project overlay pathnames | 14 | 14 | `H-P-000004`…`H-P-000017` | **M3** |
| OC-46 | S4 | `doctrine-daphne`, `018-…` pathnames | 2 | 2 | `H-P-000091`…`H-P-000092` | **M4** |
| OC-47 | S3 | docs pathnames | 72 | 72 | `H-P-000018`…`H-P-000090` | **M5** |
| OC-48 | S9 | repo-ops pathnames | 4 | 4 | `H-P-000001`…`H-P-000093` | **M2** |
| OC-49 | S3 | history pathnames (all 3 frozen-base rows were quarantine pathnames, now under the fixed `.kittify/migrations/mission-state/quarantine/` exclusion root; rule survives for any future non-excluded match) | 0 | 0 | (none at this base) | **M5** |
...
| M5 | OC-26, 29, 30, 31, 32, 33, 34, 35, 47, 49 | 611 + 815 + 27,990 + 2,960 + 1,660 + 262 + 259 + 14 + 72 + 0 = **34,643** |
| M6 | none (frozen base) | **0** |
| **Union** | 43 populated OCs, each listed exactly once (OC-49's rule is still listed above but carries 0 rows) | **48,964** = 48,245 content + 719 pathname = all TSV rows |

Pairwise disjointness follows from each OC appearing in exactly one wave list above (checked by enumeration: 3 + 22 + 3
+ 6 + 9 + 0 = 43 = the number of populated classes in `inventory.md` §3, after `DM-01M0P6C8C7Q6SPBT412V39RPN0`
zeroed OC-49). Per surface the union reproduces `inventory.md` §2: S1 633 (M2) · S2 222 (M1) · S3 34,032 (M5) · S4 572
(OC-41 M2 83 + the rest M4 489) · S5 82 (OC-02 M1 80 + OC-03 M2 2) · S6 111 (M3) · S7 12,189 (M2) · S8 788 (OC-11 M4
75 + OC-15 M2 48 + OC-18 M2 54 + OC-26 M5 611) · S9 295 (M2) · S10 40 (M2). The partition is identical to
`methodology.md` §1.2 (re-derived 2026-08-23: OC-03 M1→M2, OC-41 M4→M2; re-derived again 2026-08-23 for the
four-root exclusion). Before the OC-03/OC-41 re-derivation the sums were 304 / 13,259 / 111 / 647 / 34,729 / 0 on a
49,050-row TSV; before the four-root exclusion they were 302 / 13,344 / 111 / 564 / 34,729 / 0 = 49,050 (single
`kitty-specs/` root, TSV SHA-256 `3631531b…`) — the figures WP05 and the mission review reproduced. The current
34,643/48,964 figures reflect both re-derivations on the regenerated TSV (SHA-256 `d8a09ef1…`).
...
| guard | arm last: tree-independent per-path fingerprint (§0); baseline store **untracked only** (never inside `kitty-specs/`); baseline = M1's own opening manifest; `test_transition_guard_shrink_only` wired into the merge gate | baseline hash |
...
- **M5**: no history exemption beyond the four fixed exclusion roots (`kitty-specs/`,
  `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`, `.kittify/missions/`), none of which it edits or
  renames for a pre-existing path; re-cite rule for archive referrers; generated docs data regenerated; OC-30
  rename-in-place; link check scoped outside the four fixed exclusion roots; filename/referrer closure. The operator
  decision that previously gated M5 (`DM-01M0P6C8C7Q6SPBT412V39RPN0`, serialized historical records) is **resolved**
  (option 1: exclude alongside `kitty-specs/`); M5 has zero remaining local design questions.
...
Forbidden terminal states — X1/X2/X3, "user-visible only", supported-surface-only, immutable current-tree outside the four
fixed exclusion roots, runtime managed-path ledger — appear in this plan only as rejections.
