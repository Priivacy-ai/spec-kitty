...
| CR-06 | `import doctrine`/`doctrine.api` → the named `src/charter/` sub-package (module-only shim re-exports + DeprecationWarning; every `importlib.resources.files("doctrine")` site retargeted before the shim lands) | OC-16 (31) | M2 | 8 | shim module list + warning test | `test_doctrine_import_shim_warns`, `test_charter_api_is_canonical_surface` | M6 |
...
| M2 → M3 | canonical code topology incl. the named `src/charter/` sub-package, relocated skills tree, dual-root reader/migrator (CR-07 active, data still at the old root) | I2; map hash recorded; no M1/M2-owned live code hit outside CR-02…07; boundary gates green |
...
2. Build `canonical-operator-surface-map.md`: one `MAP-###` row per M2-owned OC/hit group with `surface_kind ∈
   ...
   import under `src/doctrine/**`; the named offer-side sub-package and the enumerated collision set (`__init__.py`,
   ...
   workflow/CI/script/generated-template/build-system coordinates.
3. Resolve every collision with the existing `src/charter/` aggregate to `merge-existing` (exact target module inside
   the named sub-package / facade) or exact `relocate` — **before the first source edit**; unresolved rows block; the
   one-way import rule and the renamed boundary gates must be green in the dry run.
