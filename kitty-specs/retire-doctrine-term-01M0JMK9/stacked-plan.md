...
- **Guard** (methodology §2.1, re-keyed 2026-08-23): tree-independent shrink-only fingerprint — per audited path, the
  ...
  only** (never inside `kitty-specs/`, even the wave's own mission dir — a tracked store there would force M6's
  deletion of it to show as a `D` entry under `kitty-specs/`, violating M6's own archive gate, which forbids any
  edit/rename/delete of a pre-existing archive path); M6 deletes the store. (`match_sha256` embeds the tree OID and is
