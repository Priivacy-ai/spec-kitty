...
2. **Given** the existing ADR `2026-07-15-1-doctrine-offers-charter-activates-runtime-consumes.md`, **When** the new ADR is reviewed, **Then** it explicitly amends or supersedes the terminology portion of that ADR while leaving its resolution mechanics intact, and the old ADR's status is updated accordingly in the index.
3. **Given** the new ADR, **When** a reviewer reads it without other context, **Then** they can state unambiguously: (a) what "charter" names, (b) the three-way distinction (charter.md file / active charter / inactive charter), (c) which terms survive, (d) the scope boundary (internal identifiers untouched), and (e) the compatibility policy.
...
3. **Given** the terminology guard (`tests/architectural/test_no_legacy_terminology.py`), **When** a wave lands, **Then** the methodology states how the guard is updated shrink-only so it stays green at every stack level.
...
- **The charter file is doctrine-laden and load-bearing.** `.kittify/charter/charter.md` uses "doctrine" heavily and feeds `spec-kitty charter sync` / context resolution (Charter Resolution Hints block); updating it must not break context resolution or the compiled reference set.
...
- **Deprecation aliases must actually work.** Old executable names (e.g. `spec-kitty doctor doctrine`) must keep functioning with a deprecation warning during the 3.x window; the plan must define how alias behavior is tested and how removal at 4.0 is verified (audit, not assumption).
...
| FR-002 | Three-way distinction defined | As an operator, I want the ADR to define *charter.md file*, *active charter* (wired in), and *inactive charter* as distinct canonical concepts, so that the vocabulary is unambiguous. | High | Open |
...
| FR-004 | Scope boundary recorded | As an operator, I want the ADR to record that internal code identifiers (`src/doctrine/` package, module names, import paths) are out of scope for the retirement, so that downstream missions do not expand scope. | High | Open |
...
|----|-------|-------------|----------|----------|--------|
| NFR-001 | Inventory completeness | 100% of user-facing surface categories are enumerated with occurrence counts; a case-insensitive audit finds 0 unclassified "doctrine" occurrences in in-scope user-facing surfaces. | Completeness | High | Open |
...
| C-003 | Historical artifacts immutable | Old ADRs and archived missions retain legacy wording as immutable snapshots, explicitly marked legacy; they are not rename targets. | Scope | High | Open |
| C-004 | Terminology guard integrity | Downstream waves keep `tests/architectural/test_no_legacy_terminology.py` green at every stack level; the guard is updated shrink-only as each wave lands. | Technical | High | Open |
...
- **charter.md file**: The compiled governance document at `.kittify/charter/charter.md`; the first canonical sense of "charter".
...
| **charter.md file** | The compiled governance document (`.kittify/charter/charter.md`). | "the charter" when the activation state is meant; "constitution" (legacy, pre-063) |
...
- **SC-001**: The ADR is merged and registered in the 3.x index; an independent reviewer states the three-way distinction, surviving kinds, scope boundary, and compatibility policy correctly from the ADR alone (1/1 review pass).
- **SC-002**: The inventory covers 100% of user-facing surface categories with occurrence counts and stable class identifiers; a spot-check audit finds 0 unclassified user-facing occurrences in in-scope surfaces.
