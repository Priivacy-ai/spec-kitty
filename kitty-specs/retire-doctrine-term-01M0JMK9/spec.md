...
`DM-01M0NMS9WPH33EPFCJQRTQVNSA` (the `kitty-specs/` historical archive is immutable),
`DM-01M0NMSD60JYG7K7V5MJCKJ3P8` (`inventory-hits.tsv` is ephemeral evidence), and
`DM-01M0P6C8C7Q6SPBT412V39RPN0` (the immutable historical-record exclusion set is four fixed roots —
`kitty-specs/`, `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`, `.kittify/missions/` — not
`kitty-specs/` alone). This mission plans complete extinction of the case-insensitive token `doctrine`
from the repository's current tree outside the four fixed, enumerated exclusion roots. At M6/I6:

- forced-text content audit over `HEAD` with the four fixed pathspecs (one `:(exclude)<root>` per root)
  returns zero occurrences;
- NUL-safe tracked-path audit over `HEAD`, after dropping paths under any of the four roots, returns zero
  matching pathnames;
- no X1/X2/X3, internal, historical, fixture, control, metadata, generated, test, or current-tree ADR
  exception remains anywhere outside those roots;
- the two fixed exclusion *kinds* are Git object history outside `HEAD` and the four immutable
  historical-record roots. Each root is an operator-fixed audit boundary, not an occurrence class,
  allowlist, baseline, or X-classification; no wave renames a mission slug or directory, or edits/renames
  any pre-existing file under any of the four (runtime may keep appending new records), and any narrowing
  beyond those four roots fails closed.
...
3. The ADR states the narrow override: for this program only, current-tree terminology extinction
   ...
   historical-current-tree immutability outside the four fixed exclusion roots. It never authorizes data
   loss: content is preserved at a canonical destination and conflicts block migration until resolved. Git
   object history and the four fixed exclusion roots remain untouched (for pre-existing paths; runtime
   may keep appending new records to `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`, and
   `.kittify/missions/`).
...
Every case-insensitive content occurrence and tracked pathname at the frozen base outside the four fixed
exclusion roots is a work item.

1. WP02 runs the forced-text all-blob and NUL-safe pathname audits from `contracts/inventory-schema.md`,
   both applying the four fixed exclusion roots (`kitty-specs/`,
   `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`, `.kittify/missions/`).
...
4. Scope includes non-public code, `src/doctrine/`, internal symbols/imports/files, tests, fixtures,
   ...
   event/history prose, and filenames/referrers. Content and pathnames inside any of the four fixed
   exclusion roots are recorded only as non-contractual per-root orientation counts.
...
5. M5 rewrites/renames every remaining current-tree prose/history surface outside the four fixed exclusion
   roots, including ADRs, docs archives, evidence, filenames, and all referrers; a referrer that cites an
   immutable archive path containing the token is rewritten to cite the mission by `mission_id`/mid8 or a
   token-free path. Git object history and the four fixed exclusion roots are untouched (runtime may keep
   appending new records to `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`, and
   `.kittify/missions/`).
...
- **Current-tree history**: M5 rewrites historical prose/files outside the four fixed exclusion roots in
  the checked-out tree under the explicit override. Prior bytes remain available only from Git object
  history.
- **Historical missions and serialized runtime records**: `kitty-specs/` is an immutable archive; tracked
  serialized runtime records keyed to immutable archive slugs or retired profile IDs (mission-state
  quarantine `status.events.jsonl` under `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/*.jsonl`,
  `.kittify/missions/**/retrospective.yaml`) are likewise treated as immutable historical records
  (`DM-01M0P6C8C7Q6SPBT412V39RPN0`). No pre-existing slug, directory, or file under any of the four roots
  is renamed or edited by any wave; runtime may keep appending new records; both audits exclude all four by
  independent fixed pathspecs.
...
| FR-004 | Every current-tree content/path hit outside the four fixed exclusion roots is in scope, including non-public/internal/history/test/generated/metadata surfaces. |
| FR-005 | 3.x warning aliases are temporary; completed 4.0 and completed migrations contain zero token occurrences/pathnames. |
| FR-006 | Inventory manifest is set-equal to the forced-text content and NUL-safe tracked-path audits (both excluding the four fixed exclusion roots), proven by deterministic regeneration against the recorded hash. |
...
| C-003 | Terminology-extinction override: current-tree ADR/docs/archive/history bytes and filenames outside the four fixed exclusion roots are mutable retirement work; the two fixed exclusion kinds are Git object history outside `HEAD` and the immutable four-root historical-record set — `kitty-specs/`, `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`, `.kittify/missions/` (`DM-01M0NMS9WPH33EPFCJQRTQVNSA`, amended by `DM-01M0P6C8C7Q6SPBT412V39RPN0`). |
...
`ls-tree` uses `--full-tree`, so a subdirectory run can never report zero), both applying all four fixed
exclusion roots (`kitty-specs/`, `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`,
`.kittify/missions/`). Content succeeds only for raw `git grep` rc 1 plus empty stdout; pathname succeeds
only for raw `git ls-tree` rc 0 plus zero matching NUL records after the four-root drop; symlink targets
(`120000` entries) are read and audited; a normalised (NFKC, format/zero-width characters stripped)
content pass must also be zero. Git errors and return-code/output inconsistencies fail closed.
...
omitted exclusion root, or any other narrowed root blocks I6. Evidence records the resolved commit
...
- **SC-002**: regenerated inventory is set-equal to frozen content/path audits and matches the recorded
  hash; no unclassified or excluded rows outside the four fixed exclusion roots.
...
- Git history outside `HEAD` remains accessible; it and the four immutable historical-record roots
