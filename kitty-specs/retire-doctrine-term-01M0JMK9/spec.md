...
| FR-003 | Kind vocabulary preserved | As an operator, I want the ADR to state that `directive`, `tactic`, `styleguide`, `toolguide`, `paradigm`, `procedure`, agent profile, glossary pack, and mission step contract remain canonical kind terms (the full charter-activatable artifact-kind vocabulary), so that CLI cascade syntax and DRG edge vocabulary are undisturbed. | High | Open |
...
- **Charter kind**: The surviving artefact-kind vocabulary (`directive`, `tactic`, `styleguide`, `toolguide`, `paradigm`, `procedure`, agent profile, glossary pack, mission step contract — the full charter-activatable artifact-kind set); orthogonal to the three senses.
...
3. **The user-facing surface boundary is as enumerated in FR-006** (CLI surfaces, docs/glossary, prompts/skills/agent artifacts, charter file, generated output); a surface not enumerated is treated as out of scope until the inventory proves otherwise. This default yields to explicit in-scope statements elsewhere in this spec — C-004's per-surface verification assignment presupposes that `packs/`, `.kittify/charter/`, and root-level docs are in-scope surfaces even though FR-006's list does not spell them out.
