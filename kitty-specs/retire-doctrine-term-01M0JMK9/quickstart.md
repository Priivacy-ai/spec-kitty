...
2. complete current-tree scope outside the four fixed exclusion roots, including internal
...
7. M5 current-tree history rewrite outside the four fixed exclusion roots, archive/record-root immutability
   (no pre-existing slug/directory/file rename or edit under any of the four; referrers recited by
   `mission_id`/mid8 or token-free path), and the two fixed exclusion kinds (Git object history, the four
   fixed exclusion roots);
8. M6 compatibility/control/fixture/baseline removal and exact zero audits.

Any “user-visible only”, X1/X2/X3, immutable-current-tree-outside-the-four-fixed-roots, unsupported internal,
...
`contracts/inventory-schema.md` with `mode=inventory` (four fixed `:(exclude)<root>` pathspecs — one per
root: `kitty-specs/`, `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`, `.kittify/missions/`;
paths under any of the four dropped after the ls-tree rc check); do not use a shell pipeline. Preserve
argv, raw git return code, and stdout/stderr hashes. A content rc 1 with empty stdout is the expected
no-match result; rc >1 or inconsistent output is an audit error. Any pathname-command nonzero rc is an
audit error before the NUL stream may be classified.
...
byte-identical match to the SHA-256, row count, and per-kind/S/OC counts recorded in `inventory.md`
(content 48,245 / pathname 719 / total 48,964 with the four-root exclusion), whose per-root excluded
orientation counts must also be present. Reject X/exempt/ignored rows, missing pathnames, sampling, `-I`,
shell-expanded file lists, any exclusion other than the four fixed roots, or current-repo deferrals.
...
- M5: all remaining current-tree ADR/docs/archive/history/evidence prose, filenames, and referrers outside
  the four fixed exclusion roots; each of the four byte-identical for pre-existing paths; archive referrers
  recited by `mission_id`/mid8 or token-free path.
- M6: every alias/key/path/control/fixture/tombstone/baseline/allowlist removed; numeric-byte tests; exact
  zero over `HEAD` outside the four fixed exclusion roots.
...
then exits 0. Charter is included; the four fixed exclusion-root pathspecs/drops are the only permitted
boundary. Any hit, git failure, return-code/output inconsistency, audit error, unreadable input, an
omitted exclusion root, any other narrowed root, exception list, baseline, or external deferral blocks I6.
...
The two fixed exclusion kinds are Git object history outside `HEAD` (not scanned) and the four immutable
historical-record roots — `kitty-specs/`, `.kittify/migrations/mission-state/quarantine/`, `kitty-ops/`,
`.kittify/missions/` (`DM-01M0NMS9WPH33EPFCJQRTQVNSA`, amended by `DM-01M0P6C8C7Q6SPBT412V39RPN0`).
