...
## Work Package WP01: Script Foundation & Page 1 Layout (Priority: P1) 🎯 MVP
...
### Included Subtasks
- [ ] T001 Create `generate_pdf.py` with imports, brand color constants, and argparse CLI
...
### Risks & Mitigations
- Logo image may be too large for the layout → resize to a fixed width (e.g., 150px) in the script.
- reportlab not installed → clear ImportError message with `pip install reportlab` hint.
...
### Risks & Mitigations
- Content may not fit exactly 2 pages → adjust font sizes, spacing, or trim descriptions to fit.
- 2-column grid alignment may be tricky → use fixed-width Table columns with explicit colWidths.

---

## Dependency & Execution Summary

- **Sequence**: WP01 → WP02 (linear chain — WP02 builds on WP01's script)
- **Parallelization**: Not applicable — only 2 WPs, WP02 depends on WP01
