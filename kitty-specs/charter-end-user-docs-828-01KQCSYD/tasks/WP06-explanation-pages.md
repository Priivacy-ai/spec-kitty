...
4. **Proposal lifecycle** — The retrospective produces proposals (doctrine changes, directive additions/removals). Proposals are reviewed by the default dry-run form `agent retrospect synthesize --mission <mission>` then applied by `agent retrospect synthesize --mission <mission> --apply`.
