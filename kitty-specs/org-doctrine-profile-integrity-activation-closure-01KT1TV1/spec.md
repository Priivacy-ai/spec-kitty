...
| FR-032 | Mission-type augmentation SHALL be delivered by **expanding the org-pack DRG canonical kind universe (`_ORG_DRG_CANONICAL_KINDS`) to include mission types** (decision locked; the "separate path" alternative from R-010 is rejected). This is a binding change to the 8-kind universe: `_ORG_DRG_KIND_ALIASES` and the contract-test sweep that guards drift against `charter.activations._ALLOWED_KINDS` MUST be updated in lockstep, and mission types MUST NOT be silently dropped from augmentation coverage. | Must | Proposed |
...
| NFR-006 | Architectural boundaries SHALL remain intact and the mission SHALL not introduce new dead symbols. | Layer-rule suite passes. The dead-symbol suite is resolved for all in-scope symbols: the 2 charter sub-apps (FR-020), `CharterPackConfigError` (FR-035), and the 7 stale allowlist entries (FR-036). Removing the 7 stale + 4 OperationalContext entries is allowlist **shrinkage** (permitted, WARN-only under the Burn-down Policy). The 2 git/lanes offenders (`SparseCheckoutKind`, `LANE_AUTO_REBASE_FAILED`) are a documented pre-existing `main` regression tracked by [#1588](https://github.com/Priivacy-ai/spec-kitty/issues/1588) and are **out of scope**. Because the Burn-down Policy (charter §"Burn-down Policy" (a)) **fails CI on allowlist growth above baseline**, this mission MUST NOT add net-new allowlist entries: if the two external offenders must be tolerated to keep CI green, that requires an explicit `tests/architectural/_baselines.yaml` change with rationale + the #1588 reference (a deliberate baseline edit, not a silent allowlist addition), and net allowlist count must stay at or below baseline. | Proposed |
