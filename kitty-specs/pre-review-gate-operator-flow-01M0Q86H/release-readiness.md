...
|---|---|---|---|
| Policy, engine, aggregation, registry, public command | Integrated focused suite across the mission-owned review and CLI surfaces | 180 passed, 2 expected platform skips before the final adversarial remediation. The final count is refreshed after remediation. | Local platform is macOS; platform-specific skips are reported separately. |
...
| Ruff | `uv run ruff check` over all mission-owned production/test surfaces | Passed on the integrated branch before final remediation; rerun is required before PR. | `tasks_move_task.py` format-only drift was independently reproduced on the base; lint passed. |
| Strict typing | `uv run mypy --strict src/specify_cli/review/gate_budget.py src/specify_cli/review/pre_review_gate.py src/specify_cli/review/verdict_aggregation.py src/specify_cli/review/gate_registry.py` | Passed: no issues in 4 files before final remediation; rerun is required before PR. | CLI command module is outside this focused strict-mypy invocation. |
