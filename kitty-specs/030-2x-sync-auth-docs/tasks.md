...
**Tests**: Not applicable — documentation mission. Validation is via `docfx build` zero-warning check.
...
- `sync now` interface is inferred from `batch.py` — mark with a note that this is preliminary
...
### Risks & Mitigations
- `sync now` flags are inferred, not from actual implementation → note in doc that interface is provisional
...
- Credential file is created by `auth login`, not user-edited — document as read-only reference
...
### Risks & Mitigations
- Credential file format may change → document based on current `sync/auth.py` CredentialStore
...
### Risks & Mitigations
- Architecture may evolve — keep explanations at conceptual level, avoid implementation details
...
### Risks & Mitigations
- CLI output may change when commands are restored → use representative output from deleted source
...
### Risks & Mitigations
- `sync now` interface is inferred → mark examples as provisional, link to reference for current flags
...
### Risks & Mitigations
- Dashboard UI may change → keep descriptions at feature level, not pixel-level

---

## Work Package WP07: Server Connection Tutorial (Priority: P1) 🎯 MVP

**Goal**: Create `docs/tutorials/server-connection.md` — end-to-end guide from zero to first sync.
...
- Must work as a standalone guide — a new user should NOT need to read other docs first
...
### Risks & Mitigations
- Tutorial becomes outdated if CLI interface changes → keep steps high-level with links to reference for exact flags
...
- Explanations directory is new — create toc.yml from scratch using same format
...
### Risks & Mitigations
- DocFX may not be installed locally → document build command, flag as optional validation

---

## Dependency & Execution Summary

- **Phase 1 (parallel)**: WP01, WP02, WP03, WP06 — no dependencies, can all run simultaneously
...
**MVP Scope**: WP01 (reference) + WP04 (auth how-to) + WP07 (tutorial) — enables a developer to authenticate and start syncing with minimal documentation.
