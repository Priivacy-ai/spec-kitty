...
- **Scenario 4** → [`Priivacy-ai/spec-kitty#1141`](https://github.com/Priivacy-ai/spec-kitty/issues/1141). **Investigated 2026-05-19** within window via mission `investigate-canary-followups-1142-1141-01KS02TV`. Result: cheapest-first sweep H4→H3→H2→H1 — H4 (fixture state) RULED OUT (captured `for_review → in_review` row proves fixture reached `in_review`); H3 (sequencing race) RULED OUT (peek is a subprocess after move-task exit, SQLite WAL settled); H2 (canary drift) PARTIALLY RULED OUT (canary's `force=True + reason` contract matches current spec-kitty `tasks.py:1751-1759`; the captured failure is upstream of the force-check); **H1 LIKELY** (silent fan-out failure on backward emission — `fire_saas_fanout` at `status/adapters.py:121-126` swallows all handler exceptions, so the rollback row lands in `status.events.jsonl` locally but does not reach `OfflineQueue`). Recommendation **A** (open a 1-WP follow-up mission to instrument `fire_saas_fanout`, add a unit test for backward queue emission, and bisect from a trusted-runner workstation). Issue remains open pending the follow-up mission. Comment: https://github.com/Priivacy-ai/spec-kitty/issues/1141#issuecomment-4488224564. Outcome record: `kitty-specs/investigate-canary-followups-1142-1141-01KS02TV/research/outcome-1141.md`.
