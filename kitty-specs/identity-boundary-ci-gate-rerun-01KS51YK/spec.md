...
| FR-005  | Each repo's `README.md` gains a section with the exact heading `## Identity-Boundary CI Gate` explaining what the gate enforces, where the canary lives, how to bump the pinned e2e SHA when an intentional contract change ships, and the exact required-check name the admin must add. | accepted |
