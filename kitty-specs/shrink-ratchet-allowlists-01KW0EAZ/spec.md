...
mission removes the evidence-backed stale entries, fixes a dead-symbol-gate parser bug that masked some
of them, and corrects drift in the tracking issue. Sister issue #2048 (the `category_4` `9→8` reversal)
is handled separately in PR #2152 and is **out of scope** here.
...
3. **Given** the `_extract_all_literal` parser bug is fixed, **when** the dead-symbol gate runs, **then** it inspects `write_pipeline.py`'s public `__all__` symbols (previously invisible), and any newly-surfaced genuinely-dead symbol is resolved (wired, removed, or justifiably allowlisted) within this mission.
4. **Given** the dangling `legacy_contract_allowlist` entry is removed, **when** `tests/contract/test_example_round_trip.py` runs, **then** it passes with the entry gone.

### Edge cases
- Removing the 2 slice-F entries while the parser bug still blinds `write_pipeline.py` is safe (the entries are inert), but the fix (FR-006) makes that provable — order the work so the suite is green at each commit.
...
| FR-002 | Remove the 2 stale entries `specify_cli.cli.commands.charter.activate::charter_activate_app` and `…deactivate::charter_deactivate_app` from `category_b_grandfathered_legacy` (`test_no_dead_symbols.py`) — both symbols were deleted under FR-020 (absence asserted by `tests/specify_cli/test_charter_activate_cli.py`) — and set `_baselines.yaml` `category_b_grandfathered_legacy` to **284**. | Pending |
...
| FR-005 | Correct the drift in the tracking issue / docs: `legacy_contract_allowlist` lives in `tests/contract/` (not `tests/architectural/`); live counts are `category_7_grandfathered_orphans = 7` (issue said 6) and `category_b_grandfathered_legacy = 286` (issue said 271). Record corrections in the mission artifacts / a comment on #2049. | Pending |
| FR-006 | Fix the `_extract_all_literal` parser bug in `tests/architectural/test_no_dead_symbols.py` (a top-level non-`__all__` `AnnAssign` triggers an early `return frozenset()`, blinding the gate to every public symbol of `charter/synthesizer/write_pipeline.py`). After the fix, triage any newly-surfaced genuinely-dead symbol (wire it, remove it, or add a justified allowlist entry) so the gate stays green. | Pending |
...
3. The dead-symbol gate inspects `write_pipeline.py` public symbols after the parser fix; `pytest tests/architectural/ tests/contract/` is green.
4. `ruff check .` and `mypy` are clean on the diff.
5. The PR closes #2049 (and records the FR-005 corrections); ~8 stale allowlist entries retired across 4 categories.

## Key Entities

- **`_baselines.yaml`** — the ratchet ledger (four keys decremented + one sibling).
- **`test_no_dead_symbols.py`** — holds `category_a`/`category_b` frozensets and the buggy `_extract_all_literal`.
...
- Fixing the parser bug (FR-006) may surface previously-hidden `write_pipeline.py` symbols as dead; the audit suggests the masked symbols are reachable, but the implementer triages whatever the un-blinded gate reports.
