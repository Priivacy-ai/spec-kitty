...
| FR-010 | `scan_builtin_cross_grain_duplicates` is re-added to `__all__` in `src/charter/activation/action_grain.py`, justified by the new `src/` runtime caller from FR-009 (both land together). | Draft |
...
- **`scan_builtin_cross_grain_duplicates`** (`src/charter/activation/action_grain.py`) — the cross-grain integrity scan,
...
- **`MissionTypeProfileRepository`** (`src/charter/activation/mission_type_profile_repository.py`) — owner of the
