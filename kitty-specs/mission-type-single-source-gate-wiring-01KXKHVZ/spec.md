...
|----|-------------|---------------------|--------|
| NFR-001 | The canonical accessor introduces no import-time filesystem I/O into the **hot** charter modules. | Importing `charter.mission_type_profiles` or `charter.pack_context` performs zero `mission_types/` reads — their roster resolution happens only inside function bodies (verified by `test_charter_import_time_io.py` + code inspection). **Carve-out (C-012):** the two public value-constant rosters — `activations.ALLOWED_MISSION_TYPES` and `synthesizer.interview_mapping._MISSION_IDENTIFIER_ANSWERS` — are *derived at module scope* via the cached accessor (a single cached scan per process, bounded by NFR-002), because an unowned arch test consumes `ALLOWED_MISSION_TYPES` as a value and it must remain an importable frozenset. Their import triggers ≤1 cached read, which is acceptable. | Draft |
...
| C-009 | The two mission-root **bootstrap detectors** — `kernel/paths.py` `_looks_like_missions_root` and `specify_cli/runtime/home.py`'s near-duplicate — are excluded from single-sourcing: the kernel one cannot import doctrine (kernel⊄doctrine) and both are chicken-and-egg (they *locate* the root `MissionTypeRepository.default()` reads). They stay hardcoded; their logical duplication is tracked as a post-merge follow-up. `constants.py` `MISSION_TYPE_*` scalars (a partial competing SSOT, 3 of 4) are acknowledged, not folded. | Draft |
...
| C-012 | The public value-constant rosters `activations.ALLOWED_MISSION_TYPES` and `interview_mapping._MISSION_IDENTIFIER_ANSWERS` remain **module-level frozenset values** (derived from the accessor), NOT converted to functions — `ALLOWED_MISSION_TYPES` is imported as a value by the unowned arch test `test_activation_registry_schema.py` (`frozenset(ALLOWED_MISSION_TYPES)`), which stays green by construction. This is the NFR-001 carve-out (≤1 cached import-time scan). | Draft |
...
| SC-005 | Importing the hot charter modules (`mission_type_profiles`, `pack_context`) triggers zero `mission_types/` filesystem reads; the two value-constant rosters (C-012) trigger at most one cached read (no unbounded import-time I/O regression). |
...
- **`kernel/paths.py` + `specify_cli/runtime/home.py` bootstrap probes** — layer-forbidden
  (kernel→doctrine) and chicken-and-egg (they locate the missions root the repository would read); left
  as-is (C-009). Their logical duplication (a 5th type breaks both identically) is a tracked post-merge
  follow-up, not part of this bundle.
