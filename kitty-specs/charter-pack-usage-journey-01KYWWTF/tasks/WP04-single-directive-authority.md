...
`src/charter/activation/resolver.py` (`_resolve_directives_selection:233`, catalog-fallback `:258-260`;
`resolve_project_governance:289`, unfiltered catalog build `:316`; the operator strings **M1 owns** at
`:187`/`:250`; the wrapper `:57-139` — NOT the fix; consumer `:415`) and `src/charter/activation/pack_context.py`
