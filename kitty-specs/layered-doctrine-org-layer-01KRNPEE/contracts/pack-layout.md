...
17. The built-in pack (`packs/built-in/`) is a distinct case from org-pack fetch/assemble:
    its `pack-manifest.yaml` is generated by `spec-kitty doctrine regenerate-graph`
    (see ADR `2026-08-16-1-pack-metadata-manifest-unification`), not by `doctrine fetch`.
