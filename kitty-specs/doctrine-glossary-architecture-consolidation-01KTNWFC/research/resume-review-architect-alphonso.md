...
- **FR-008 charter `extends:` (WP08)** — **HOLDS.** Contract C1 already mandates resolution through `charter.activation.activation_engine` (plan/commit) + `charter.activation.cascade`, "no parallel resolver" — exactly the consolidation posture the placement/guard decisions reinforce. `src/charter/` exists. No amendment.
