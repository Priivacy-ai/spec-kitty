...
## Objective
Implement `org-charter.yaml` **`extends:`** — additive multi-org charter config with base-org precedence and cycle detection, resolved through the existing `charter.activation.activation_engine` (plan/commit) + `charter.activation.cascade`. **No parallel resolver** (FR-008, R-10, C-005). Non-destructive (C-004).
