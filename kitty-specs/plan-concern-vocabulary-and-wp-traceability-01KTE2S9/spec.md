...
| FR-006 | `WorkPackageEntry` in `src/specify_cli/core/wps_manifest.py` must gain an optional field `plan_concern_refs: list[str]` with a default empty list and a field validator that rejects any entry not matching the pattern `IC-\d{2}` (e.g. IC-01, IC-23). | Proposed |
| FR-007 | `generate_tasks_md_from_manifest()` in `wps_manifest.py` must render a "Plan concerns: IC-01, IC-03" line per WP when `plan_concern_refs` is non-empty. | Proposed |
...
| FR-009 | The `tasks-packages` prompt (`src/doctrine/missions/mission-steps/software-dev/tasks-packages/prompt.md`) must carry `plan_concern_refs` from `wps.yaml` into generated WP prompt file frontmatter. | Proposed |
...
| FR-013 | `finalize-tasks` must emit a non-fatal warning (not an error) for any WP missing both `plan_concern_refs` (empty or absent) and `cross_cutting: true`, to guide authors without blocking existing missions. | Proposed |
