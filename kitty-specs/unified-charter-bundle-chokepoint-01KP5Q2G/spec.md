...
- **The chokepoint is under-adopted.** `ensure_charter_bundle_fresh()` exists at `src/charter/activation/sync.py:50-90` and auto-refreshes the derived bundle when `charter.md` exists but the extracted YAMLs are missing or stale. Only two internal loaders call it: `load_governance_config()` at `src/charter/activation/sync.py:204` and `load_directives_config()` at `src/charter/activation/sync.py:244`. The primary consumer of charter state — `build_charter_context()` at `src/charter/activation/context.py:406-661` — bypasses the chokepoint. Dashboard charter path resolution at `src/specify_cli/dashboard/charter_path.py:8-17` performs an existence check only, not a freshness guarantee.
...
- **Not extending v1.0.0 manifest scope to `references.yaml` or `context-state.json`.** `references.yaml` is produced by `src/charter/activation/compiler.py:169-196` (a separate pipeline invoked by `spec-kitty charter generate`). `context-state.json` is runtime state written lazily by `src/charter/activation/context.py:385-398`. Both have different invariants than the `sync()`-produced files. Broadening manifest scope to cover them requires its own design tranche and a new manifest schema version.
...
- **Multiple worktrees reading concurrently.** The chokepoint's sync step must be safe under concurrent invocation; `src/charter/activation/sync.py` already uses `atomic_write` from `kernel.atomic`, and this mission reuses that primitive for any additional writes.
- **Project with no `charter.md` at all.** Chokepoint returns `None` (per current `ensure_charter_bundle_fresh()` contract at `src/charter/activation/sync.py:54`); readers receive an explicit "missing charter" signal and degrade accordingly. This behavior is preserved unchanged. The migration's JSON report sets `charter_present: false` and `applied: false`.
...
| --- | --- | --- |
| FR-001 | The codebase must declare a **bundle manifest contract** at `src/charter/bundle.py` that enumerates, in typed Pydantic form, every file path that constitutes the v1.0.0 unified bundle (the files `src/charter/activation/sync.py :: sync()` materializes), whether each is tracked or derived, and which source file each derived file is derived from. The manifest is the single source of truth consumed by the chokepoint, the migration, and the bundle contract tests. | Draft |
...
- **Unified bundle (v1.0.0)** — the set of files `src/charter/activation/sync.py :: sync()` materializes: `governance.yaml`, `directives.yaml`, `metadata.yaml` (plus the tracked source `charter.md`). Declared by `CharterBundleManifest` v1.0.0; does NOT include `references.yaml` (compiler pipeline) or `context-state.json` (runtime state).
...
- **Chokepoint** — `ensure_charter_bundle_fresh()` at `src/charter/activation/sync.py:50-90`. After Phase 2 it is the sole entry path for every reader of the three v1.0.0 manifest derivatives. It resolves to the canonical root, checks completeness and staleness, triggers `sync()` if needed, and returns a `SyncResult | None`.
...
- Every reader site in FR-004 is updated to call `ensure_charter_bundle_fresh()` before reading any v1.0.0 manifest derivative. In particular: `build_charter_context()` (bypass at `src/charter/activation/context.py:406-661`), `resolve_project_charter_path()` at `src/specify_cli/dashboard/charter_path.py:8-17`, dashboard scanner + server charter-presence paths, charter CLI handlers, next and workflow prompt builders.
...
- `src/charter/activation/sync.py` — the chokepoint itself performs the authoritative read.
...
- `src/charter/activation/compiler.py:169-196` — C-012 out-of-v1.0.0-scope (compiler pipeline).
- `src/charter/activation/context.py:385-398` — C-012 out-of-v1.0.0-scope (runtime state).
...
- `src/charter/activation/sync.py` (modify — `ensure_charter_bundle_fresh()` + `SyncResult.canonical_root`)
...
- `src/charter/activation/context.py` (modify — `build_charter_context()` routes through chokepoint)
...
- `src/charter/activation/compiler.py` — left unchanged (compiler pipeline owns references.yaml)
- `src/charter/activation/context.py:385-398` — `context-state.json` write path left unchanged (runtime-state concern)
