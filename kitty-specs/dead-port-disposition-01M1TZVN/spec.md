...
1. This mission runs **AFTER Mission A** `fsm-write-path-integrity-01M1TZV6` — the four shared runtime files (`runtime_bridge_io.py`, `_internal_runtime/engine.py`, `decision.py`, `runtime_bridge_engine.py`) are NOT parallel-safe; no edits to them until Mission A's WPs on them are merged. Still open and unaffected by this landing pass.
...
**Independent Test**: The ADR landed (PR #3898 merged, Accepted 2026-09-06): its decision is executed with the parity oracle updated in the same PR and no name collision remains.

**Acceptance Scenarios**:

1. **Given** the ADR (PR #3898) is merged AND Accepted (satisfied 2026-09-06), **When** its decision is executed, **Then** the name collision is resolved per the ADR, the `sync_emitter` vocabulary/signature pass happens as ONE pass over the 29 sites (not two), the flush-target defect is adjudicated per the ADR (never silently frozen or silently fixed), and `test_side_effect_sinks_are_actually_reached` is updated in the same PR if the capture is removed.
2. **Given** any disposition, **When** work touches `runtime_bridge_retrospective.py`, **Then** `_BufferingRuntimeEmitter` (`:69`) is untouched — it is rollback machinery, the only thing preventing an unretractable `MissionRunCompleted` on a rolled-back terminal advance, and is not deletable under any disposition.
3. **Given** a removal that erases a subpackage's last live `specify_cli` edge, **When** the PR is assembled, **Then** the same PR carries the `test_runtime_ledger_has_no_stale_entries` ledger edit (and respects the widened doctrine-scan baseline pinning `runtime_bridge_io.py`/`runtime_bridge_composition.py` lazy reaches).

---

### Edge Cases

- **ADR outcome is "explicit deferral"**: did not occur — the ADR (PR #3898) was Accepted 2026-09-06; this edge case is retained for historical completeness only (see Ground-Truth Errata; Open Decision 7 resolved).
...
| FR-012 | Execute the landed emitter ADR | As a maintainer, I want the ADR's disposition executed exactly (collision resolution, `sync_emitter` vocabulary as one pass, flush-target adjudication, parity-oracle update in the same PR) — the gate is satisfied: PR #3898 merged 2026-09-06 and the ADR (`docs/adr/3.x/2026-09-06-2-runtime-event-emitter-disposition.md`) carries `status: Accepted`, so execution proceeds per the ADR's chosen option (rewire-ready consolidation) rather than the shrunk residue-only variant. | Medium | Open |
...
| C-003 | Emitter ADR gate (satisfied) | The RuntimeEventEmitter disposition ADR (PR #3898, `docs/adr/3.x/2026-09-06-2-runtime-event-emitter-disposition.md`) merged 2026-09-06 and carries `status: Accepted` — the gate is OPEN. Emitter rewiring, deletion, renaming, and `sync_emitter` signature changes execute per the ADR's chosen option (rewire-ready consolidation, which deletes `event_emitter.py`); the flush-target defect is adjudicated per the ADR rather than left record-don't-touch. | Technical | High | Open |
| C-004 | Dependency-removal mechanics | the `transitions>=0.9.2` entry in `[project].dependencies` (`pyproject.toml`) deleted; `uv.lock` regenerated with `uv lock`, never hand-edited; version bump in `pyproject.toml` + `CHANGELOG.md` entry (house rule); one dependency change per PR — `truststore` removal already landed via the quick-wins PR #3899 (merged 2026-09-06) and must not be re-bundled here. | Technical | High | Open |
...
7. [RESOLVED — **ADR timing.** The WP03 gate check ("ADR merged AND Accepted") is satisfied: PR #3898 merged 2026-09-06 and the ADR (`docs/adr/3.x/2026-09-06-2-runtime-event-emitter-disposition.md`) carries `status: Accepted`. The P4 slice executes the ADR's chosen option (rewire-ready consolidation) rather than shipping the shrunk residue-only variant; no follow-up mission/WP is needed for the emitter execution itself.]
...
6. **No `truststore` removal here** — done via quick-wins PR #3899 (merged 2026-09-06); this mission must not re-touch it.
...
- **SC-007**: The emitter slice executes the ADR's disposition — PR #3898 merged AND the ADR Accepted (2026-09-06): decision executed, collision gone, parity oracle updated in the same PR.
