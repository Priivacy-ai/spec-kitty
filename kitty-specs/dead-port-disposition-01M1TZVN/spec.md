...
As a maintainer, the small confirmed-dead residue that couples to this mission's gate files gets cleaned here (once its external gates open), and residue with zero coupling goes to the quick-wins PR — one owner per gate file, no two PRs racing on `test_no_dead_symbols.py` hash pins. The quick-wins PR is **PR #3899** (draft): it covers the scripts/dep/symlink/prose set (CODEOWNERS, CONTRIBUTING symlink, the orphan scripts + their test, `truststore` in pyproject+uv.lock, Makefile/pytest.ini/testing-parallel prose) — the `src/mission_runtime` residue is owned HERE. In scope here: the stale `constitution` exclusion at `test_layer_rules.py:65-73` (a package that no longer exists — mission 063 done), sequenced strictly after PR #3888; the 13 un-demoted test-only `__all__` exports (demotion from package `__all__`, never deletion — `content_present_at_primary_tip` and `get_packs_root_default` have in-package src callers); the `team_projection/` tombstone; and the `ActionContext` alias (two files: `mission_runtime/context.py:~338` AND the `__getattr__` block in `mission_runtime/__init__.py:127,139-142`) — all re-routed into this mission's residue slice, since #3899's verified file list does not carry them. Routed to #3899 (not here): `truststore` and the rest of its scripts/dep/symlink/prose scope.
...
1. **Given** PR #3888 has merged, **When** the `constitution` exclusion at `test_layer_rules.py:65-73` is deleted, **Then** the layer-rules gate stays green and the rebase over #3888's `_RUNTIME_ALLOWED_SPECIFY_CLI` ledger changes is trivial.
...
| FR-005 | Drop the `transitions` dependency | As a maintainer, I want `pyproject.toml:82` removed and `uv.lock` regenerated via `uv lock` so the dependency leaves the product (`six` stays via `python-dateutil`). | High | Open |
...
| FR-013 | Delete the stale `constitution` exclusion | As a maintainer, I want `test_layer_rules.py:65-73` (excluding a package that no longer exists) removed — strictly after PR #3888 lands. | Low | Open |
...
| C-004 | Dependency-removal mechanics | `pyproject.toml:82` deleted; `uv.lock` regenerated with `uv lock`, never hand-edited; version bump in `pyproject.toml` + `CHANGELOG.md` entry (house rule); one dependency change per PR — `truststore` removal belongs to the quick-wins PR #3899 and must not be bundled. | Technical | High | Open |
| C-005 | Same-PR gate edits | The DSL-retirement PR itself removes the three dead-symbol pins at `test_no_dead_symbols.py:718-727`, updates the `_gate_coverage.py:1456` shard row if `tests/specify_cli/mission_v1/` empties, and (Option FULL) the fold-gate entry at `test_wp_frontmatter_fold.py:60`. | Technical | High | Open |
...
- **`transitions` dependency**: `pyproject.toml:82` (`transitions>=0.9.2`), lock `0.9.3` with a `six` edge; loaded on every `spec-kitty next` via the eager chain.
