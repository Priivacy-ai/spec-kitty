...
| FR-005 | `agent/tasks.py` MUST be reduced toward a thin command-registration shim, delegating substantive logic to the extracted seams. | Approved |
...
| FR-007 | The bespoke `is_protected` pre-checks now superseded by the router — `_skip_target_branch_commit()` (`tasks.py:954–971`), `_protected_branch_status_commit_error()` (`tasks.py:921–932`), their guard conditionals (`move_task:2448–2470`, `mark_status:3021–3029`), and the `_planning_commit_worktree` import (`tasks.py:3928`) — MUST be deleted once the tails route through `commit_for_mission`. | Approved |
...
| SC-002 | `agent/tasks.py` maxCC drops from ~178 to ≤ 15 (every function), the file shrinks to ≤ ~1200 LOC, and substantive logic lives in the 5 seams. |
| SC-003 | All three planning-commit tails reach git only via `commit_for_mission`; no bespoke `is_protected` pre-checks remain in `tasks.py`; and the protected-primary message text + exit codes are proven byte-identical to current behavior (FR-008). |
