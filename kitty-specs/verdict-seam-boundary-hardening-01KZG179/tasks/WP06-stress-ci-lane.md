...
In `tests/status/test_emit_durability.py`, stop the `@stress` durability test riding the fast pool. Preferred: drop the module-level `fast` sweep for that specific test (or split it into its own module / re-home under `tests/stress/`) so the `fast-tests-status` selector (`-m "fast and not windows_ci and not (git_repo or integration)"`) no longer collects it, while `-m stress` does. Confirm with `pytest --collect-only -m stress -q` (collects it) and `pytest --collect-only -m "fast and not ..." tests/status/ -q` (does not).
