...
base_commit: 6905aae94edbff895f05d27ebb6e7302caa80a6c
created_at: '2026-08-08T11:02:59.533447+00:00'
subtasks:
- T011
- T012
- T013
...
Extend the classifier so a reader constructed via a helper (not a direct `read_text`+parse in the function body) is recognized — specifically `migration/backfill_runtime_state.py::_review_from_frontmatter` (confirmed at ~L520) must surface as a reader row. Keep the predicate tight enough not to over-match unrelated helpers.
- **Dedicated two-way non-vacuity teeth (NFR-004 — this is a NEW branch, needs its own test):** T014's teeth exercise the *direct*-reader predicate; add teeth that specifically target the **helper-construction** predicate: (a) a synthetic helper-constructed reader MUST be flagged (fails if the new branch is removed/neutered), and (b) a synthetic non-reader helper-constructed record MUST NOT be flagged (guards the over-match risk). A fixture-presence assert alone is insufficient — the teeth must fail when the predicate is deleted.
