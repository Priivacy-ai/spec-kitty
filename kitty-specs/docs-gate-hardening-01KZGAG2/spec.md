...
| NFR-004 | In-process gate, no subprocess/network | The slash-command gate is a pure in-process set-diff — it imports `CONSUMER_SKILLS` and parses one Markdown file, with **no subprocess and no network**, so its CI cost is inspection-verifiable rather than a timing claim. (Importing the registry transitively initializes the `specify_cli` package — a fast, dependency-light import, ~140ms — which is acceptable; the guarantee is "no subprocess/network", not "no transitive package init".) | Performance | Medium | Open |
