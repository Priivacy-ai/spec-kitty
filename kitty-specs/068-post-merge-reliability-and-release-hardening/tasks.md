...
| T026 | Author test suite at `tests/lanes/test_recovery_post_merge.py` and `tests/cli/commands/test_implement_base_flag.py` covering Scenario 7 end-to-end (WP01–WP05 done-and-deleted, WP06 starts via `--base main`), `scan_recovery_state` finds merged-deleted deps, `--base bogus-ref` fails with clear error, post-merge unblocking integration | WP05 | |
