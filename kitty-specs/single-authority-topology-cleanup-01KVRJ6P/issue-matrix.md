...
|-------|-------|---------|--------------|
| #2070 | Mission B — CommitTargetKind type eradication + richer-API adoption (the driver) | in-mission | FR-001..FR-007 (type eradication + FLATTENED delete + ensure_topology + topology=None absorption + C1/C2/C6 dedup) |
...
| #2062 | Un-backfilled flattened mission leaks the stale coord husk on the read path | in-mission | FR-004 (read-path `topology is None` absorption extends the #2062 fix to un-backfilled flattened missions); WP01 lands the live RED repro (T002, drains in WP06) |
...
| #2084 | `accept` blocks on spec-kitty's own bookkeeping files (topology-blind dirty gate) | in-mission | FR-008 (topology-aware residue allowance via `is_coordination_artifact_residue_path`, gated on actual coord topology) |
| #2085 | An orchestrated mission can't pass `spec-kitty accept` | in-mission | FR-009 (unchecked-tasks gate derives from WP terminal status); acceptance-matrix half explicitly OUT of scope (C-010) |
| #1887 | Squash-merges leak coord-worktree paths into the target branch index | in-mission | FR-012 (merge `advance_branch_ref` callers + post-merge invariant consult the single residue authority; same #2084 topology-blind shape — squad-converged fold) |
| #1891 | `agent … --json` broken: `CommitResult is not JSON serializable` | in-mission | FR-013 (CONDITIONAL — rides FR-001's `CommitResult`/`CommitTarget` cleanup; `map-requirements --json` half only; gated on plan-time feasibility, carves to standalone if `CommitResult` is disjoint from `.kind` removal) |
