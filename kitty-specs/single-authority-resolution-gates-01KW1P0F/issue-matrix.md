...
|-------|-------|---------|--------------|
| #2154 | mark_status write-leg lands on coord; move_task validates primary (WP blocked) | in-mission | WP02 (FR-001 — routes :1807 write through resolve_planning_read_dir kind=TASKS_INDEX) |
| #2155 | mixed-bundle auto-commit silently drops coord-owned status write | in-mission | WP02 (FR-002 — routes implement.py:1311 [+ re-verify tasks.py:1555] via BookkeepingTransaction; guard untouched) |
| #2164 | Un-canonicalized handle reaches the topology-blind primitive (no regression guard) | in-mission | WP01 gate + WP03/04/05 sweep (FR-004/005 — closed by construction) |
| #2160 | Coord-authority class (write leg bypasses the kind-aware authority) | in-mission | WP02 closes the #2154+#2155 instances |
| #1842 | Literal `/tmp/` paths in test files | in-mission | WP07 (FR-007 — frozen-baseline ratchet; full litter sweep OUT of scope) |
| #2034 | Mission-owned contract tests excluded from CI shards | in-mission | WP07 (FR-008 — conditional/empirical collect-only verify; named files already run → likely verify-only) |
