...
base_branch: kitty/mission-org-charter-activations-runtime-wiring-01KWPS9E
base_commit: 1a7b9d249687ecdd7a8822938c982623cdede869
created_at: '2026-07-04T15:18:16.980790+00:00'
subtasks:
- T001
- T002
- T003
...
- **Precedent to mirror** (union/order ONLY): `_read_org_required_selections` (`src/charter/activation/context.py:732`) + `_load_doctrine_selection` (`:768`). Enumerator: `_enumerate_org_pack_paths` (`:682`).
...
- **Runtime import (verified gap)**: `context.py:15` imports `ActivationEntry` under `TYPE_CHECKING` only. T006/T007 must add a **runtime** import of `ActivationEntry` (+ `_activation_identity_key` from `charter.activation.activations`) in `context.py`.
...
- **T004** [FR-003] Relocate `_activation_identity_key` from `org_charter.py:286` → `src/charter/activation/activations.py` (needs only a `json` import there; body uses `json.dumps` + entry fields, no `org_charter`-local dependency — verified); re-import at `org_charter.py:44`/`:450`. Behavior-preserving; `test_org_charter_union.py` `_fold_policies` tests gate the import integrity.
...
- 2026-07-04T16:01:10Z – claude – shell_pid=363471 – Moved to for_review
- 2026-07-04T16:02:59Z – user – shell_pid=363471 – Adversarial review APPROVE (opus): red-first proven bidirectionally; FR-004 raise live-verified; layer boundary clean; extraction behavior-preserving; suppression tightened.
