...
| FR-003 | Single shared identity-key implementation | As a maintainer, I want the 4-tuple identity key relocated from `org_charter.py` **down into `charter.activation.activations`** (single caller at `org_charter.py:450`), shared by both the `org_charter` fold and the `charter`-layer resolve union, so the two paths cannot drift. | Medium | Open |
...
- **`ActivationEntry`** (`charter.activation.activations`): action-scoped record — `activation_context {mission_type, action}`, `doctrine_pack_id`, `artifact_id`, `artifact_kind`. Charter-layer-owned; `model_validate`-able.
...
- **Identity key** `(activation_context, doctrine_pack_id, artifact_id, artifact_kind)`: dedup identity, to be unified into one shared function in `charter.activation.activations` (FR-003).
