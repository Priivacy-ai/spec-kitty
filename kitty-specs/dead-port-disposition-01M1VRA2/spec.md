...
| NFR-003 | Net surface reduction | The concrete seam module (88 lines) is deleted and the net line count under the runtime `next` tree decreases. | Maintainability | Medium | Not met — deviation recorded 2026-09-06: the module is deleted (−88) but the factory, registry, `for_mission`, and seam docstring the ADR requires add +120 to `_internal_runtime/events.py`; mission net under `src/runtime/next/` is +161/−105 = +56. The ADR's "net LOC still drops" estimate was wrong; the intent (one seam class, no collision, no dead duplicate) is met. Confirmed by mission review (DRIFT-1); ADR consequence text amended in-PR (#3921); E3 follow-ups tracked in #3929. |
