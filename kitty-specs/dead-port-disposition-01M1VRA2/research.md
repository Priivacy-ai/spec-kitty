...
- **Alternatives considered**: flush into the plain seam *and* replay decision events into the wrap (rejected: double bookkeeping); make `DecisionGitLog` the engine emitter even under strict retrospective policy and drop the buffer (rejected: re-opens the unretractable `MissionRunCompleted` problem the buffer exists to solve).
