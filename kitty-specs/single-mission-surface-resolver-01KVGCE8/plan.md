...
- **Purpose**: Remove the hidden divergence so the equivalence test asserts over a clean surface. Unify the two `primary_feature_dir_for_mission` (T1/FR-009: raw-slug vs mid8-composed) to ONE definition over a single composition grammar (T5, `_compose_mission_dir` as sole composer); extract ONE shared `resolve-dir-or-typed-error` delegator (T4) from the duplicated wrappers in `aggregate._resolve_read_dir` and `mission_runtime/resolution.py`.
...
- **Risks**: the canonical `primary_feature_dir` form is a **behavior decision** (mid8-composing form locates backfilled/`<slug>-<mid8>` dirs) — pin it with a per-caller-class regression test, NOT a blind merge. The T4 delegator's fallback-target + exception-set difference must be reconciled (a recorded decision).
...
- **Purpose**: Preserve `STATUS_READ_PATH_NOT_FOUND` / `MISSION_AMBIGUOUS_SELECTOR` end-to-end through `next` / `mission_runtime` instead of flattening to `MISSION_NOT_FOUND` (#2010 bug #15). No resolver change.
- **Relevant requirements**: FR-005
- **Affected surfaces**: `mission_runtime/resolution.py`, the `next`/runtime_bridge caller boundary; the error types already exist
- **Sequencing/depends-on**: independent — can land early (no resolver change, highest blast-radius symptom).
- **Risks**: caller adoption breadth; reproduce bug #15 (SC-003) before fixing.
