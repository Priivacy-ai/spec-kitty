...
- **Two distinct models — not a collision.** `MissionType` (the artefact model, `src/doctrine/missions/models.py`) is edited by IC-01; `MissionTypeProfile` (the governance-profile model, `src/charter/activation/mission_type_profiles.py`) is edited by IC-05. Different files, different layers → **no `models.py` merge collision** (an earlier note calling this a collision was wrong). The existing `doctrine/missions/mission_type_repository.py::MissionTypeRepository` loads the *artefact* model and must not be confused with the new `MissionTypeProfile` overlay repo (IC-05).
