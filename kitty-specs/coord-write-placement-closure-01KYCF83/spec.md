...
|----|-------|-------------|----------|----------|--------|
| NFR-001 | Whole-tree coverage | The write-placement enforcement scans 100% of `src/` Python modules with no module-level allowlist, except two retained sanctioned SUBTREE carve-outs (`src/mission_runtime/` — the placement port cannot be scanned by its own gate; `src/specify_cli/upgrade/migrations/` — one-time migration code); a synthetic bypass added to any non-carve-out module reds the gate. Per-primitive exclusions are individually justified inline. **Closed for the `migration/` subtree** (placement-port-residuals-closure-01KYDEF0 WP03, 2026-07-26): the `src/specify_cli/migration/` carve-out was DROPPED — empirically it sanctioned nothing real (zero `CommitTarget`/`safe_commit` construction in the subtree) — restoring true "any module" scan precision for that scope; the `upgrade/migrations/` prefix is retained (still an active one-time-migration carve-out, per WP03's C-002). The SEPARATE, intentional `migration/` blanket in `test_mission_resolver_walker_gate.py::_MIGRATION_WALKER_DIR_PREFIXES` is a different, permanent resolver-walker carve-out (C-004), unaffected by this closure. | Coverage | High | Closed |
...
- **SC-002**: A synthetic non-seam-derived mission-artifact write added to any `src/` module **outside the two sanctioned subtree carve-outs** (see NFR-001) reds the whole-tree enforcement gate (previously invisible outside the 17-module allowlist). *(FR-001, and FR-002/FR-003's now-routed writers fall under the same whole-tree gate.)* **Closed for the `migration/` subtree** (placement-port-residuals-closure-01KYDEF0 WP03, 2026-07-26): dropping the `src/specify_cli/migration/` carve-out was empirically verified to red a planted synthetic bypass there (red-first proof, T014), so "any module" now holds precisely for that scope too; `upgrade/migrations/` remains the one retained one-time-migration carve-out (C-002), and the separate, permanent `_MIGRATION_WALKER_DIR_PREFIXES` resolver-walker blanket (C-004) is untouched.
