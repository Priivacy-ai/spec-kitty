...
| FR-005 | Add a small **pointer-only** `README.md` to each doctrine source module (and domain-relevant modules) linking the doctrine-kinds entry + the schema diagram (**in-mission targets that always resolve**) and, opportunistically, the owning domain plan (doctrine-charter is already on `main`; packs-extraction/api-dashboard arrive with Scope A). **Independently landable** — the link-resolution check never depends on an external merge. Extend existing READMEs (N≈17 already exist), don't clobber; a machine lint enforces pointer-only | US3 | Medium | Open |
