...
asset?" The failure today: `charter.activation.resolver.DoctrineService` (`src/charter/activation/resolver.py:57-139`) is a
...
*Second revision, post-plan squad*: a 4-lens adversarial review (architect-alphonso, reviewer-renata,
debugger-debbie, planner-priti) independently verified the Phase 0 research and found 3 more construction
sites (one with a fail-open bug, folded into FR-002/FR-008), a live `._inner` reach-around that would have
defeated every gate this mission ships (closed as new FR-010), an IC-sequencing bug in the plan, and several
fakeable-assertion gaps in the NFRs — all folded into the FR/NFR text below rather than left for an
implementer to discover mid-WP. See `research.md`'s "Post-Plan Squad Findings" section for the full record.
...
`doctrine.service.DoctrineService(...)` construction sites, bypass `charter.activation.resolver.DoctrineService`
...
doctrine-asset access path. Both sites already call `charter.activation.resolver.DoctrineService` separately, correctly,
...
build_activation_aware_doctrine_service` and `charter.activation.doctrine_service_builder.
...
`src/charter/activation/resolver.py`, its two (now-unified) builder call sites, and the two explicitly-excluded
...
1. **Given** the dashboard status-icon renderer (`tasks_status_cmd.py:712,823`), **When** it needs an agent
   profile, **Then** it resolves through `charter.activation.resolver.DoctrineService`, and the concern the removed
...
2. **Given** `src/charter/activation/compiler.py:802` and the four `_doctrine_collect.py` sites, **When** they read
   `.agent_profiles` / `.glossary_packs` from a raw `DoctrineService`, **Then** they read the same data
   through `charter.activation.resolver.DoctrineService`; the four `_doctrine_collect.py` diagnostic sites use the
...
mission-type resolution. Neither routes through the charter factory; `charter.activation.template_resolver.
...
`charter.activation.resolver.DoctrineService` itself (mirroring `CharterTemplateResolver`'s current public surface) that
...
`src/charter/activation/context_renderers/template_include.py` (a lazy `ResolutionTier` import). Both fold into FR-003's
...
`doctrine/resolver.py`'s tier functions only via `charter.activation.resolver.DoctrineService`; a direct import of
...
`src/charter/activation/mission_type_profile_repository.py:66::builtin_missions_root()`
...
`charter.activation.resolver.DoctrineService` filters exactly 3 properties (`paradigms`, `procedures`,
...
`charter.activation.mission_type_profile_repository.MissionTypeProfileRepository` /
`charter.activation.mission_type_profiles.resolve_mission_type_context()`. Per operator decision (real 10/10 coverage,
not a 9/10 carve-out), this mission adds equivalent activation filtering **to that repository**, not to
`charter.activation.resolver.DoctrineService` — a genuinely different, non-mechanical change from the other 9 kinds.
...
|----|-------------|--------|
| FR-001 | Eliminate 4 of the 7 originally-flagged direct `AgentProfileRepository(...)` construction sites outside `charter.activation.resolver.DoctrineService`: `src/runtime/next/runtime_bridge_io.py:576`, `src/specify_cli/tool_surface/profiles/projection.py:84`, `src/specify_cli/cli/commands/agent/tasks_status_cmd.py:712` and `:823`. **Excluded (planning-phase reclassification):** `src/specify_cli/invocation/registry.py:48` and `src/specify_cli/cli/commands/profiles_cmd.py:83` construct against `.kittify/profiles` (a local-override directory outside the doctrine activation model, slated for separate future rework) — leave untouched. **Confirmed bootstrap carve-out, not migrated** (post-tasks squad correction): `src/charter/activation/profile_resolution.py:81`'s `_default_agent_profile_repository()` is a zero-argument, module-level cached function with no `repo_root` to build a factory from — document it as a genuine C-002 bootstrap case, do not attempt to route it through the factory. `projection.py:84` needs a new public accessor on the factory (`agent_profile_repository`) for mutation-capable repository access, since the filtered property alone cannot support `register_overlay()` (the only method this site needs — `get_ancestors()` is unused here). For the two `tasks_status_cmd.py` sites whose comments cite avoiding a "runtime→charter→doctrine boundary ratchet": the concern is confirmed a red herring against the existing gate (`tests/architectural/test_runtime_charter_doctrine_boundary.py` only scans module-level imports; both the old and new construction are function-local) — the real, unmeasured risk is construction cost, covered by NFR-005. | Draft |
| FR-002 | Eliminate the unwrapped raw `doctrine.service.DoctrineService(...)` construction sites — the original 6 (`src/charter/activation/compiler.py:802`, `src/specify_cli/cli/commands/_doctrine_asset.py:75`, `src/specify_cli/cli/commands/_doctrine_collect.py:193`, `:283`, `:420`, `:828`) plus **3 more found by the post-plan squad**: `src/specify_cli/charter_runtime/lint/checks/org_layer.py:244` and `:275`, and `src/specify_cli/cli/commands/charter/generate.py:56` — routing each through `charter.activation.resolver.DoctrineService` instead of the raw inner service. `org_layer.py:252-253`'s `except ImportError: pass` currently **fails open**, silently returning the bare unfiltered inner service when the wrap import fails — this must become fail-closed (raise or fail the operation), never a silent unfiltered fallback. The four `_doctrine_collect.py` diagnostic sites (profile/glossary-pack health, cross-layer collision detection, provenance lookup) need the **unfiltered, all-layer** view to avoid silently narrowing doctor/health output for deactivated packs; wrap them via the factory's explicit unfiltered mode (`charter.activation.resolver.DoctrineService(inner, pack_context=None)`) rather than the activation-filtered path — same class, same single canonical authority (C-001), documented rationale at each site. | Draft |
| FR-003 | Retarget the 5-tier template/command resolver axis (`src/doctrine/resolver.py::_resolve_asset`, tiers OVERRIDE/LEGACY/GLOBAL_MISSION/GLOBAL/PACKAGE_DEFAULT, ~lines 164-213) and its duplicated tier logic in `resolve_mission` (~lines 284-342) by adding resolution methods to `charter.activation.resolver.DoctrineService` (mirroring `CharterTemplateResolver`'s current public surface) that call into `doctrine/resolver.py` internally — the tier functions stay in `doctrine/resolver.py` (the sanctioned `charter → doctrine` import direction), only the entry point moves. Retarget `charter.activation.template_resolver.CharterTemplateResolver`'s one real caller (`specify_cli/runtime/resolver.py`'s tier-5 routing) onto the new factory methods, pinning the construction contract (the caller has a `missions_root`/cached `from_missions_root` shape today; the factory is built from `repo_root` — this WP resolves that mismatch, not tasks-time improvisation). **Corrected scope** (post-plan squad): `src/charter/resolution.py` and `src/charter/activation/context_renderers/template_include.py` are NOT bypasses — both import only the `ResolutionResult`/`ResolutionTier` *types* (not resolution calls), sanctioned by an existing facade contract; retargeting a type import onto a factory method is a category error and is dropped from this FR. A real, previously-unenumerated third tier surface was found instead: `doctrine.template_catalog.resolve_template_by_id`, with 5 importers (`charter/template_catalog.py`, `specify_cli/runtime/resolver.py`, `charter/context.py`, `cli/commands/charter/list_cmd.py`, `charter/context_renderers/template_include.py`) — named as explicit deferred debt alongside `runtime/resolver.py`'s tier 1-4 reimplementation (R6), not folded into this FR's blast radius. | Draft |
| FR-004 | Promote `src/doctrine/missions/repository.py:98::MissionTemplateRepository.default_missions_root()` (already correctly `importlib.resources`-based) as the single shared missions-root authority, and retarget the other two hardcodes onto it: `src/charter/activation/mission_type_profile_repository.py:66::builtin_missions_root()` (currently `Path(__file__).resolve().parents[1]/"doctrine"/"missions"`) and `src/specify_cli/runtime/home.py:107-108`'s hardcoded `dev_roots` fallback tuple. `builtin_missions_root()` was itself previously promoted as a shared authority (WP06/#2668, referenced at `charter/action_grain.py:203-204` and `charter/mission_type_profiles.py:646-647`) — this FR makes it a **thin delegate** to `default_missions_root()`, not a second co-equal authority; existing callers of `builtin_missions_root()` are unaffected. `home.py`'s import of the doctrine-layer `MissionTemplateRepository` is function-local, matching the file's own existing pattern at `home.py:49` (also function-local) — the mission does not introduce a new module-level `runtime → doctrine` edge; note this explicitly in the PR description as a residual risk that shares the exact blind spot `#2986` (deferred, C-003) would close, not a silently different shape. `home.py`'s `importlib.resources` lookup step (lines 97-104, preceding the `dev_roots` fallback) is a fourth resolution path for the same root and is explicitly OUT of this FR's scope — only the `dev_roots` tuple is retargeted. Ship an equality regression test proving both retargeted sites now resolve identically to `default_missions_root()`'s output. **Revised scope**: full convergence onto `doctrine.pack_paths.built_in_dir` is deferred to `#3091` (C-003) — `pack_paths` has no `missions/` content directory today; this FR closes the duplication, not the eventual packs-relocation. | Draft |
| FR-005 | Extend `charter.activation.resolver.DoctrineService` (`src/charter/activation/resolver.py:57-139`) to activation-gate the 6 mechanical currently-unfiltered kinds — `directive`, `tactic`, `styleguide`, `toolguide`, `mission_step_contract`, `glossary_pack` — replacing the `__getattr__` unfiltered passthrough (lines 136-139) for those kinds with the same three-state activation-aware filtering (`None`=catalog default / `frozenset()`=explicit empty opt-out / `{ids}`=filtered) already proven for the 3 existing gated kinds (`PackContext` already carries a matching `activated_<kind>` field for each; `doctrine.service.DoctrineService` already exposes a matching raw property — this is a mechanical copy of the existing pattern). Ship a bare-project regression test per newly-gated kind proving the default catalog is NOT silently emptied. | Draft |
| FR-006 | Add equivalent activation filtering for the `mission-type` token **entirely within `charter.activation.mission_type_profiles.resolve_mission_type_context()`** — **not** in `charter.activation.mission_type_profile_repository.MissionTypeProfileRepository`'s own file (post-tasks squad correction: that file is a different WP's exclusive ownership for an unrelated change; touching it here would create a real ownership overlap), and **not** in `charter.activation.resolver.DoctrineService`, since `doctrine.service.DoctrineService` has no `mission_types` property and `PackContext.activated_mission_types` is not three-state (the catalog-default collapse already happens at `PackContext` construction). This is a structurally different, non-mechanical change from FR-005's 6 kinds; ship a bare-project regression test proving set-equality against `builtin_mission_type_id_set()` (not a fakeable subset check) when nothing is explicitly activated. | Draft |
| FR-007 | Ship non-vacuous architectural gate(s) (AST-walk or call-site-census style, self-mutation proven, per the WP10 idiom) for each of **five** closed bypass categories: the four original (FR-001/002/003/004) plus a fifth closing the `._inner` reach-around (below). Zero-tolerance baseline — no shrink-only allowlist, per the operator's "no exceptions" decision (C-002). **Post-plan squad corrections**: (a) the gate must resolve each `DoctrineService(` call site's **bound qualname** (via import resolution, not text matching) to distinguish `doctrine.service.DoctrineService` (forbidden) from `charter.activation.resolver.DoctrineService` (the sanctioned wrapper, including its `pack_context=None` unfiltered-mode constructions) — a text-only grep cannot tell the two apart; (b) the two `.kittify/profiles` exclusions (FR-001) must be keyed by composite identity (file + qualname + line, mirroring `test_charter_path_literal_authority.py`'s allowlist shape), never a whole-file exclusion, so a genuinely new bypass added to the same file still reds; (c) every self-mutation proof must inject its violating construct at function-local or nested (`try`/`except`) scope specifically, matching the actual shape of the real violations found (R3) — a mutation that only tests module-level injection does not prove non-vacuity, per the exact lesson WP10 already learned. Extend `tests/architectural/test_org_activation_seam.py` and `tests/architectural/test_layer_rules.py` where their existing coverage is adjacent; do not re-assert what they already prove. | Draft |
| FR-008 | Unify the divergent "canonical" construction paths discovered during planning and the post-plan squad into one builder: `specify_cli.doctrine_service_factory.build_activation_aware_doctrine_service`, `charter.activation.doctrine_service_builder._build_activation_aware_doctrine_service`, and the inline "build raw, conditionally wrap" pattern repeated in `org_layer.py:244,275` and `generate.py:56` (FR-002) all collapse onto one function. Resolve the two named builders' discovered divergence by picking the *fuller* behaviour on each axis: `active_languages=infer_repo_languages(repo_root)` is always computed (the `charter` builder's behaviour) and `org_roots` is always self-resolved via `resolve_org_roots` (the `specify_cli` builder's behaviour) — never an arbitrary pick that silently drops either. **Sequencing note superseded** (post-tasks squad): the original plan staged this FR's proof across two separate work packages (a 3-kind proof, extended to 9 once FR-005 landed elsewhere). After the post-tasks squad merged FR-005's 6-kind work into the same work package as this FR, the regression test proves identical output across all 9 gated kinds in one unstaged pass — there is no cross-work-package sequencing constraint to observe. | Draft |
| FR-010 | Close the `._inner` attribute reach-around: `src/specify_cli/invocation/registry.py:64` and `src/specify_cli/invocation/org_profiles.py:117` both read `service._inner.agent_profiles` directly, bypassing every gate FR-001-008 add — an unguarded escape hatch that defeats the entire mission if left open (a genuinely gated 9-of-10 factory with a `._inner` side door is not a sole door). Both sites need the `agent_profile_repository` accessor named in FR-001, calling `get_provenance()` specifically (not `register_overlay()`, which `projection.py` needs, or `resolve_profile()`, which `runtime_bridge_io.py` needs) — use the same new public accessor rather than reaching past it a third and fourth time. | Draft |
| FR-011 | Record #2986, #3036, #3039, #3091, and #3022 as explicit deferred follow-ons, each with its one-line reason (different pair/violation-class/track), confirmed adjacent-but-separate by a pre-spec research squad. **Strengthened scope** (post-plan squad, Standing Order #8): PR-description prose alone is not durable — the precedent mission (`doctrine-charter-split-unification-01KZ0SRB`) already sets the bar by carrying `issue-matrix.json` rows with `verdict: "deferred-with-followup"` for two of these exact issues. This mission adds an `issue-matrix.json` row **and** an actual tracker comment naming this mission on each of the five issues before merge, so a later reader can discover the deferral by opening the issue itself, not only by reading this mission's PR. | Draft |

### Non-Functional Requirements

| ID | Requirement | Threshold / Measure | Status |
|----|-------------|---------------------|--------|
| NFR-001 | Zero in-scope raw bypass call sites remain, provably, by *qualname*, not by text match. | A committed architectural test resolves each `AgentProfileRepository(`/`DoctrineService(` call site's bound import to its originating module and asserts zero matches for `doctrine.service.DoctrineService`/`agent_profiles.repository.AgentProfileRepository` outside `src/charter/activation/resolver.py` and the one unified builder (FR-008) — explicitly NOT a text-only grep, since `charter.activation.resolver.DoctrineService(inner, pack_context=None)` (FR-002's sanctioned unfiltered mode) and a forbidden raw `doctrine.service.DoctrineService(...)` share the literal substring `DoctrineService(` and a name-only gate cannot tell them apart. The two `.kittify/profiles` sites (`registry.py:48`, `profiles_cmd.py:83`, FR-001) are excluded by composite key (file+qualname+line), not by file. Zero `._inner` attribute accesses on a `charter.activation.resolver.DoctrineService` instance outside `src/charter/**` (FR-010). | Draft |
...
| NFR-003 | Every new gate is non-vacuous. | Each gate shipped under FR-007 has a self-mutation test that reintroduces the exact closed bypass shape **at function-local or nested scope** (not module-level-only — matching the actual shape of the real violations, per R3) and asserts the gate fails naming the offending site — proven for all five categories (including the FR-010 `._inner` closure), not a sample. | Draft |
...
- **`charter.activation.resolver.DoctrineService`** (`src/charter/activation/resolver.py:57-139`) — the canonical, activation-aware
...
- **`ArtifactKind`** (`src/doctrine/artifact_kinds.py:128-139`) — the 10-kind (9 charter-activatable kinds +
  `mission-type` token) universe; 9 are gated via `charter.activation.resolver.DoctrineService`, `mission-type` is gated
  separately via `resolve_mission_type_context()` (FR-006).
- **`resolve_mission_type_context()`** (`src/charter/activation/mission_type_profiles.py`) — the sole function the
  `mission-type` token's activation gating lands on, since it has no property on the raw `DoctrineService`
  and its own repository (`MissionTypeProfileRepository`, `src/charter/activation/mission_type_profile_repository.py`)
  is a different WP's exclusive ownership for an unrelated change.
...
- **The five deferred issues** (#2986, #3036, #3039, #3091, #3022) — explicitly out of scope (C-003), each
  closed with an `issue-matrix.json` row and a tracker comment naming this mission (FR-011).
- **The `._inner` reach-around** (`registry.py:64`, `org_profiles.py:117`) — the escape hatch found by the
  post-plan squad that would have defeated every gate this mission ships if left open; closed by FR-010 via
  the same new lineage/mutation accessor FR-001 already needs.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: Zero unwrapped `doctrine.service.DoctrineService(...)` constructions (by resolved qualname, not
  text match — including the 3 additional sites found by the post-plan squad in `org_layer.py`/`generate.py`)
  and zero direct `AgentProfileRepository(...)` constructions remain anywhere in `src/` outside
  `charter.activation.resolver.DoctrineService`, its one unified builder function, and the two explicitly-excluded
  `.kittify/profiles` sites (excluded by composite key, not by file) — gate-enforced, not a point-in-time
  grep.
- **SC-002**: The 5-tier template/command resolver axis and its `resolve_mission` duplicate are reachable
  only through new methods on the charter factory from outside `src/charter/**`. `charter/resolution.py` and
  `template_include.py`'s type-only imports are confirmed not bypasses (corrected post-plan) and are excluded
  from this criterion.
- **SC-003**: The three duplicate missions-root hardcodes consolidate onto one promoted authority
  (`MissionTemplateRepository.default_missions_root()`, with `builtin_missions_root()` now a thin delegate
  rather than a second co-equal authority), proven by an equality regression test; the PR description states
  that full convergence with `doctrine.pack_paths.built_in_dir` still awaits `#3091`.
- **SC-004**: `charter.activation.resolver.DoctrineService` activation-gates all 9 charter-activatable `ArtifactKind`
  members (up from 3), and `resolve_mission_type_context()` separately activation-gates the `mission-type`
  token — real 10/10 coverage across two functions, not a mechanical 9/10 shortcut; a bare project with no
  activated packs sees a catalog **equal to** the raw unwrapped inner service's output for every kind
  (equality-proven, not merely "non-empty") and the mission-type set equals `builtin_mission_type_id_set()`.
- **SC-005**: A non-vacuous, self-mutation-proven (function-local/nested-scope injection, per NFR-003)
  architectural gate exists for each of the **five** bypass categories (including the FR-010 `._inner`
  closure), with a frozen zero-tolerance baseline that excludes the two `.kittify/profiles` sites and the
  `_doctrine_collect.py` unfiltered-mode sites by composite key, never by whole file.
- **SC-006**: #2986, #3036, #3039, #3091, and #3022 remain untouched by this mission's diff and each carries
  an `issue-matrix.json` row and a tracker comment naming this mission, confirmed still open at merge time.
- **SC-007** (sequencing superseded, post-tasks squad — see FR-008): The unified builder (covering the two
  named builder functions plus the `org_layer.py`/`generate.py` inline pattern) proves identical output
  across all 9 gated kinds in one unstaged pass, since FR-005's 6-kind work landed in the same work package
  (WP01) as the builder unification — there is no cross-work-package sequencing constraint to observe.
- **SC-008**: A pre-mission p95 latency baseline for `spec-kitty agent tasks status` on a 100+-WP fixture is
  captured and recorded before any FR-001 site is migrated (NFR-005) — not inferred retroactively.
