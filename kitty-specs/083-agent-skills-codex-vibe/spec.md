...
| FR-004 | The runtime MUST render every canonical `/spec-kitty.*` command template (`specify`, `plan`, `tasks`, `tasks-outline`, `tasks-packages`, `tasks-finalize`, `implement`, `review`, `accept`, `merge`, `analyze`, `research`, `checklist`, `status`, `dashboard`, `charter`) as an Agent Skills package (`<skill-root>/spec-kitty.<command>/SKILL.md` plus any required body files) into the configured skill root for each supported agent. | Proposed |
...
| FR-013 | The AI choices list, agent directories registry, and migration helpers (`AI_CHOICES`, `AGENT_DIRS`, `AGENT_DIR_TO_KEY`, `AGENT_SKILL_CONFIG`, and `get_agent_dirs_for_project()`) MUST be updated to include `vibe` as a shared-root agent whose primary skill root is `.agents/skills/`. | Proposed |
