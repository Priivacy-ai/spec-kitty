...
| C-003 | Journal stays delivery-agnostic | `event_journal` "imports nothing from `specify_cli.delivery`" (`journal.py:19`) and its `Event` record is "deliberately delivery-agnostic" (`models.py:2-9`). The new columns are a **data projection only**; the consent predicate lives in the delivery context as an injected port. Do not put consent knowledge in the journal. | Technical | High | Open |
