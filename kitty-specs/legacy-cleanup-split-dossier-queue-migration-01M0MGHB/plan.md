...
scope.
**Testing**: `pytest`, targeted at `tests/dossier/`,
`tests/sync/test_events_namespace.py`, `tests/sync/test_dossier_pipeline.py`,
and `tests/architectural/` (new guard test +
`test_shared_package_boundary.py`), per NFR-003 and the charter's "Run only
the affected test packages" rule. Full `pytest tests/` reserved for
pre-merge/post-merge validation only.
...
site, read-only evidence for FR-008). ~4 existing test files touched/
extended, 1 new guard test file added.
...
│   │                         #   _validate_payload delegation branch
...
│   └── test_dossier_pipeline.py  # FR-004: add new autospec/real-call regression
│                                 #   test(s); existing plain-Mock-patched tests
...
- `src/specify_cli/dossier/events.py` and `src/specify_cli/sync/emitter.py`
  are both CLI-owned business logic under `src/specify_cli/` — the "Internal
  Runtime Boundary" the charter describes as "CLI-owned implementation code,
  not an external shared dependency."
- No new module is created outside these two existing packages
  (`specify_cli.dossier`, `specify_cli.sync`) plus one new test module under
  the existing `tests/architectural/` convention. There is no new seam to
  name — this mission tightens an existing one.
- **No CLI command reaches past a service into kernel internals** — this
  mission does not touch any `typer.Option`/command surface at all; the four
  functions changed (`emit_artifact_indexed`, `emit_artifact_missing`, plus
  the `_PAYLOAD_RULES`/`_validate_payload` pair) are internal library
  functions called by `dossier_pipeline.py` and `drift_detector.py`, never
  exposed directly as a CLI command. `fire_dossier_event` (the sync
  boundary these emitters hand off to) is unchanged.
...
4. **Test bar (binding, per FR-004 in spec.md)**: the regression test(s)
   proving this must not rely on `tests/sync/test_dossier_pipeline.py`'s
   *existing* `@patch("specify_cli.dossier.events.emit_artifact_indexed")`
   ...
   parameter promotion were reverted. A new test (or an existing test
   modified to add `autospec=True`/`spec=emit_artifact_indexed`) is
   required — see "Red-First / ATDD Test Mapping" below for the exact test
   name and revert-behavior this plan commits to.
...
| Test shards with coverage (kernel 90% floor, fast-doctrine, fast-status, integration-doctrine, integration-status, e2e/cross-cutting, slow) | **Yes** — always | Applies unconditionally; this mission's new/changed tests land in the `fast-doctrine`-adjacent and `slow`/architectural shards depending on CI's existing bucketing of `tests/dossier/`, `tests/sync/`, `tests/architectural/` — no kernel-package (`src/kernel/`) code is touched, so the 90% kernel floor is unaffected by this mission's diff (it neither adds nor removes kernel coverage). |
...
| `src/specify_cli/sync/emitter.py` | ~90 (delete ~70 lines of 4 hand-maintained dict entries; add ~40 lines: sentinel, `_DOSSIER_EVENT_TYPES`, `_validate_dossier_payload`, branch in `_validate_payload`) |
| `tests/dossier/test_events.py` | ~10 (import re-point only, FR-009) |
| `tests/sync/test_dossier_pipeline.py` | ~80-120 (new FR-004 regression test(s) with `autospec=True`/real-call assertions) |
...
**Total estimate: roughly 500-700 LOC changed across 5 files** (2 source, 3
test files touched, 1 new test file). This fits comfortably in **one PR,
reviewable in a single sitting** — the default one-PR-per-mission shape for
...
4. **Phase 3 — FR-006/FR-007: `validate_event` delegation with sentinel
   reconciliation.** In `sync/emitter.py`: add the sentinel + 4 dossier
   entries pointing at it, add `_validate_dossier_payload`, add the
   early-return branch in `_validate_payload` (see "FR-006/FR-007 sentinel
   shape" above). Independent of Phases 1-2 (different file, no shared
   symbols) — could in principle run in parallel, but sequenced after
   Phase 2 in one PR for a simpler linear reviewable commit history (charter
   §Code Quality "Linear" PR requirement) and because Phase 5's guard test
   benefits from both emitter signatures and the validation delegation
   being final before it asserts against the real tree.
...
| FR-004 | **New** test in `tests/sync/test_dossier_pipeline.py` (e.g. `test_emit_artifact_indexed_keyword_promotion_survives_bridge_removal` / `test_emit_artifact_missing_blocking_keyword_survives_bridge_removal`), using `autospec=True` (or `spec=emit_artifact_indexed`/`spec=emit_artifact_missing`) or exercising the real functions end-to-end — NOT the existing plain-`MagicMock` `@patch` decorators, which were verified during this planning phase to accept any keyword and therefore would NOT go red on a reverted promotion | Reverting FR-004's parameter promotion (while FR-003 still deletes `**kwargs`) makes `dossier_pipeline.py`'s `step_id=`/`required_status=`/`blocking=` keyword calls raise `TypeError` at the real call boundary — with `autospec=True`, the mock enforces the real signature and raises the same `TypeError`, so the test goes red; with a plain `Mock` it would NOT (this is exactly why the plain-Mock tests don't count as FR-004 proof). |
...
| FR-006 | **New** test driving a hand-constructed invalid dossier payload (e.g. missing `namespace`) through `EventEmitter._validate_payload()`, asserting it returns `False` **and** the captured warning text contains a real field/violation identifier sourced from `ConformanceResult` (not the old generic "field has invalid value" string) — this is SC-005 made concrete | Reverting FR-006 (restoring the 4 hand-maintained dict entries) makes the warning text revert to the old generic format — the assertion on real-violation-detail content in the warning string fails. |
...
parallelized across multiple agents/developers.** The ~500-700 LOC estimate
(see "PR Shape") and the tight file-level coupling between phases (Phase 1
and Phase 2 both edit the same ~150 contiguous lines of `events.py`; Phase 3
is the only phase touching a different file and is still sequenced serially
per "Linear" PR history discipline) make this a poor candidate for
