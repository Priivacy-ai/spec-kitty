...
- **Q: Issue #1058's plan step 1 targets a queue-drain migration transform in
  ...
  **A: No — it is gone, superseded by mission #3293.** `_migrate_legacy_dossier_payload`
  ...
  (`src/specify_cli/sync/queue.py:455-473`) is a plain read/decode of rows already in
...
- **Q: Deleting `_consume_legacy_values` and the `*args`/`**kwargs` bridge from
  ...
  **A: No — it would break a live, production, in-repo call site if done naively.**
  ...
  ...)` (lines 101-116) and `emit_artifact_missing(..., blocking=artifact.required_status
  == "required", ...)` (lines 126-139). `step_id`, `required_status`, and `blocking` are
...
`SpecKittyEventEmitter._validate_payload()`; the event is rejected with a warning
...
| NFR-003 | Test suite scope stays bounded | Per-WP validation targets `tests/dossier/`, `tests/sync/test_events_namespace.py`, `tests/sync/test_dossier_pipeline.py`, and `tests/architectural/` (for the new guard test and `test_shared_package_boundary.py`); the full `pytest tests/` run is reserved for pre-merge / post-merge validation per the charter's Testing Requirements section. | Process | Medium | Open |
...
- **`_PAYLOAD_RULES` dossier entries** (`specify_cli.sync.emitter`): change from four
  ...
  **Reconciling FR-006 and FR-007**: `_validate_payload`'s single generic code path
  ...
  this by picking one concrete shape, e.g.: `_PAYLOAD_RULES[event_type]` for the four
  dossier types becomes a sentinel value (or the four keys are removed from
  `_PAYLOAD_RULES` and instead folded into `VALID_EVENT_TYPES` via a separate
  `DOSSIER_EVENT_TYPES` frozenset — plan.md picks one), and `_validate_payload` gains
  an explicit early-return branch, e.g. `if event_type in DOSSIER_EVENT_TYPES: return
  self._validate_dossier_payload(event_type, payload)`, ahead of the generic
  `rules["required"]`/`rules["validators"]` access — so the two code paths never both
  try to interpret the same dict value the same way. This reconciliation is a binding
  requirement of this spec, not an open question; only the exact shape is deferred to
  planning.
...
- **SC-005**: `_PAYLOAD_RULES`'s four dossier entries route through
  ...
  `SpecKittyEventEmitter._validate_payload()` returns `False` and the captured warning
