...
| **Architectural alignment / LD-3** | The reader calls the new `charter.bundle` helper via a LAZY import (matching computer.py's existing lazy `charter.bundle`/`charter.activation.synthesizer.manifest` imports) — no new eager cross-package import on the `spec-kitty next` hot path (NFR-002/003). | **PASS** |
