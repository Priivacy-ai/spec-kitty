...
| T013 | Add `resolve_transitive_refs()` in `src/doctrine/drg/query.py` + equivalence test suite | WP03 | |
| T014 | Add `InlineReferenceRejectedError` to 7 per-kind validators + negative-fixture test suite | WP03 | [P] with T013 |
| T015 | Flip `resolver.py`/`compiler.py` (twin packages) to DRG helper + add live-path regression test | WP03 | |
| T016 | Flip 5 call sites to `build_context_v2`; rename `build_context_v2` → `build_charter_context`; delete legacy; update re-exports | WP03 | |
| T017 | Delete `src/charter/reference_resolver.py`; remove `include_proposed` from `src/charter/catalog.py` + callers | WP03 | |
| T018 | Rewrite `tests/charter/test_context.py`; delete parity/resolver tests only after replacement green; finalize occurrence artifacts | WP03 | |
