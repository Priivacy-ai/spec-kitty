...
Phase 0 of the Doctrine Reference Graph (DRG) rebuild landed in 2026-04 and produced a parity-safe `build_context_v2()` at `src/charter/activation/context.py:495`, a DRG infrastructure package at `src/doctrine/drg/`, a merged-graph validator, and a graph asset at `src/doctrine/graph.yaml`. [PR #609](https://github.com/Priivacy-ai/spec-kitty/pull/609) closed the final Phase 0 parity gaps (live-path graph validation, state-management parity, directive filtering, action-guideline rendering).
...
- **`src/charter/activation/reference_resolver.py` and `load_doctrine_catalog(include_proposed=...)` still exist.** They power the legacy transitive-reference resolution that Phase 0 replaced with the merged-graph walk, and they are still called from `src/charter/activation/compiler.py` and `src/charter/activation/resolver.py`.
...
| FR-010 | `src/charter/activation/reference_resolver.py` must be deleted, along with its imports and call sites in `src/charter/activation/compiler.py` and `src/charter/activation/resolver.py`. The affected compiler/resolver modules must be refactored to consume the DRG merged-graph walk instead (or deleted if they only existed to drive the legacy resolver). | Draft |
| FR-011 | `load_doctrine_catalog()` in `src/charter/activation/catalog.py` must no longer accept an `include_proposed` parameter, and every call site must be updated to match. | Draft |
...
- Delete `src/charter/activation/reference_resolver.py`, remove its imports from `src/charter/activation/compiler.py` and `src/charter/activation/resolver.py`, and refactor those modules (or delete them if they only existed to drive the legacy resolver).
...
- Remove `include_proposed` from `src/charter/activation/catalog.py` and all callers.
...
- Legacy module files that exist only to drive the reference resolver (`src/charter/activation/compiler.py`, `src/charter/activation/resolver.py`) may shrink dramatically or disappear in WP1.3. The plan phase will decide whether they are refactored or deleted based on what functionality — if any — survives the resolver excision.
...
| `src/charter/activation/compiler.py` or `src/charter/activation/resolver.py` carry logic beyond reference resolution that the mission inadvertently deletes. | Medium | Medium (feature loss) | Plan-phase decision on rewrite-vs-delete must be backed by an explicit functional audit; WP1.3 spec must call out every non-resolver behavior found. |
...
- `src/charter/activation/schemas.py` (strip `applies_to`)
...
- `src/charter/activation/_drg_helpers.py` (NEW — `_load_validated_graph`)
- `src/specify_cli/charter/_drg_helpers.py` (NEW twin)
- `src/charter/activation/context.py` (collapse to single builder)
- `src/charter/__init__.py` + `src/specify_cli/charter/__init__.py` (exports)
- `src/charter/activation/reference_resolver.py` (DELETE)
- `src/charter/activation/compiler.py` + `src/charter/activation/resolver.py` (swap resolver import; do NOT delete)
- `src/charter/activation/catalog.py` (drop `include_proposed`)
