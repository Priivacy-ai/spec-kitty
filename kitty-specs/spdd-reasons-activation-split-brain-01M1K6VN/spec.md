...
**Finding.** `resolve_project_governance` (`resolver.py:815-875`) resolves "active governance
from project + org charter selection data," and its return type, `GovernanceResolution`, is
docstring-labeled "Resolved governance activation result" (`resolver.py:595`) — an activation
...
1. **The directive base is a silent full override, not a union or a display.**
   `_resolve_directive_base` (`resolver.py:675-737`) already has a
   `PackContext.activated_directives`-aware fallback (lines 728-737, with a documented
   three-state guard) — but it is consulted ONLY when `doctrine.selected_directives` is empty
   (line 716: `if doctrine.selected_directives: ... return list(doctrine.selected_directives),
   "charter"`, the charter-priority/validation branch spanning lines 716-726). When the stale,
   authored `selected_directives` is non-empty — the ordinary case for any project that selected
   directives at bootstrap — it is returned **verbatim**, and `activated_directives` is never
   consulted at all. Unlike the "exclude/under-seed" softness Decision Record 2 found in
   `action_doctrine_bundle.py`, this is a full override.
...
Both feed `GovernanceResolution.directives`/`.paradigms`, consumed by five real, verified call
...
- `src/charter/activation/compact.py:293-317` (`_resolve_governance_summary`) — feeds the
...
- `src/charter/activation/resolver.py:937-952` (`collect_governance_diagnostics`) — a fifth
  call site, inside `resolver.py` itself, missed by the original four-site count and found
  during this fix round's fresh-sweep review. It is also re-exported as a public facade symbol
  from `src/charter/__init__.py` (lines 125, 202), so it is a real, potentially
  externally-consumed API surface, not a test-only helper. Unlike the other four, it does not
  need its own FR: it only forwards `resolution.diagnostics` (line 952) or, on
  `GovernanceResolutionError`, `exc.issues` (line 951) — never `.directives`/`.paradigms`
  directly — so FR-011's re-derivation of `resolve_project_governance`'s base directive/paradigm
  sets fixes this path too, as a side effect, with no separate handling required.
...
compact governance summary (`compact.py`), the `doctor doctrine` governance-resolution
check (`doctor.py`), and `resolver.py`'s own `collect_governance_diagnostics` (re-exported from
`charter/__init__.py`) — I want the resolved directive set and paradigm list to reflect what is
...
from any of these five surfaces — including the exact "downstream doctrine/charter panel"
...
3. **Given** `governance.charter.selected_directives` names a directive that IS also present in
   ...
   Story 1 Scenario 3).
4. **Given** the five real consumers (`prompt_builder.py`'s legacy governance fallback,
   `context_json.py`'s `--json` `all_directives`/`directives_source`, `compact.py`'s governance
   summary, `doctor.py`'s `check_governance_resolution`, and `resolver.py`'s own
   `collect_governance_diagnostics`), **When** any of them calls `resolve_project_governance`,
   **Then** each reflects the corrected, `activated_*`-derived resolution without further code
   changes to the five consumer call sites themselves (the fix is isolated to
   `resolve_project_governance`/`_resolve_directive_base` inside `resolver.py`).
...
- **A general audit of every other consumer of `governance.charter.selected_*` in the
  ...
  was selected, not gating what is active) and is not touched by this mission. This carve-out
  also does not separately name `collect_governance_diagnostics` (`resolver.py:937-952`, a
  fifth `resolve_project_governance` call site re-exported from `src/charter/__init__.py`,
  found during this fix round's fresh-sweep review) because it needs no separate handling: it
