...
- topologies/lifecycle: flat, coord-capable metadata, active and terminal examples where available;
...
One specimen may cover multiple features, but each role/era/size/topology stratum gets an independently selected row; the sample expands if any row is empty. Query targets are selected by SHA-256 order from the eligible atomic-fact population, not by expected candidate performance. No confirmatory specimen may have been used in exploratory conversion.
...
- **Enumeration/selection**: `git ls-tree -r -z --long cf0f7e3a7 -- kitty-specs` supplies tracked bytes and paths. Role classifiers are frozen basename/glob tables; mission era is legacy numeric vs current ULID regex; topology/type come only from `meta.json`; terminal state comes only from canonical status events. Missing metadata is `unknown`, never inferred. Size strata use nearest-rank order over `(bytes, path)`: rank `ceil(p*n)` for p=0.5/0.9; "large-but-runnable" is largest eligible file at most 2 MiB. The selector greedily fills independently ordered strata, enforces two files per mission, then expands until all available rows are filled. Exact paths/blob hashes are output, not hand-picked.
...
- **Answers**: all procedures emit `query_id,fixture_id,atom_type,subject,predicate,object,source_path,source_blob,start_byte,end_byte,status`. Exact-set atom matching supplies correctness; partial credit is allowed only for registry-enumerated compound answers.
...
- C7 greenfield canonical DoclingDocument with generated Markdown/graph views;
- C8 greenfield canonical typed semantic model with generated Markdown/graph views;
- C9 greenfield canonical graph with generated Markdown view.

Brownfield C7–C9 are structurally rejected: replacing an editable existing source either overwrites user content or creates forbidden dual authority. Their greenfield rows remain counterfactual only if they define one-way cutover, immutable pre-cutover source preservation, sole writer, transaction, rollback, and complete consumer migration. Other explicitly pruned cells remain in the registry: remote-default, DRG namespace reuse, bidirectional editing, unconsented hosted projections, and cross-repository fusion without authorization/identity/contradiction rules.
