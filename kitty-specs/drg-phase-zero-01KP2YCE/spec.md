...
For every supported `(profile, action, depth)` combination, `build_context_v2()` produces governance context that matches the output contract of the currently shipped `src/charter/context.py` path. Differences are either exact matches or itemized intentional improvements (e.g., legacy path was wrong-sized).

**Verification**: Run invariant test matrix; assert semantic identity or accepted difference with explicit justification per entry.
...
**Verification**: Calibration test asserts these inequalities. When violated, the fix is adjusting `scope` edges in `graph.yaml`, never adding filtering logic in the context builder.

### Scenario 4: CI catches governance context regressions

A contributor modifies a directive YAML file and opens a PR. The invariant test detects that the DRG path and legacy path now diverge for one `(profile, action)` pair. The PR is blocked until the contributor either updates `graph.yaml` to reflect the change or updates the expected-differences manifest.
...
This must be resolved before the invariant test can use a single oracle.

## Functional Requirements

| ID | Requirement | Status |
|----|-------------|--------|
| FR-001 | Reroute all `build_charter_context()` callers to the canonical `src/charter/context.py` path. After reroute, `src/specify_cli/charter/context.py` has zero internal callers. | Proposed |
...
| FR-006 | Implement `build_context_v2(profile, action, depth)` that queries the merged DRG (shipped + project-local layers), walks `applies` and `scope` edges to depth 1, walks `requires` transitively, walks `suggests` to user-configured depth, includes `vocabulary` edges as glossary scope, materializes each resolved artifact, and returns a structured prompt block. | Proposed |
| FR-007 | Provide an invariant regression test that compares the output of `build_context_v2(profile, action, depth)` against the canonical `src/charter/context.py` `build_charter_context()` for every shipped profile x action x depth combination. Semantic identity is the default; intentional differences must be itemized in an accepted-differences manifest. | Proposed |
...
1. The invariant regression test passes for 100% of shipped profile x action x depth combinations, with any accepted differences explicitly itemized and reviewed.
...
4. All `build_charter_context()` call sites use the canonical `src/charter/context.py` path; `src/specify_cli/charter/context.py` has zero internal callers.
...
| Legacy context compatibility surface | `src/specify_cli/charter/context.py` | Delete after call-site reroute confirmed |
...
2. The call-site reroute (FR-001) is the only change to existing production behavior. If reroute causes issues, reverting that single commit restores the old import paths.
3. No existing tests are modified. New tests are additive.

**Rollback trigger**: If the invariant test reveals that `src/charter/context.py` and `src/specify_cli/charter/context.py` have materially different output for any active call site, pause and resolve the divergence before proceeding. Do not average or reconcile both implementations.
