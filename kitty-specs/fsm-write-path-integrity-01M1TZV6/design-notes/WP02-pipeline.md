...
| D-2 | The plain batch door alone skipped the `<execution_mode>:<root>` workspace-context default on `claimed → in_progress` (#946); `_prepare_event`, and so the transactional batch, always applied it. | **Parity taken**: the batch composes the pipeline verbatim, so the default now applies. Production callers (`status/work_package_lifecycle.py:159/:198`) always pass `workspace_context` for this edge, so only direct callers that omitted it observe it. Pinned by `test_batch_claimed_to_in_progress_defaults_workspace_context_like_every_other_door` with the #946 provenance in its docstring. | Confirm parity, or reinstate the #946 skip as an explicit pipeline policy knob if the operator wants the plain batch to fail closed on a missing workspace. **D-2 resolution (2026-09-07): reverted to fail-closed per operator** (mission-review DRIFT-3) — `prepare_transition(default_workspace_context: bool = True)`; the plain batch door passes `False`, the guard refuses with the historical "requires workspace context" message, the other three doors keep the default. Pinned by `test_batch_claimed_to_in_progress_without_workspace_context_is_refused` and `TestDefaultWorkspaceContextKnob`. |
