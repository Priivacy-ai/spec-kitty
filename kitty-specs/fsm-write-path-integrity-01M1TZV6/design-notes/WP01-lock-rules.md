...
L1  mission status lock (re-entrant per thread, 3 deep in production)
     └──▶ L4 (coord worktree) ──▶ git subprocess   ← ONLY via BookkeepingTransaction (R9, unchanged)
```

- No new code path takes L1 and then spawns git (NFR-001 / C-002). Checked per site: ④ appends then `materialize` (no git); ⑤ read + append; ⑥ artifact reads + event-log read + append; ⑦ frontmatter reads + event-log reads + one append. The git probe in `_git_common_dir` runs **before** the acquire, never while holding.
- **R9 note.** The standing L1-across-git violation is `BookkeepingTransaction` (`coordination/transaction.py:290` acquires; `commit()` runs `safe_commit` under it). WP01 does not change it. WP01 *does* grow the victim population of a hung-git-under-L1 stall (families ④–⑦ now wait on that lock), which is exactly why rule (b) bounds the merge-path take and why a finite default remains the named follow-up on #3893.
