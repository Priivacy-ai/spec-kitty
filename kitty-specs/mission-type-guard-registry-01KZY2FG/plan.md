...
| `tests/architectural/test_shared_package_boundary.py` / `test_pyproject_shape.py` | **Runs on this PR (`arch-adversarial`, always-on), expected to pass** | `arch-adversarial` runs `tests/architectural` (including these two files) on every push of this PR regardless of which paths changed — the job's own governing comment states it deliberately carries no dorny path-filter gate so it fires on 100% of pushes (`.github/workflows/ci-quality.yml:2144`, job definition; `if:` condition at line 2147: `(always()) && !contains(github.event.pull_request.labels.*.name, 'pr:deferred') && !contains(github.event.pull_request.labels.*.name, 'pr:skip-ci')`) — the only opt-outs are the `pr:deferred`/`pr:skip-ci` PR labels, not a docs-only/non-docs-only path distinction — not only incidentally at a separate merge-time full-suite run. No dependency, `pyproject.toml`, or `spec-kitty-events`/`spec-kitty-tracker` import is touched by this mission, so both are expected to pass. |
