...
| FR-004 | Composed-path neutral degrade logs the unregistered family at WARNING level or above | As an operator debugging why a custom mission type's guards behave neutrally, I want a log line recording the specific unregistered `mission_family` value at the moment of the degrade, emitted at **WARNING level or above** (matching the existing degrade-and-continue logging convention at `runtime_bridge.py:348-354` — `DecisionGitLog` construction failing and falling back to a plain emitter, the genuine precedent for a WARNING-level "problem detected, continuing anyway" log in this file — not DEBUG or lower), so the condition is discoverable in default-configured logs without reading source. Cross-reference: FR-003 (this is the sole interface differentiator for that neutral result) and FR-007 (`doctor mission-type` gives operators a proactive, non-log-dependent way to discover the same unregistered/unresolved state). | High | Open |
...
1. **The project-wide doctrine-override hatch** — `_project_has_doctrine_overrides`
   (`src/charter/mission_type_profiles.py:1041`): an unknown `mission_type` hard-fails only when
...
2. **The two divergent meta readers** — `_read_meta_mission_type`
   (`src/charter/mission_type_profiles.py:681`), which reads only the canonical `mission_type`
