...
title: '/tmp burn-down: doctrine + agent'
dependencies: []
requirement_refs:
- FR-001
- FR-002
...
# WP05 – /tmp burn-down: doctrine + agent
...
## Objective
Convert the 12 baselined test files below OFF literal `/tmp` — genuine write-leaks to `tmp_path`/fixtures, arbitrary path-literals to non-`/tmp` sentinels — preserving each test's exact behavior. **Do NOT edit `tmp_ratchet_baseline.txt`** (WP08 owns it; a converted file left baselined is simply skipped by the ratchet, so the gate stays green).
...
## Commit
`git add <your files> && git commit -m "test(doctrine): convert WP05 tests off literal /tmp (tmp_path/sentinels) — refs #1842"`
...
- 2026-07-06T23:18:25Z – claude – shell_pid=1002852 – Assigned agent via action command
- 2026-07-06T23:27:49Z – claude – shell_pid=1002852 – Moved to for_review
- 2026-07-06T23:51:24Z – user – shell_pid=1002852 – Moved to approved
