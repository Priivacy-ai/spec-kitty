...
"""
        ...
        output_dir.mkdir(parents=True, exist_ok=True)
        # conf.py declares `html_static_path = ['_static']`; Sphinx treats a
        # missing static path as a build warning (fatal under -W), so create the
        # directory the config references rather than shipping a conf.py that
        # cannot build.
        (output_dir / "_static").mkdir(parents=True, exist_ok=True)
        try:
