...
Note: ``__all__`` is intentionally omitted. The public symbols here have no
``src/`` caller until the WP06+ rewire wires them; declaring them in ``__all__``
now would trip the symbol-level dead-code gate
(``tests/architectural/test_no_dead_symbols.py``). The rewire WPs add ``__all__``
once the ports gain live callers.
"""
