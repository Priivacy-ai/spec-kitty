...
def preflight_resynthesis(repo_root: Path, kind: str, artifact_id: str, scope: CascadeScope | None,
                          graph: DRGGraph) -> None:
    ...
    _load_interview_for_generate(
        repo_root=repo_root, answers_path=_interview_path(repo_root), from_interview=True,
        resolved_mission_type=None, profile="minimal",
    )
    ...
    if scope is not None and kind != "mission-type":
        source = resolve_artifact_urn(ArtifactKind.from_operator_token(kind), artifact_id,
                                      doctrine_root=resolve_doctrine_root(), layer_roots=roots, org_roots=org_roots)
        cascaded = cascade_activation_targets(graph, source, scope)
        for kind_value, identifiers in cascaded.activated.items():
            for identifier in identifiers:
                config_id = resolve_config_id(f"{kind_value}:{identifier}", doctrine_root=resolve_doctrine_root(),
                                              layer_roots=roots, org_roots=org_roots)
