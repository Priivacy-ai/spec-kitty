...
def _resolve_charter_path(repo_root: Path) -> Path:
    """Find charter file in project, trying new and legacy locations."""
...
def generate(
    ...
    try:
        repo_root = find_repo_root()
        charter_dir = repo_root / ".kittify" / "constitution"
        answers_path = _interview_path(repo_root)
        ...
        charter_path = charter_dir / "constitution.md"
...
def sync(
    ...
    """Sync charter to structured YAML config files."""
    try:
        repo_root = find_repo_root()
