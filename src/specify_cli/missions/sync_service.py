"""Sync daemon orchestration service (canonical home).

Canonical location: ``specify_cli.missions.sync_service``.

This module owns ``SyncService``, ``SyncTriggerResult``, and the
``_build_trigger_request`` helper. They are domain-layer building blocks for
ensuring the local sync daemon is running and asking it to flush, surfaced to
the dashboard via a thin adapter (FR-007 of the parent
dashboard-service-extraction mission).

A backward-compatibility shim remains at ``dashboard.services.sync``
re-exporting these names; that shim is scheduled for removal in release
``3.2.0``. Do not add business logic to the shim — edit this module instead.
"""
