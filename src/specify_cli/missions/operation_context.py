"""Resolve one Mission identity and checkout context for lifecycle requests.

The repository root remains the Git/topology anchor.  The mission anchor may be
an explicit root, a managed Spec Kitty checkout, or a caller-owned linked
worktree.  Resolution is read-only and refuses to select a surface when
allowed checkouts contain conflicting immutable mission identities.
"""
...
from specify_cli.core.paths import get_main_repo_root, is_worktree_context


def _nearest_checkout_root(start: Path) -> Path | None:
    """Return the nearest ancestor carrying a Git checkout marker."""
...
def git_common_dir_for_checkout(checkout: Path) -> Path | None:
    """Return Git's shared common directory for *checkout*, read-only.

    Using Git's own resolution keeps generic external worktrees and relative
    ``gitdir:`` pointers correct without duplicating pointer parsing here.
    """
    try:
    ...
    if not common_dir.is_absolute():
        common_dir = checkout.resolve() / common_dir
...
class MissionSurfaceConflictError(RuntimeError):
    """Raised before writes when allowed surfaces disagree on identity."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            f"{self.error_code}: selector {selector!r} resolves to conflicting "
            f"Mission identities across: {roots}"
        )
...
def _implicit_candidates(repository_root: Path, cwd: Path) -> list[_Candidate]:
    ...
    if (
        ...
        topology = classify_worktree_topology(current_checkout, repo_root=repository_root)
        if topology in {WorktreeTopology.COORD_WORKTREE, WorktreeTopology.LANE_WORKTREE}:
            # Managed topology keeps the repository-root checkout authoritative.
            # The current checkout is still indexed as a conflict probe so a
            # stale or divergent copy cannot hide split-brain.
            candidates.append(_Candidate(repository_root, CheckoutKind.MANAGED))
            candidates.append(_Candidate(current_checkout, CheckoutKind.MANAGED, selectable=False))
...
def _resolve_from_index(indexed: _IndexedCandidate, selector: str) -> ResolvedMission | None:
    try:
...
def _conflict_projection(
    ...
    return tuple(
        sorted(
            relevant.values(),
            key=lambda candidate: (_path_key(candidate.root), candidate.mission_id, candidate.mission_slug),
        )
    )
...
def resolve_mission_operation_context(
    ...
    *,
    ...
    """Resolve mission identity and its repository/anchor roots read-only.

    Without ``explicit_root``, the current same-repository caller-owned or
    managed worktree is considered before the repository-root fallback. Every
    candidate is indexed once; conflicting mission IDs fail closed.
    """
    ...
    candidates = (
        [_Candidate(explicit_anchor, CheckoutKind.EXPLICIT)]
        if explicit_root
        else _implicit_candidates(repository_root, (cwd or Path.cwd()).resolve())
    )
    indexed = [_index_candidate(candidate) for candidate in candidates]
    resolved = [
        (candidate, mission)
        for candidate in indexed
        if (mission := _resolve_from_index(candidate, selector)) is not None
    ]
    if not resolved:
    ...
    if len({candidate.mission_id for candidate in conflict_candidates}) > 1:
        raise MissionSurfaceConflictError(selector=selector, candidates=conflict_candidates)
