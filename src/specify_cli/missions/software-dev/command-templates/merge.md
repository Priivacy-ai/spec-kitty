...
- Branch: Can be any project branch; the command will switch to the merge target branch in the primary repository
...
**Correct the issue:**
1. Navigate back to the primary repository root: `cd /path/to/project`
2. Verify `pwd` is not in `.worktrees/`
...
- The merge command detects all WP branches and merges them sequentially
- You do NOT run merge once per WP
