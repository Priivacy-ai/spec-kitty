...
class SaaSTrackerService:
    ...
    def resolve_and_bind(
        ...
        *,
        ...
        bind_ref: str | None = None,  # noqa: ARG002 -- reserved for re-bind flow
        ...
        if resolution.match_type == "exact":
            if resolution.binding_ref:
                ...
                self._persist_binding(
                    provider, resolution.binding_ref, resolution.display_label, None
                )
                return BindResult(
        ...
        raise TrackerServiceError(
            f"Unexpected match_type '{resolution.match_type}' from bind-resolve."
        )
    ...
    def _persist_binding(
        ...
        """Write binding to config, preserving legacy project_slug."""
    ...
    def _confirm_and_persist(
        ...
        """Confirm a candidate token and persist the resulting binding."""
        try:
