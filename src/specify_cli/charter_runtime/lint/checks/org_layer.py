...
def _build_service_with_org_layer(
    ...
    if not org_roots:
        return None
    # built_in_root=None → repositories self-resolve packs/built-in/<kind> (WP04
    # seam); resolve_doctrine_root() would point at the emptied src/doctrine tree.
...
def _build_built_in_only_service(
    ...
    inner = DoctrineService(built_in_root=None, project_root=project_root)
    if pack_context is not None:
        try:
            from charter.resolver import DoctrineService as ActivationDoctrineService  # noqa: PLC0415
            return ActivationDoctrineService(inner, pack_context=pack_context)
