...
class Mission:
    ...
    def _load_and_validate_config(self) -> MissionConfig:
        ...
        normalized_config = {
            key: value for key, value in raw_config.items() if key not in MISSION_COMPAT_IGNORED_FIELDS
        }
...
def get_deliverables_path(feature_dir: Path, feature_slug: Optional[str] = None) -> Optional[str]:
    ...
    if meta_file.exists():
        try:
            ...
            mission = meta.get("mission") if isinstance(meta.get("mission"), str) else "software-dev"
            if mission == "research":
...
def get_mission_for_feature(feature_dir: Path, project_root: Optional[Path] = None) -> Mission:
    ...
    try:
        ...
        warnings.warn(
            f"Mission '{mission_key}' not found for feature {feature_dir.name}, using software-dev as default",
            stacklevel=2,
        )
