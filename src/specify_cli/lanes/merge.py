    ...
    *,
    ...
    try:
        ...
        if strategy == MergeStrategy.SQUASH:
            ...
            if result.returncode != 0:
                raise RuntimeError(f"Failed to fast-forward {target_branch} after rebase: {result.stderr.strip()}")
        ...
        if result.returncode != 0:
            raise RuntimeError(f"Failed to update {target_branch} ref: {result.stderr.strip()}")
    finally:
        subprocess.run(
