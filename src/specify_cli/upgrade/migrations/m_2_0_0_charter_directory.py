"""Migration: Move charter to `.kittify/charter/` directory."""
...
class CharterDirectoryMigration(BaseMigration):
    """Move charter file from memory root to dedicated charter directory."""

    migration_id = "2.0.0_charter_directory"
    description = "Move charter to .kittify/charter/ directory"
    target_version = "2.0.0"

    def detect(self, project_path: Path) -> bool:
        """Return True when legacy charter location is still in use."""
        old_path = project_path / ".kittify" / "memory" / "charter.md"
    ...
    def apply(self, project_path: Path, dry_run: bool = False) -> MigrationResult:
        """Apply charter relocation and optional initial sync extraction."""
        result = MigrationResult(success=True)

        old_path = project_path / ".kittify" / "memory" / "charter.md"
        new_dir = project_path / ".kittify" / "charter"
        new_path = new_dir / "charter.md"
        ...
        if old_path.exists() and not new_path.exists():
            ...
            try:
                from specify_cli.charter.sync import sync
                ...
                result.warnings.append(
                    f"Initial extraction skipped ({e}). Run 'spec-kitty charter sync' manually."
                )
            return result
        ...
        if old_path.exists() and new_path.exists():
            result.changes_made.append(
                f"Charter already at {new_path.relative_to(project_path)}, "
        ...
        if new_path.exists() and not old_path.exists():
            result.changes_made.append(f"Charter already at {new_path.relative_to(project_path)}")
            return result

        # Scenario 4: Neither exists -> skip (no charter)
        result.changes_made.append("No charter found, skipping migration")
        return result
...
class Migration:
    ...
    version = "2.0.0"
    description = CharterDirectoryMigration.description

    def apply(self, project_path: Path, dry_run: bool = False) -> list[str]:
        impl = CharterDirectoryMigration()
