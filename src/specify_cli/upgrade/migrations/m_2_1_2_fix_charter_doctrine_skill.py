"""Migration: expand charter-doctrine skill with extraction rules and schemas.
...
_SKILL_NAME = "spec-kitty-charter-doctrine"

# The old SKILL.md lacks the "How the Charter System Works" architecture
...
class FixCharterDoctrineSkillMigration(BaseMigration):
    """Expand charter-doctrine skill with extraction rules and schemas."""

    migration_id = "2.1.2_fix_charter_doctrine_skill"
    description = (
        "Expand charter-doctrine skill with governance.yaml schema, "
        "extraction rules, interview profiles, and doctrine asset catalog"
    )
    target_version = "2.1.2"

    def detect(self, project_path: Path) -> bool:
        """Return True if any charter-doctrine SKILL.md lacks the new docs."""
    ...
    def can_apply(self, project_path: Path) -> tuple[bool, str]:
        """Check if project has charter-doctrine skill files."""
    ...
    def apply(self, project_path: Path, dry_run: bool = False) -> MigrationResult:
        """Replace charter-doctrine SKILL.md with expanded version."""
        changes: list[str] = []
        errors: list[str] = []
        ...
        try:
            ...
            if fallback.is_file():
                ...
                return MigrationResult(
                    success=False,
                    errors=["Cannot locate canonical SKILL.md for charter-doctrine"],
                )
        ...
        if not changes and not errors:
            changes.append("All charter-doctrine skill files already up to date")
