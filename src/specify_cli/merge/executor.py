...
def execute_merge(
    ...
    if resume_state:
        ...
        ordered_workspaces = [
            (wt_path, wp_id, branch) for wt_path, wp_id, branch in ordered_workspaces if wp_id in remaining_set
        ]
...
def _cleanup_sparse_checkout_config(repo_root: Path) -> None:
    ...
    try:
        ...
        worktree_count = sum(1 for line in result.stdout.splitlines() if line.startswith("worktree "))
