...
``agent_profile:<profile-id>`` :class:`~doctrine.drg.models.DRGNode` objects, so
...
Home rationale (KD-1 / C-001): the walk lives under ``doctrine.drg`` — below
...
(:func:`doctrine.drg.migration.extractor._discover_built_in_nodes_in_dir`):
...
from doctrine.drg.models import DRGNode, NodeKind, is_valid_urn
...
#: :class:`doctrine.agent_profiles.repository.AgentProfileRepository`, which
#: accepts both.
