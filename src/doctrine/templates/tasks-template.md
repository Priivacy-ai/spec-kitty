...
**Prompt Files**: Each work package references a matching prompt file in `/tasks/` generated by `/spec-kitty.tasks`. Status is managed via `status.events.jsonl`. Use `spec-kitty agent tasks move-task` to change WP status. Treat this file as the high-level checklist; keep deep implementation detail inside the prompt files.
