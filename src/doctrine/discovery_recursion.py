...
Both the loader (:mod:`doctrine.base`, :mod:`doctrine.agent_profiles.repository`)
...
:mod:`doctrine.artifact_kinds` (itself zero-dependency). ``charter`` imports
...
def overlay_scan_is_recursive(kind: ArtifactKind | None) -> bool:
    ...
    *kind* is ``None`` when a caller cannot map its scan to a canonical
    :class:`ArtifactKind` — e.g. a :class:`~doctrine.base.BaseDoctrineRepository`
