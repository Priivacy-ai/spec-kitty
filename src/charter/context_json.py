...
that file). Backs :func:`~charter.context.build_charter_context_json`'s
...
:func:`~charter.doctrine_service_builder._build_doctrine_service` via a
function-local ``from charter.context import _build_doctrine_service``
rather than a direct import of ``charter.doctrine_service_builder`` — several
existing tests (``tests/charter/test_context.py::test_project_directive_entries_fallbacks``)
patch only ``charter.context._build_doctrine_service`` and expect
``_project_directive_entries`` (re-exported from this module) to observe it;
routing through ``charter.context`` keeps that single patch-point contract.
:func:`_project_charter_json_block` similarly resolves ``YAML`` via a
function-local import from ``charter.context`` — ``tests/charter/test_context.py``
patches ``charter.context.YAML`` directly.
"""
