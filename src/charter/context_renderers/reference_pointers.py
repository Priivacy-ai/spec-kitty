...
* ``_REFERENCE_SOURCE_INDEX_CACHE`` is a process-wide cache of the
  ...
  every :func:`charter.context.build_charter_context` call. The cache lives
  here (not in ``charter.context``) because it is populated and read only by
  :func:`_reference_source_index`.
* ``_REFERENCE_POINTER_LIMIT`` / ``_REFERENCE_POINTER_FLOOR`` are re-exported
  from :mod:`charter.context` (a thin re-export, mirroring the delivery-table
  ...
  ``charter.context``'s bootstrap-text rendering path; the remaining helpers
