...
Relocated verbatim from ``charter.context``: the dispatch-facing compact
...
``charter.context`` (org-pack-discovery / doctrine-service-builder /
...
(``charter.context`` imports this module for its re-export shim), mirroring
the existing lazy-import precedent already used throughout ``charter.context``.
...
def _render_compact_governance(
    ...
    *,
    ...
    :func:`charter.compact.render_compact_view` for the full rationale).
    ...
    # Cycle note: ``_load_doctrine_selection`` stays in ``charter.context``
    # (see module docstring); function-local import avoids a load cycle.
    from charter.context import _load_doctrine_selection  # noqa: PLC0415
    ...
    if profile is not None:
        ...
        # stay in ``charter.context`` (see module docstring); function-local
        # imports avoid a load cycle.
        from charter.context import _build_doctrine_service, _render_profile_sections  # noqa: PLC0415
