...
Relocated verbatim from ``charter.context``: the org∪project activation-entry
...
``charter.context._iter_org_charter_docs`` — that helper is part of the
org-pack-discovery cluster, which stays in ``charter.context`` until a later
WP relocates it. A function-local import breaks the load-time cycle a
top-level import would create (``charter.context`` imports this module for
...
def _read_org_activations(repo_root: Path) -> list[ActivationEntry]:
    ...
    """
    from charter.context import _iter_org_charter_docs  # noqa: PLC0415 — breaks a load-time cycle (see module docstring)
