...
``charter.context`` (single-owner, no-net-growth for that file). Three test
modules import these names directly from ``charter.context``
(``tests/charter/test_action_bundle_delivery.py``,
``tests/charter/test_context_display_charter_md.py``,
``tests/doctrine/drg/test_unknown_kind_fails_loudly.py``) -- ``charter.context``
