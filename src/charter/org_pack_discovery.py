...
charter-level :class:`~charter.schemas.DoctrineSelectionConfig` resolver that
...
``from charter.context import ...`` (breaking the load-time cycle a
top-level import would create, since ``charter.context`` imports this
module for its re-export shim). That precedent is unaffected by this
relocation — ``charter.context`` continues to re-export both names.
"""
...
def _missing_pack_diagnostic(repo_root: Path) -> str | None:
    ...
    :func:`~charter.context.build_charter_context` so the operator sees the
