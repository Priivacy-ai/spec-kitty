...
def _declared_action_labels(repo_root: Path) -> frozenset[str]:
    ...
    :data:`~charter.context.BOOTSTRAP_ACTIONS` constant so the fast-path set
...
def validate_local_support_declarations(
    ...
    *repo_root*, when supplied, feeds :func:`_declared_action_labels` -- FR-008's
    ...
    fast-path :data:`~charter.context.BOOTSTRAP_ACTIONS` constant. Omitting it
    ...
    # Cycle note: `charter.context` sits ABOVE `charter.interview` in the
    # existing import graph (`charter.context` -> `charter.language_scope` ->
    # `charter.interview`, both at module scope), so a module-level
    # `from charter.context import BOOTSTRAP_ACTIONS` here would import a
    # partially-initialized `charter.interview` back through that chain.
    # Function-local import breaks the cycle (established pattern -- see
    # `charter.resolver`'s lazy `CharterInterview` import).
    from charter.context import BOOTSTRAP_ACTIONS  # noqa: PLC0415
