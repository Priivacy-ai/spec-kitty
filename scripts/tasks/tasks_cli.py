...
def _derive_current_lane(mission_dir: Path, wp_id: str) -> str:
    ...
    events = read_events(mission_dir)  # raises StoreError on corrupt JSONL
    if not events:
        return "planned"
...
def stage_update(
    ...
    # Derive mission slug and WP ID for canonical event
    ...
    *,
    ...
    try:
        ...
        if not normalize_encoding:
            raise
        cleaned = normalize_mission_encoding(repo_root, mission)
        if cleaned:
...
def update_command(args: argparse.Namespace) -> None:
    ...
    if _check_legacy_format(mission, repo_root):
    ...
    wp = locate_work_package(repo_root, mission, args.work_package)
...
def history_command(args: argparse.Namespace) -> None:
    repo_root = find_repo_root()
    wp = locate_work_package(repo_root, args.mission, args.work_package)
    agent = args.agent or wp.agent or "system"
    shell_pid = args.shell_pid or wp.shell_pid or ""
...
def list_command(args: argparse.Namespace) -> None:
    repo_root = find_repo_root()
    mission_path = repo_root / "kitty-specs" / args.mission
    mission_tasks_dir = mission_path / "tasks"
    if not mission_tasks_dir.exists():
        raise TaskCliError(f"Mission '{args.mission}' has no tasks directory at {mission_tasks_dir}.")

    # Check for legacy format and warn
    use_legacy = is_legacy_format(mission_path)
    if use_legacy:
        _check_legacy_format(args.mission, repo_root)
    ...
    if use_legacy:
        ...
        for lane in LANES:
            lane_dir = mission_tasks_dir / lane
        ...
        for path in sorted(mission_tasks_dir.glob("*.md")):
            ...
            wp = WorkPackage(
                ...
                path=path,
                current_lane=lane,
                relative_subpath=path.relative_to(mission_tasks_dir),
                frontmatter=front,
                body=body,
                padding=padding,
            )
    ...
    if not rows:
        print(f"No work packages found for mission '{args.mission}'.")
        return
...
def _resolve_mission(repo_root: Path, requested: Optional[str]) -> str:
    if requested:
        return requested
...
def status_command(args: argparse.Namespace) -> None:
    repo_root = find_repo_root()
    mission = _resolve_mission(repo_root, args.mission)
    try:
...
def accept_command(args: argparse.Namespace) -> None:
    ...
    print(f"✅ Mission '{mission}' accepted at {result.accepted_at} by {result.accepted_by}")
...
def merge_command(args: argparse.Namespace) -> None:
    ...
    if args.mission:
        ...
        mission = current_branch
    else:
        mission = detect_mission_slug(find_repo_root(), cwd=repo_root)
    ...
    if current_branch != mission:
        raise TaskCliError(
            f"Current branch '{current_branch}' does not match detected mission '{mission}'."
    ...
    if args.dry_run:
        ...
        if args.strategy == "squash":
            steps.append(f"  - Merge {mission} with --squash and commit")
        elif args.strategy == "rebase":
            steps.append(
                f"  - Rebase {mission} onto {args.target} manually (command exits before merge)"
            )
        else:
            steps.append(f"  - Merge {mission} with --no-ff")
        if args.push:
        ...
        if args.delete_branch:
            steps.append(f"  - Delete branch {mission}")
        print("\n".join(steps))
        return
    ...
    if args.strategy == "rebase":
        raise TaskCliError(
            "Rebase strategy requires manual steps. Run `git checkout {mission}` followed by `git rebase {args.target}`."
        )
    ...
    if args.strategy == "squash":
        merge_proc = git(["merge", "--squash", mission], check=False)
        if merge_proc.returncode != 0:
            raise TaskCliError(
                "Merge failed. Resolve conflicts manually, commit, then rerun with --keep-worktree --keep-branch."
            )
        meta_path = _prepare_merge_metadata(primary_repo_root, mission, args.target, args.strategy, args.push)
        if meta_path:
            meta_rel = str(meta_path.relative_to(primary_repo_root))
            git(["add", meta_rel])
        git(["commit", "-m", f"Merge mission {mission}"])
    else:
        merge_proc = git(["merge", "--no-ff", "--no-commit", mission], check=False)
        if merge_proc.returncode != 0:
            raise TaskCliError(
                "Merge failed. Resolve conflicts manually, commit, then rerun with --keep-worktree --keep-branch."
            )
    ...
    if args.delete_branch:
        delete = git(["branch", "-d", mission], check=False)
        if delete.returncode != 0:
            git(["branch", "-D", mission])

    print(f"Merge complete: {mission} -> {args.target}")
def build_parser() -> argparse.ArgumentParser:
    ...
    update.add_argument("mission", help="Mission directory slug (e.g., 008-awesome-mission)")
    ...
    history.add_argument("mission", help="Mission directory slug")
    ...
    list_parser.add_argument("mission", help="Mission directory slug")

    rollback = subparsers.add_parser("rollback", help="Return a work package to its prior lane")
    rollback.add_argument("mission", help="Mission directory slug")
