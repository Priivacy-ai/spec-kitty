...
def detect_conflicting_wp_status(
    status_lines: List[str], mission: str, old_path: Path, new_path: Path
) -> List[str]:
    """Return staged work-package entries unrelated to the requested move."""
    base_path = Path("kitty-specs") / mission / "tasks"
...
def locate_work_package(repo_root: Path, mission: str, wp_id: str) -> WorkPackage:
    ...
    if not tasks_root.exists():
        raise TaskCliError(f"Mission '{mission}' has no tasks directory at {tasks_root}.")
    ...
    if not candidates:
        raise TaskCliError(f"Work package '{wp_id}' not found under kitty-specs/{mission}/tasks.")
    if len(candidates) > 1:
