...
Ceiling recalibration (Priivacy-ai/spec-kitty#2749, adjudicated 2026-08-18):
    The ceiling was raised from 1.05s to 1.60s (``ci_target_median_seconds`` in
    the baseline JSON) as the deliberate, documented recalibration #2749
    reserved. History: from 2026-07-17 the gate was FLAKY at the 1.05s boundary
    — with import-neutral changes it measured ~1.25s (FAIL) on one CI run and
    passed (<1.05s) on the next, because ``spec-kitty next`` cold-start sits at
    ~1.25-1.35s (CI) with runner-variance spikes, so the ceiling decided
    pass/fail on environment noise rather than on any real regression. The
    latency floor traces to the doctrine-as-SSOT / step-authority work (#2723
    "step.yaml single source; action_sequence/template_set projected", #2722):
    the ``step_projection.py`` runtime seam runs on every ``next`` (the cost is
    command execution, not imports — ``import specify_cli`` is ~50ms). The
    deeper optimization (caching step projection on the ``next`` path) remains
    #2853's territory; 1.60s gives ~20% headroom over the observed CI median
    while still catching a genuine step-change regression. See the baseline
    JSON's ``_ci_target_note`` / ``_known_regression_note`` for the measured
    evidence.
"""
