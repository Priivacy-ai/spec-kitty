...
retired ``src/doctrine/missions/`` *data* path.

Adversarial-squad finding M1 on PR #3453: WP02 (see
``test_claudemd_template_source.py``) corrected CLAUDE.md/AGENTS.md, but the
same stale ``src/doctrine/missions/mission-steps/...`` and
``src/doctrine/missions/<type>/...`` fragments survived in five active
doctrine surfaces under ``packs/**`` and ``src/doctrine/skills/**`` — an
...
``src/doctrine/missions/mission-steps/`` nor
``src/doctrine/missions/software-dev/`` exist on disk). The canonical data
...
every ``src/doctrine/missions/<remainder>`` occurrence found in scope is
resolved against the real filesystem: if ``src/doctrine/missions/<remainder>``
...
_REFERENCE_RE = re.compile(r"src/doctrine/missions/([\w\-{}<>./*]*)")
...
def _resolves_on_disk(remainder: str) -> bool:
    """True if src/doctrine/missions/<remainder> is a real path today."""
...
def test_no_stale_doctrine_missions_data_path_in_active_doctrine_prose() -> None:
    ...
    assert not violations, (
        "Stale src/doctrine/missions/ data references found in active doctrine "
