...
| specify_cli/core/mission_creation.py:374 | `create_mission_core` | `feature_dir = resolved_root / KITTY_SPECS_DIR / mission_slug_formatted` | mission_slug_formatted | raw-path-join | routed-through-resolver | `create_mission_core` joins `mission_slug_formatted`, the OUTPUT of the canonical `mission_dir_name` grammar seam (FR-032/FR-044) — not a raw operator slug; create-time-canonical (the dir is being created here). |
...
| specify_cli/retrospective/tracer_writer.py:177 | `_local_staging_path` | `staging_path : Path = repo_root / KITTY_SPECS_DIR / mission_slug / _TRACES_DIRNAME / filename` | mission_slug | raw-path-join | raw-bypass | `_local_staging_path` composes `KITTY_SPECS_DIR/mission_slug` directly — no resolver call — to compute the LOCAL staging path for a `traces/<category>.md` write; unlike the two existing `raw-bypass` rows (diagnostic-only, no FS sink) the caller (`append_tracer_finding`) DOES `.mkdir(parents=True, exist_ok=True)` + `.write_text(...)` this path. It is a write-side composition (WP03's `write_seam.write_artifact` then stages/commits the file onto the COORD branch), not a read-selection lookup — the read-side resolvers this audit polices (`resolve_status_surface_with_anchor` / `candidate_feature_dir_for_mission`) resolve where to READ FROM, not where a local write-then-stage file should land, so there is no canonical read-resolver to route through here. `mission_slug` carries no `assert_safe_path_segment` guard at this specific join (same deferred CLI-arg class as the untrusted-path-audit sibling row for this same line). Deferred (CLI-arg, low-risk local staging file, follow-up #2037). |
