...
| cli/commands/agent/workflow_cores.py:372 | has_prior_rejection | sub_artifact_dir = feature_dir / / wp_slug | wp_slug | Path-join (/) | trusted-source | `wp_slug` is the WP file stem (`wp.path.stem`, threaded in via `claim_result.wp_slug`) — a derived on-disk filename, not external input; `feature_dir / "tasks" / wp_slug` then `.exists()`/`.glob`. RELOCATED verbatim from `cli/commands/agent/workflow.py:921` (`_has_prior_rejection`) by mission coord-authority-trio-degod-01KX7094 WP02's trio decomposition: the function moved into `workflow_cores.py` and was renamed `_has_prior_rejection` → `has_prior_rejection` (module-private → module-public export), which changes the composite qualname and orphaned the old row. Same sink, same trusted provenance, disposition unchanged — reconciled here by WP05 cycle-1 closeout (the old `workflow.py:921` row is removed as a ghost). |
| cli/commands/agent/workflow_executor.py:868 | implement_try_render_fix_mode_prompt | sub_artifact_dir = feature_dir / / wp_slug | wp_slug | Path-join (/) | trusted-source | Same `wp.path.stem`-derived `wp_slug` (via `claim_result.wp_slug`) provenance as the sibling `sub_artifact_dir` reads; read-only `ReviewCycleArtifact.latest(...)` lookup for the fix-mode prompt. RELOCATED from the underscore-prefixed local `_sub_artifact_dir = feature_dir / "tasks" / wp_slug` that lived inline inside `workflow.py`'s pre-split monolithic `implement()` (formerly inventoried at `workflow.py:1621`, qualname `implement`) by mission coord-authority-trio-degod-01KX7094 WP02's trio decomposition, which extracted the block into `implement_try_render_fix_mode_prompt` and dropped the underscore on the local (`_sub_artifact_dir` → `sub_artifact_dir`) — same sink, same trusted provenance, disposition unchanged. Reconciled here by WP05 cycle-1 closeout (the old `workflow.py:1621` row is removed as a ghost). |
...
| review/arbiter.py:401 | _find_review_cycle_artifact | wp_dir = tasks_dir / wp_slug | wp_slug | Path-join (/) | unreachable | `_find_review_cycle_artifact`: `tasks_dir / wp_slug` used for `.exists()` only (`candidate = wp_dir / normalized`; `if candidate.exists(): return candidate`); never opens the raw join. (var `wp_subdir`/`wp_id` → `wp_dir`/`wp_slug` + line 388→401 by PR #2641 recover-missing-lane-workspaces switching the `review-cycle://` lookup to the `wp_slug` segment — read-only semantics unchanged.) |
...
| review/cycle.py:154 | _canonical_and_opposite_homes | candidate_feature_dir_for_mission ( repo_root , mission_slug… | mission_slug | Path-join (/) | routed-through-seam | `_canonical_and_opposite_homes`: `mission_slug` folded through `candidate_feature_dir_for_mission` (the shared write-seam resolver, propagates `MissionSelectorAmbiguous` — no silent pick, C-009) before `/ "tasks" / wp_slug`; the composed canonical home is consumed read-only by `_scan_home_for_compatibility` (`.exists()`/`.is_dir()`/`.glob("review-cycle-*.md")`), never opening the raw join. Seam present. (NEW row surfaced by PR #2641 recover-missing-lane-workspaces extracting the canonical/opposite review-cycle home resolution into `_canonical_and_opposite_homes`; supersedes the prior `resolve_review_cycle_pointer` `candidate_feature_dir_for_mission ( repo_root , parts.miss…` canonical-parse join.) |
| review/cycle.py:159 | _canonical_and_opposite_homes | primary_feature_dir_for_mission ( | mission_slug | Path-join (/) | routed-through-seam | `_canonical_and_opposite_homes`: the opposite-partition home folds `mission_slug` through `_canonicalize_primary_read_handle` (the canonical PRIMARY read fold, propagates `MissionSelectorAmbiguous` — no silent pick) before `primary_feature_dir_for_mission(...) / "tasks" / wp_slug`; consumed read-only by `_scan_home_for_compatibility` (`.exists()`/`.is_dir()`/`.glob`), never opening the raw join. Seam present. (NEW row surfaced by PR #2641 adding the opposite-home cross-partition compatibility scan; the FR-004 canonical-handle fold routes the handle through `_canonicalize_primary_read_handle` so the topology-blind `primary_feature_dir_for_mission` lands on the durable `<slug>-<mid8>` home.) |
...
| **total** | **33** | dispositioned rows in the sink table |

Of the 33 rows, 28 are AST-discovered (the live matcher prints
