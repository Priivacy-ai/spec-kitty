...
| migration/backfill_runtime_state.py:575 | backfill_runtime_state_repo | candidates = [ kitty_specs / mission_slug ] if ( kitty_specs… | mission_slug | Path-join (/) | routed-through-seam (TODO) | `backfill_runtime_state_repo`: `mission_slug` is a resolver-validated mission handle (Named-untrusted rule SC-003 forbids `trusted-source`), joined as `kitty_specs / mission_slug` and gated by an `.is_dir()` existence probe that returns `[]` on miss; the surviving directory is then handed to `backfill_runtime_state(feature_dir)` (a reachable write on `.is_dir()` success, so NOT `unreachable`). No `assert_safe_path_segment`/`safe_mission_slug` seam guards this library entrypoint yet — it is the deferred backfill library (#2684 runtime-state eviction); CLI wiring + seam routing tracked in #2816. Deferred (library param, existence-gated). |
...
| routed-through-seam (TODO) | 8 | 2 RESOLVED-at-source by WP02 (FR-009; matcher-per-site artifact) + 5 deferred CLI-arg low-risk (follow-up #2037) + 1 deferred backfill library param (#2684; CLI wiring #2816) |
...
| **total** | **34** | dispositioned rows in the sink table |
