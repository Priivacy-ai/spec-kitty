...
| cli/commands/agent/workflow.py:1603 | wp_slug | Path-join (/) | trusted-source | Same `wp.path.stem` provenance; read-only `ReviewCycleArtifact.latest(...)`. (line shifted −2 by mission 01KVJPEQ; then 1598→1603 by mission #2115 native-flow read-surface fix adding lines above — same join, disposition unchanged) |
| cli/commands/agent/workflow.py:2649 | wp_slug | Path-join (/) | trusted-source | `wp_slug = wp.path.stem`; parent now `primary_feature_dir_for_mission` (assert_safe_path_segment). Segment derived, dir guarded. (line shifted −2 by mission 01KVJPEQ; then 2640→2649 by mission #2115 native-flow read-surface fix adding lines above + the parent re-pointed to the primary seam — same join, disposition unchanged) |
...
| merge/done_bookkeeping.py:418 | mission_slug | Path-join (/) | unreachable | Builds a *relative* `kitty-specs/<slug>/status.events.jsonl` only to stringify into a `git show <ref>:<path>` argument; no local FS open — git resolves inside the tree object. (relocated VERBATIM from `cli/commands/merge.py:613` into the `_target_bookkeeping_status_paths` seam by mission #2057 decompose-merge-god-module-01KVXHDK WP08; then 419→418 by mission #2115 native-flow read-surface fix — same join, disposition unchanged) |
| merge/done_bookkeeping.py:420 | mission_slug | Path-join (/) | unreachable | Same git-ref-argument path as :418 (worktree-rewrite branch); no FS sink. (relocated VERBATIM from `cli/commands/merge.py:615` by mission #2057 WP08; then 421→420 by mission #2115 native-flow read-surface fix — same sink, disposition unchanged) |
