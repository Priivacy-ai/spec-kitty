...
| cli/commands/agent/tasks.py:1902 | mission_slug | Path-join (/) | routed-through-seam (TODO) | `worktree_kitty / mission_slug / "tasks"` `.exists()` probe from a raw `--mission` slug; reachable, unguarded. Deferred (CLI-arg, .exists() probe only). (line shifted −11 by mission 01KVJPEQ read-side adoption rebase; then +2 by mission 01KVMBD6 #2058 god-tag comment) |
...
| missions/_read_path_resolver.py:885 | mission_slug | Path-join (/) | routed-through-seam | `primary_feature_dir_for_mission` calls `assert_safe_path_segment(mission_slug)` immediately before `get_main_repo_root(repo_root) / KITTY_SPECS_DIR / mission_slug`. Seam present. (line shifted +130 by mission 01KVJPEQ; then +103 by mission 01KVN754 coord_feature_dir/probe_coord_state extraction; then +125 by 01KVN754 WP05 read-path EMPTY/DELETED fold + DELETED hard-fail block; then +16 by PR #2065 read_primary_meta canonicalize-on-miss fix) |
