...
| cli/commands/agent/tasks_move_task.py:1289 | _mt_resolve_gate_baseline | return BaselineTestResult . load ( st . feature_dir / / wp_s… | wp_slug | Path-join (/) | trusted-source | `st.feature_dir / "tasks" / wp_slug / "baseline-tests.json"`; identical provenance to the `_mt_run_pre_review_gate` row above — `wp_slug` from `_resolve_wp_slug(...)` (derived on-disk filename stem, not external input), `st.feature_dir` the trusted coord-write dir, and a read-only `BaselineTestResult.load()`. This is the shared baseline-resolution helper mission doctrine-controlled-transition-gates-01KY51Z7 (WP09, epic #2535 half A) extracted from `_mt_run_pre_review_gate` so the doctrine-bound handler context and the FR-004 override tier diff against the SAME baseline artifact — same disposition as its origin row. |
...
| --- | --- | --- |
| routed-through-seam | 12 | already safe (seam cited) |
...
| **total** | **35** | dispositioned rows in the sink table |
