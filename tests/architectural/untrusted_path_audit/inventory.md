...
| cli/commands/agent/tasks.py:1900 | mission_slug | Path-join (/) | routed-through-seam (TODO) | `worktree_kitty / mission_slug / "tasks"` `.exists()` probe from a raw `--mission` slug; reachable, unguarded. Deferred (CLI-arg, .exists() probe only). (line shifted −11 by mission 01KVJPEQ read-side adoption rebase) |
...
| cli/commands/merge.py:590 | mission_slug | Path-join (/) | unreachable | Builds a *relative* `kitty-specs/<slug>/status.events.jsonl` only to stringify into a `git show <ref>:<path>` argument; no local FS open — git resolves inside the tree object. (line shifted −1 by mission 01KVJPEQ read-side adoption rebase) |
| cli/commands/merge.py:592 | mission_slug | Path-join (/) | unreachable | Same git-ref-argument path as :590 (worktree-rewrite branch); no FS sink. (line shifted −1 by mission 01KVJPEQ) |
| cli/commands/merge.py:1107 | mission_slug | Path-join (/) | routed-through-seam (TODO) | `scan_specs / mission_slug / "meta.json"` then `read_text()` after `.exists()`; raw `--mission`/target slug, no seam. Deferred (CLI-arg, .exists()+read_text). (line shifted +5 by `_assert_status_surface_path_is_trusted` rename-comment, Op 01KVJK8R; then −1 by mission 01KVJPEQ read-side adoption rebase) |
...
| missions/_read_path_resolver.py:641 | mission_slug | Path-join (/) | routed-through-seam | `primary_feature_dir_for_mission` calls `assert_safe_path_segment(mission_slug)` (line 640) immediately before `get_main_repo_root(repo_root) / KITTY_SPECS_DIR / mission_slug`. Seam present. (line shifted +130 by mission 01KVJPEQ `resolve_handle_to_read_path` seam insertion) |
...
- **Deferred (CLI-arg, low-risk) → follow-up #2037:** `cli/commands/agent/mission.py:312`,
  `cli/commands/agent/tasks.py:1900`, `cli/commands/decision.py:464`,
  `cli/commands/merge.py:1107` — CLI-sourced (`--mission`) slugs with only
