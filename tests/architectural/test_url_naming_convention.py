...
ACTION_ALLOWLIST: dict[str, str] = {
    ...
    # SSE push-transport: /api/events/missions is a stream scoped to the missions
    # collection. Two noun segments without a {param} separator are intentional —
    # `events` names the transport mechanism, `missions` names the domain scope.
    "/api/events/missions": "GET: SSE push stream scoped to missions; transport-mechanism prefix acceptable for streaming endpoints",
}
