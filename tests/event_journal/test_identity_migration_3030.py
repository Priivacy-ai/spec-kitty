...
# The journal DDL exactly as it shipped before #3030 — 8 columns, no identity.
# Frozen here on purpose: regenerating it from today's constants would make the
# test vacuous the moment the constants change.
...
def test_pre_migration_journal_gains_the_identity_columns(tmp_path: Path) -> None:
    ...
    assert COL_PROJECT_UUID not in _columns(db_path)

    EventJournal(db_path)

    columns = _columns(db_path)
    assert COL_PROJECT_UUID in columns
    assert COL_PROJECT_SLUG in columns
    ...
    try:
        return list(
            conn.execute(
                ...
                f"coalesce_key, archived_at, drain_blocked_reason FROM {TABLE_NAME} "
                "ORDER BY event_id"
            )
        )
    finally:
...
def test_identity_columns_are_nullable(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert notnull[COL_PROJECT_UUID] == 0
        assert notnull[COL_PROJECT_SLUG] == 0
    finally:
        conn.close()
