...
def test_team_round_trips_verbatim(state_root: Path):
    credentials.store(repo="spec-kitty", relay_url="http://a", token="tok-a", token_kind="shared_team", team="demo")
    loaded = credentials.load(repo="spec-kitty")
    assert loaded is not None
...
def test_empty_team_is_rejected_when_provided(state_root: Path):
    with pytest.raises(ValueError):
        credentials.store(repo="spec-kitty", relay_url="http://a", token="tok-a", token_kind="shared_team", team="")
...
def test_stored_toml_omits_team_key_entirely_when_unset(state_root: Path):
    ...
    with credentials.credentials_path().open("rb") as fh:
        raw = tomllib.load(fh)
    assert "team" not in raw["spec-kitty"]
