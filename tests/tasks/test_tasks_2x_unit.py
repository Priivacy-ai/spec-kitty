...
class TestFindMissionSlug:
    ...
    def test_find_from_cwd_with_kitty_specs(self, mock_cwd: Mock, mock_root: Mock, tmp_path: Path):
        """Should extract mission slug from cwd containing kitty-specs."""
        from specify_cli.cli.commands.agent.tasks import _find_mission_slug

        # Setup: cwd is in kitty-specs/mission-slug directory
        ...
        with patch("specify_cli.core.mission_detection._get_main_repo_root") as mock_main:
            mock_main.return_value = tmp_path

            slug = _find_mission_slug()
    ...
    def test_find_from_git_branch(self, mock_cwd: Mock, mock_root: Mock, mock_subprocess: Mock, tmp_path: Path):
        """Should extract mission slug from git branch name."""
        from specify_cli.cli.commands.agent.tasks import _find_mission_slug
        ...
        mock_subprocess.return_value = Mock(
            returncode=0,
            stdout="008-test-mission\n"
        )

        # Create kitty-specs directory to validate the slug
        (tmp_path / "kitty-specs" / "008-test-mission").mkdir(parents=True)
