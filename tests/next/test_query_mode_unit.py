...
def _make_mock_decision(
    ...
    **overrides,
    ...
    from runtime.decisioning.decision import Decision, DecisionKind
...
class TestQueryModeDoesNotAdvance:
    def test_bare_call_invokes_query_not_decide(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("runtime.bridge.runtime_bridge.query_current_state", return_value=mock_decision) as mock_query,
    ...
    def test_answer_without_result_human_output_is_error_not_query(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("runtime.bridge.runtime_bridge.query_current_state", return_value=mock_decision),
        ):
            result = runner.invoke(
                cli_app,
...
class TestBuildPromptSafe:
    def test_build_prompt_safe_suppresses_stdout_noise(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        from runtime.decisioning.decision import _build_prompt_safe

        def noisy_build_prompt(**_kwargs):
            print("noisy stdout")
            return None, tmp_path / "prompt.md"

        with patch("runtime.prompts.builder.build_prompt", side_effect=noisy_build_prompt):
...
class TestQueryCurrentStateErrorPaths:
    ...
    def test_missing_feature_dir_returns_unknown_state(self, tmp_path: Path) -> None:
        ...
        from runtime.bridge.runtime_bridge import query_current_state
    ...
    def test_ephemeral_query_run_exception_raises_validation_error(self, tmp_path: Path) -> None:
        ...
        with (
            patch("runtime.bridge.runtime_bridge._existing_run_ref", return_value=None),
            patch("runtime.bridge.runtime_bridge._start_ephemeral_query_run", side_effect=RuntimeError("run init failed")),
            patch("runtime.bridge.runtime_bridge.get_mission_type", return_value="software-dev"),
            patch("runtime.bridge.runtime_bridge._compute_wp_progress", return_value=None),
        ):
    ...
    def test_read_snapshot_exception_raises_validation_error(self, tmp_path: Path) -> None:
        ...
        with (
            patch("runtime.bridge.runtime_bridge.get_or_start_run", return_value=mock_run_ref),
            patch("runtime.bridge.runtime_bridge.get_mission_type", return_value="software-dev"),
            patch("runtime.bridge.runtime_bridge._compute_wp_progress", return_value=None),
    ...
    def test_invalid_first_step_raises_clear_validation_error(self, tmp_path: Path) -> None:
        ...
        with (
            patch("runtime.bridge.runtime_bridge._existing_run_ref", return_value=mock_run_ref),
            patch("runtime.bridge.runtime_bridge.get_mission_type", return_value="software-dev"),
            patch("runtime.bridge.runtime_bridge._compute_wp_progress", return_value=None),
            patch("spec_kitty_runtime.engine._read_snapshot", return_value=snapshot),
            patch("runtime.bridge.runtime_bridge.load_mission_template_file", return_value=MagicMock()),
    ...
    def test_fresh_run_query_omits_ephemeral_run_id(self, tmp_path: Path) -> None:
        ...
        with (
            patch("runtime.bridge.runtime_bridge._existing_run_ref", return_value=None),
            patch(
                "runtime.bridge.runtime_bridge._start_ephemeral_query_run",
    ...
    def test_persisted_run_query_keeps_run_id(self, tmp_path: Path) -> None:
        ...
        with (
            patch("runtime.bridge.runtime_bridge._existing_run_ref", return_value=persisted_ref),
            patch("runtime.bridge.runtime_bridge.get_mission_type", return_value="software-dev"),
            patch("runtime.bridge.runtime_bridge._compute_wp_progress", return_value=None),
            patch("spec_kitty_runtime.engine._read_snapshot", return_value=snapshot),
            patch("runtime.bridge.runtime_bridge.load_mission_template_file", return_value=MagicMock()),
    ...
    def test_existing_run_ref_returns_none_when_state_json_missing(self, tmp_path: Path) -> None:
        from runtime.bridge.runtime_bridge import _existing_run_ref
        ...
        with patch("runtime.bridge.runtime_bridge._load_feature_runs", return_value=index):
    ...
    def test_start_ephemeral_query_run_cleans_up_on_bootstrap_failure(self, tmp_path: Path) -> None:
        ...
        from runtime.bridge.runtime_bridge import _start_ephemeral_query_run
        ...
        with (
            patch("runtime.bridge.runtime_bridge.tempfile.mkdtemp", side_effect=tracking_mkdtemp),
            patch("runtime.bridge.runtime_bridge._runtime_template_key", return_value="software-dev"),
            patch("runtime.bridge.runtime_bridge._build_discovery_context", return_value=None),
            patch("runtime.bridge.runtime_bridge.start_mission_run", side_effect=RuntimeError("template missing")),
...
class TestQueryModeErrorOutput:
    def test_json_query_validation_failure_returns_error_document(self, tmp_path: Path) -> None:
        from runtime.bridge.runtime_bridge import QueryModeValidationError

        with (
            patch("specify_cli.cli.commands.next_cmd.locate_project_root", return_value=tmp_path),
            patch("specify_cli.cli.commands.next_cmd.resolve_selector", return_value=SimpleNamespace(canonical_value="069-test")),
            patch(
                "runtime.bridge.runtime_bridge.query_current_state",
...
class TestResultSuccessStillAdvances:
    def test_result_success_calls_decide_not_query(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("runtime.bridge.runtime_bridge.query_current_state") as mock_query,
