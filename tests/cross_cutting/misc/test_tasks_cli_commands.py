...
def test_update_and_rollback(feature_repo: Path, mission_slug: str) -> None:
    """Test updating WP lane and rolling back."""
    result = run_tasks_cli(["update", mission_slug, "WP01", "doing"], cwd=feature_repo)
    assert_success(result)
    run(["git", "commit", "-am", "Update to doing"], cwd=feature_repo)

    updated_wp = locate_work_package(feature_repo, mission_slug, "WP01")
    ...
    rollback_result = run_tasks_cli(["rollback", mission_slug, "WP01", "--force"], cwd=feature_repo)
    assert_success(rollback_result)

    rolled_wp = locate_work_package(feature_repo, mission_slug, "WP01")
    assert rolled_wp.current_lane == "planned"


def test_update_modifies_metadata_only(feature_repo: Path, mission_slug: str) -> None:
...
def test_list_command_output(feature_repo: Path, mission_slug: str) -> None:
    result = run_tasks_cli(["list", mission_slug], cwd=feature_repo)
    assert_success(result)
    assert "Lane" in result.stdout
    assert "planned" in result.stdout


def test_history_appends_entry(feature_repo: Path, mission_slug: str) -> None:
    ...
    assert_success(result)
    wp = locate_work_package(feature_repo, mission_slug, "WP01")
    assert "Follow-up" in wp.body


def test_acceptance_commands(feature_repo: Path, mission_slug: str) -> None:
...
def test_accept_command_encoding_error_without_normalize(feature_repo: Path, mission_slug: str) -> None:
    _prepare_done_work_package(feature_repo, mission_slug)
...
def test_accept_command_with_normalize_flag(feature_repo: Path, mission_slug: str) -> None:
    _prepare_done_work_package(feature_repo, mission_slug)
...
def test_scenario_replay(feature_repo: Path, mission_slug: str) -> None:
    ...
    summary = run_tasks_cli(["status", "--feature", mission_slug, "--json"], cwd=feature_repo)
...
def test_exact_wp_id_matching_not_prefix(feature_repo: Path, mission_slug: str) -> None:
    ...
    write_wp(feature_repo, mission_slug, "planned", "WP04")
    write_wp(feature_repo, mission_slug, "planned", "WP04b")
    run(["git", "add", "."], cwd=feature_repo)
    run(["git", "commit", "-m", "Add WP04 and WP04b"], cwd=feature_repo)

    # Update WP04
    result = run_tasks_cli(["update", mission_slug, "WP04", "doing", "--force"], cwd=feature_repo)
    assert_success(result)

    # Verify WP04 is now canonically in progress
    wp04 = locate_work_package(feature_repo, mission_slug, "WP04")
    assert wp04.current_lane == "in_progress", "WP04 should be in progress"

    # Verify WP04b is still "planned"
    wp04b = locate_work_package(feature_repo, mission_slug, "WP04b")
    assert wp04b.current_lane == "planned", "WP04b should still be in planned"


def test_exact_wp_id_matching_with_slug(feature_repo: Path, mission_slug: str) -> None:
    ...
    tasks_dir = feature_repo / "kitty-specs" / mission_slug / "tasks"
    write_wp(feature_repo, mission_slug, "planned", "WP04")
...
def test_update_stages_changes(feature_repo: Path, mission_slug: str) -> None:
    """Test: Update command stages the changes for commit."""
    result = run_tasks_cli(["update", mission_slug, "WP01", "doing", "--force"], cwd=feature_repo)
    assert_success(result)
...
def test_update_ignores_other_wp_modifications(feature_repo: Path, mission_slug: str) -> None:
    ...
    wp02_path = feature_repo / "kitty-specs" / mission_slug / "tasks" / "WP02.md"
    ...
    # Verify WP01 updated
    wp01 = locate_work_package(feature_repo, mission_slug, "WP01")
...
def test_update_with_staged_other_wp_changes(feature_repo: Path, mission_slug: str) -> None:
...
def test_sequential_updates_by_different_agents(feature_repo: Path, mission_slug: str) -> None:
    ...
    result_a = run_tasks_cli(["update", mission_slug, "WP01", "doing", "--force"], cwd=feature_repo)
    assert_success(result_a)

    # Agent B updates WP02 (without committing WP01 update first - simulates race)
    result_b = run_tasks_cli(["update", mission_slug, "WP02", "doing", "--force"], cwd=feature_repo)
    assert_success(result_b)

    # Both should be in doing now
    wp01 = locate_work_package(feature_repo, mission_slug, "WP01")
    wp02 = locate_work_package(feature_repo, mission_slug, "WP02")
