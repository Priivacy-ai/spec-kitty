...
def test_update_and_rollback(mission_repo: Path, mission_slug: str) -> None:
    """Test updating WP lane and rolling back."""
    result = run_tasks_cli(["update", mission_slug, "WP01", "doing"], cwd=mission_repo)
    assert_success(result)
    run(["git", "commit", "-am", "Update to doing"], cwd=mission_repo)

    updated_wp = locate_work_package(mission_repo, mission_slug, "WP01")
    ...
    rollback_result = run_tasks_cli(["rollback", mission_slug, "WP01", "--force"], cwd=mission_repo)
    assert_success(rollback_result)

    rolled_wp = locate_work_package(mission_repo, mission_slug, "WP01")
    assert rolled_wp.current_lane == "planned"


def test_update_modifies_frontmatter_only(mission_repo: Path, mission_slug: str) -> None:
...
def test_list_command_output(mission_repo: Path, mission_slug: str) -> None:
    result = run_tasks_cli(["list", mission_slug], cwd=mission_repo)
    assert_success(result)
    assert "Lane" in result.stdout
    assert "planned" in result.stdout


def test_history_appends_entry(mission_repo: Path, mission_slug: str) -> None:
    ...
    assert_success(result)
    wp = locate_work_package(mission_repo, mission_slug, "WP01")
    assert "Follow-up" in wp.body


def test_acceptance_commands(mission_repo: Path, mission_slug: str) -> None:
    ...
    run(["git", "commit", "-am", "Update to done"], cwd=mission_repo)

    status = run_tasks_cli(["status", "--mission", mission_slug, "--json"], cwd=mission_repo)
    assert_success(status)
    data = json.loads(status.stdout)
    assert data["mission_slug"] == mission_slug

    verify = run_tasks_cli(["verify", "--mission", mission_slug, "--json", "--lenient"], cwd=mission_repo)
    ...
    assert accept_payload.get("mission_slug") == mission_slug


def _prepare_done_work_package(mission_repo: Path, mission_slug: str) -> None:
    run_tasks_cli(["update", mission_slug, "WP01", "doing", "--force"], cwd=mission_repo)
    run(["git", "commit", "-am", "Update to doing"], cwd=mission_repo)
    run_tasks_cli(["update", mission_slug, "WP01", "done", "--force"], cwd=mission_repo)
    run(["git", "commit", "-am", "Update to done"], cwd=mission_repo)
...
def test_accept_command_encoding_error_without_normalize(mission_repo: Path, mission_slug: str) -> None:
    _prepare_done_work_package(mission_repo, mission_slug)
...
def test_accept_command_with_normalize_flag(mission_repo: Path, mission_slug: str) -> None:
    _prepare_done_work_package(mission_repo, mission_slug)
...
def test_scenario_replay(mission_repo: Path, mission_slug: str) -> None:
    ...
    run(["git", "commit", "-am", "Add history"], cwd=mission_repo)
    run_tasks_cli(["update", mission_slug, "WP01", "for_review", "--force"], cwd=mission_repo)
    run(["git", "commit", "-am", "Update to review"], cwd=mission_repo)
    run_tasks_cli(["update", mission_slug, "WP01", "done", "--force"], cwd=mission_repo)

    summary = run_tasks_cli(["status", "--mission", mission_slug, "--json"], cwd=mission_repo)
...
def test_merge_command_requires_clean_tree(merge_repo: tuple[Path, Path, str]) -> None:
    ...
    branches = run(["git", "branch"], cwd=repo_root)
    assert mission in branches.stdout
...
def test_packaged_copy_behaves_like_primary(temp_repo: Path) -> None:
    ...
    # Seed minimal mission in project path using helper (flat structure).
    mission = "002-packaged"
    write_wp(project_path, mission, "planned", "WP01")
    result = subprocess.run(
        [sys.executable, str(embedded_cli), "list", mission],
        cwd=project_path,
        text=True,
        capture_output=True,
    )
...
def test_exact_wp_id_matching_not_prefix(mission_repo: Path, mission_slug: str) -> None:
    ...
    run(["git", "add", "."], cwd=mission_repo)
    run(["git", "commit", "-m", "Add WP04 and WP04b"], cwd=mission_repo)

    # Update WP04
    result = run_tasks_cli(["update", mission_slug, "WP04", "doing", "--force"], cwd=mission_repo)
    assert_success(result)

    # Verify WP04 is now "doing"
    wp04 = locate_work_package(mission_repo, mission_slug, "WP04")
    assert wp04.current_lane == "doing", "WP04 should be in doing"

    # Verify WP04b is still "planned"
    wp04b = locate_work_package(mission_repo, mission_slug, "WP04b")
    assert wp04b.current_lane == "planned", "WP04b should still be in planned"


def test_exact_wp_id_matching_with_slug(mission_repo: Path, mission_slug: str) -> None:
    """Test: WP04 matches WP04-slug.md but not WP04b-slug.md.

    GIVEN: WP04-mission.md and WP04b-other.md exist
    WHEN: Updating WP04
    THEN: Only WP04-mission.md should change
    """
    # Create WP files with slugs (flat structure)
    tasks_dir = mission_repo / "kitty-specs" / mission_slug / "tasks"
    write_wp(mission_repo, mission_slug, "planned", "WP04")
    # Rename to have a slug
    (tasks_dir / "WP04.md").rename(tasks_dir / "WP04-mission-name.md")

    write_wp(mission_repo, mission_slug, "planned", "WP04b")
    (tasks_dir / "WP04b.md").rename(tasks_dir / "WP04b-other-mission.md")

    run(["git", "add", "."], cwd=mission_repo)
    run(["git", "commit", "-m", "Add slugged WP files"], cwd=mission_repo)
    ...
    wp04 = locate_work_package(mission_repo, mission_slug, "WP04")
    assert wp04.current_lane == "doing", "WP04-mission-name.md should be in doing"
...
def test_update_stages_changes(mission_repo: Path, mission_slug: str) -> None:
    """Test: Update command stages the changes for commit."""
    result = run_tasks_cli(["update", mission_slug, "WP01", "doing", "--force"], cwd=mission_repo)
    assert_success(result)
...
def test_update_ignores_other_wp_modifications(mission_repo: Path, mission_slug: str) -> None:
    ...
    """
    # Create WP02
    write_wp(mission_repo, mission_slug, "planned", "WP02")
    run(["git", "add", "."], cwd=mission_repo)
    run(["git", "commit", "-m", "Add WP02"], cwd=mission_repo)

    # Modify WP02 (simulating another agent editing it)
    wp02_path = mission_repo / "kitty-specs" / mission_slug / "tasks" / "WP02.md"
    ...
    # Verify WP01 updated
    wp01 = locate_work_package(mission_repo, mission_slug, "WP01")
...
def test_update_with_staged_other_wp_changes(mission_repo: Path, mission_slug: str) -> None:
    ...
    original_content = wp02_path.read_text(encoding="utf-8")
    wp02_path.write_text(original_content + "\n<!-- Agent B staged WP02 -->\n", encoding="utf-8")
    run(["git", "add", str(wp02_path.relative_to(mission_repo))], cwd=mission_repo)
...
def test_sequential_updates_by_different_agents(mission_repo: Path, mission_slug: str) -> None:
    ...
    result_a = run_tasks_cli(["update", mission_slug, "WP01", "doing", "--force"], cwd=mission_repo)
    assert_success(result_a)

    # Agent B updates WP02 (without committing WP01 update first - simulates race)
    result_b = run_tasks_cli(["update", mission_slug, "WP02", "doing", "--force"], cwd=mission_repo)
    assert_success(result_b)

    # Both should be in doing now
    wp01 = locate_work_package(mission_repo, mission_slug, "WP01")
    wp02 = locate_work_package(mission_repo, mission_slug, "WP02")
