...
def test_locate_work_package(feature_repo: Path, mission_slug: str) -> None:
    th.ensure_lane("planned")  # smoke call
    path = th.locate_work_package(feature_repo, mission_slug, "WP01")
    assert path.work_package_id == "WP01"
    assert path.current_lane == "planned"


def test_locate_work_package_exact_match(feature_repo: Path, mission_slug: str) -> None:
    ...
    wp = th.locate_work_package(feature_repo, mission_slug, "WP04")
    assert wp.work_package_id == "WP04"

    # Looking for WP04b should NOT match WP04
    wp_b = th.locate_work_package(feature_repo, mission_slug, "WP04b")
