...
def test_resolution_behavior_diverges_between_pinned_pr_and_latest_full() -> None:
    ...
    assert "full" in run_text and "pr" in run_text, (
        "the resolution step must branch explicitly on mode: pr vs full"
    )
    assert "latest" in run_text.lower() or "pypi" in run_text.lower(), (
        "full mode must resolve the latest spec_kitty_events mainline"
    )
    assert "pinned-events-rev" in run_text or "pinned_events_rev" in run_text, (
        "pr mode must resolve the recorded pinned rev input, not a moving ref"
    )
...
def test_no_bare_upload_artifact_used_for_the_env_cache() -> None:
    ...
    assert "upload-artifact" not in text, (
        "research D2: cache semantics require actions/cache, not a bare "
        "upload-artifact (which conflates cache semantics)"
    )
...
def test_cache_key_derives_from_uv_lock_and_the_resolved_events_rev() -> None:
    ...
    key_source_steps = [
        s
        for s in steps
        if "cache" in str(s.get("id", "")).lower() and "key" in str(s.get("id", "")).lower()
    ]
    ...
    assert "resolved-events-rev" in run_text or "resolved_events_rev" in run_text, (
        "cache key must be derived from the resolved spec_kitty_events rev"
    )
    assert str(key_step.get("id", "")) in key_expr or "cache-key" in key_expr, (
        "the actions/cache key must consume the computed cache-key step output"
    )
...
def test_every_uses_step_is_pinned_to_a_full_commit_sha() -> None:
    ...
    for uses in uses_steps:
        ...
        assert ref and FULL_SHA_RE.match(ref), (
            f"'{uses}' is not pinned to a full 40-character commit SHA (DIR-051)"
        )
...
def test_editable_install_runs_exactly_once_gated_on_cache_miss() -> None:
    ...
    build_steps = [
        s
        for s in steps
        if "uses" not in s and "uv sync" in str(s.get("run", ""))
    ]
    assert build_steps, "expected exactly one build step invoking `uv sync`"
    assert len(build_steps) == 1, (
        "the editable install must run in exactly one step — "
        "a single pre-warm reused by all downstream shards"
    )
...
def test_composite_exposes_the_built_environment_for_downstream_reuse() -> None:
    ...
    assert any("venv" in name or "env" in name for name in outputs), (
        "expected an output exposing the built environment path"
    )
    assert any("cache-hit" in name or "cache_hit" in name for name in outputs), (
        "expected a cache-hit output so callers can observe single-build-then-reuse"
    )
