...
REGISTRY: dict[str, RegisteredSite] = {
    ...
    # `datetime.now(UTC)` -> `now_utc()`.
    #
    # HISTORICAL (#273): drift_detector.py itself is removed by #116's
    # orphaned-collateral adjudication (no viable low-risk wiring target).
    # This key is kept as the kernel.clock migration's byte-identity record
    # for that now-deleted call site -- the producer below reproduces the
    # prior expression directly and never imports the module, so the entry
    # stays valid whether or not the module exists.
