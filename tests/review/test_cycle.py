...
# NOTE: ``test_self_referential_feedback_source_is_rejected`` and
# ``test_new_cycle_body_never_duplicates_a_prior_cycle_file`` (#2996(b),
# RED-FIRST P0 reproductions) were extracted to
# ``tests/regression/test_issue_2996_self_referential_and_duplicate_feedback.py``
...
# P0 reproduction cannot share a file with the passing ``review.cycle`` unit
# tests above). ``MISSION_SLUG`` / ``WP_ID`` / ``WP_SLUG`` and ``_init_repo``
# were local to those two tests only and moved with them.
