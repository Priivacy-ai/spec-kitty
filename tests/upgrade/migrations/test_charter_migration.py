"""Tests for migration m_2_0_0_charter_directory."""
...
class TestCharterDirectoryMigration:
    """Test the charter directory migration."""
    ...
    def test_detect_only_flags_legacy_path(self, tmp_path: Path):
        ...
        canonical = tmp_path / ".kittify" / "charter" / "charter.md"
    ...
    def test_scenario_1_old_exists_new_doesnt(self, tmp_path: Path):
        ...
        # Setup
        old_path = tmp_path / ".kittify" / "memory" / "charter.md"
        old_path.parent.mkdir(parents=True)
        old_path.write_text("# Project Charter\n\nTest content")

        # Run migration
        migration = Migration()
        with patch("specify_cli.charter.sync.sync") as mock_sync:
        ...
        assert new_path.read_text() == "# Project Charter\n\nTest content"
    ...
    def test_scenario_4_neither_exists(self, tmp_path: Path):
        ...
        # Verify changes reported
        assert any("No charter found" in change for change in changes)
    ...
    def test_idempotency(self, tmp_path: Path):
        ...
        migration = Migration()
        with patch("specify_cli.charter.sync.sync"):
    ...
    def test_creates_new_directory(self, tmp_path: Path):
        """Migration should create .kittify/charter/ directory."""
        # Setup
        old_path = tmp_path / ".kittify" / "memory" / "charter.md"
        old_path.parent.mkdir(parents=True)
        old_path.write_text("# Test")
        ...
        new_dir = tmp_path / ".kittify" / "charter"
    ...
    def test_preserves_memory_directory(self, tmp_path: Path):
        ...
        charter = memory_dir / "charter.md"
        charter.write_text("# Test")
        ...
        assert not charter.exists()  # Charter moved
    ...
    def test_relative_paths_in_changes(self, tmp_path: Path):
        ...
        assert any(
            ".kittify/memory/charter.md" in change and ".kittify/charter/charter.md" in change
