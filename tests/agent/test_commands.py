...
def test_specify_command_delegates_to_agent_lifecycle(monkeypatch) -> None:
    ...
    def fake_create_mission(mission_name: str, mission_type=None, json_output: bool = False):
...
def test_accept_checklist_json_output(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(accept_module, "detect_mission_slug", lambda _repo_root: "001-demo-mission")
...
def test_accept_json_suppresses_fallback_announcement(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_detect(_repo_root):
        return "002-auto-mission"

    monkeypatch.setattr(accept_module, "find_repo_root", lambda: repo_root)
    monkeypatch.setattr(accept_module, "detect_mission_slug", fake_detect)
    monkeypatch.setattr(accept_module, "choose_mode", lambda mode, _repo_root: mode)
    monkeypatch.setattr(accept_module, "collect_mission_summary", lambda *args, **kwargs: DummySummary())
    ...
    assert data["mission"] == "002-auto-mission"
