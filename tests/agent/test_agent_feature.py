"""Unit tests for agent mission CLI commands."""
...
class TestCreateMissionCommand:
    ...
    @patch("specify_cli.cli.commands.agent.mission.is_worktree_context", return_value=False)
    @patch("specify_cli.cli.commands.agent.mission.locate_project_root")
    @patch("specify_cli.cli.commands.agent.mission.is_git_repo")
    @patch("specify_cli.cli.commands.agent.mission.get_current_branch")
    @patch("specify_cli.cli.commands.agent.mission.get_next_mission_number")
    ...
    @patch("specify_cli.cli.commands.agent.mission.is_worktree_context")
    @patch("specify_cli.cli.commands.agent.mission.locate_project_root")
    ...
    def test_blocks_create_mission_from_worktree_with_main_repo_hint(
        ...
        assert "spec-kitty agent mission create-mission test-mission" in result.stdout
...
class TestCheckPrerequisitesCommand:
    ...
    @patch("specify_cli.cli.commands.agent.mission.validate_mission_structure")
    ...
    def test_paths_only_flag_json(
        ...
        assert output["MISSION_SPEC"] == paths["spec_file"]
...
class TestGitPreflightEnforcement:
    """Tests for _enforce_git_preflight integration in mission commands."""
...
class TestFindMissionDirectory:
    ...
    @patch("specify_cli.cli.commands.agent.mission.is_worktree_context")
    @patch("specify_cli.cli.commands.agent.mission.subprocess.run")
    def test_finds_mission_in_worktree_by_branch_name(
        self, mock_subprocess: Mock, mock_is_worktree: Mock, tmp_path: Path
    ):
        """Should find mission directory matching branch name in worktree."""
        # Setup
        from specify_cli.cli.commands.agent.mission import _find_mission_directory

        mock_is_worktree.return_value = True
        mock_subprocess.return_value = Mock(returncode=0, stdout="001-test-mission\n")

        # Create worktree structure
        kitty_specs = tmp_path / "kitty-specs"
        kitty_specs.mkdir()
        (kitty_specs / "001-test-mission").mkdir()
        (kitty_specs / "002-other-mission").mkdir()

        # Execute
        result = _find_mission_directory(tmp_path, tmp_path)

        # Verify
        assert result == kitty_specs / "001-test-mission"

    @patch("specify_cli.cli.commands.agent.mission.is_worktree_context")
    def test_multiple_missions_require_explicit_selection(
        self, mock_is_worktree: Mock, tmp_path: Path
    ):
        """Mutating mission commands should not auto-select among multiple missions."""
        from specify_cli.cli.commands.agent.mission import _find_mission_directory

        mock_is_worktree.return_value = False

        # Create main repo structure with multiple missions
        kitty_specs = tmp_path / "kitty-specs"
        kitty_specs.mkdir()
        for slug, lane in (
            ("001-mission", "done"),
        ...
        with pytest.raises(ValueError, match="Please specify explicitly"):
            _find_mission_directory(tmp_path, tmp_path)

    @patch("specify_cli.cli.commands.agent.mission.is_worktree_context")
    def test_raises_error_when_no_missions_in_main_repo(
        self, mock_is_worktree: Mock, tmp_path: Path
    ):
        """Should raise error when no missions exist in main repo."""
        # Setup
        from specify_cli.cli.commands.agent.mission import _find_mission_directory
        ...
        with pytest.raises(ValueError, match="No missions found"):
            _find_mission_directory(tmp_path, tmp_path)
    ...
    def test_falls_back_when_branch_name_unavailable(
        ...
        """Should fallback to any mission when git command fails."""
        # Setup
        from specify_cli.cli.commands.agent.mission import _find_mission_directory
        ...
        # Verify - should still find the mission via fallback
        assert result == kitty_specs / "001-test-mission"
    ...
    def test_raises_error_when_kitty_specs_not_found_in_worktree(
        ...
        with pytest.raises(ValueError, match="Mission directory not found"):
            _find_mission_directory(tmp_path, tmp_path / "nested")
