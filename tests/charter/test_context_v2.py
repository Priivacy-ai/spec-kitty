"""Tests for build_context_v2 -- DRG-based charter context (T021 + T022)."""
...
from charter.context import CharterContextResult, build_context_v2
...
def _setup_fixture_repo(tmp_path: Path) -> None:
    """Create a minimal repo layout for build_context_v2 testing."""
...
# ---------------------------------------------------------------------------
# T021: build_context_v2 functional tests
# ---------------------------------------------------------------------------
...
class TestNoPerActionFiltering:
    ...
    build_context_v2 must not contain if-statements that branch on
    ...
    def test_no_action_name_string_literals_in_function_body(self) -> None:
        """No action-name string literals in build_context_v2 body.
        ...
        """
        source = inspect.getsource(build_context_v2)
        tree = ast.parse(textwrap.dedent(source))

        # Find the function definition
        func_def = None
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name == "build_context_v2":
                func_def = node
                break

        assert func_def is not None, "Could not find build_context_v2 function"
        ...
        assert not found_literals, (
            f"build_context_v2 contains action-name string literals: "
