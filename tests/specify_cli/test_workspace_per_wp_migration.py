...
def test_specify_template_updated():
    ...
    if ".worktrees" in content_lower:
        ...
        for line in lines_with_worktrees:
            ...
            is_negated = any(neg in line_lower for neg in ["no worktree", "not create", "don't create", "does not create"])
    ...
    assert "main" in content.lower(), \
        "specify.md doesn't mention main repository/branch"

    # Should mention commit workflow
    assert "commit" in content.lower(), \
        "specify.md doesn't mention committing artifacts"
    ...
    if "worktree" in content_lower:
        ...
        for line in lines_with_worktree:
            ...
            is_negated = any(neg in line_lower for neg in [
                "not in", "not a", "no worktree", "not from", "instead of",
                "not use", "don't use", "does not", "without worktree"
            ])
...
def test_tasks_template_updated():
    ...
    assert "dependencies" in content_lower, \
    ...
    assert "implement" in content_lower, \
        "tasks.md doesn't mention implement command"

    # SHOULD mention --base flag for dependencies
    assert "--base" in tasks_template.read_text(), \
        "tasks.md doesn't document --base flag for dependent WPs"
...
def test_implement_template_exists():
    ...
    assert implement_template.exists(), \
        "implement.md template not found (should be NEW file in 0.11.0)"
