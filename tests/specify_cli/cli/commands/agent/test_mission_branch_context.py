...
def test_branch_context_reports_invoking_linked_worktree_branch(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """#3124 family: current branch comes from the invoking checkout, not primary."""
    primary = tmp_path / "primary"
    primary.mkdir()
    subprocess.run(["git", "init", "-q", "-b", "main"], cwd=primary, check=True)
    subprocess.run(
        ["git", "-c", "user.name=Test", "-c", "user.email=t@example.invalid", "commit", "--allow-empty", "-qm", "seed"],
        cwd=primary,
        check=True,
    )
