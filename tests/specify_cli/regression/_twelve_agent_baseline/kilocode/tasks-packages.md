...
# /spec-kitty.tasks-packages - Generate Work Package<!-- glossary:glossary:work-package --> Files

**Version**: 3.2.0

## Purpose

Generate individual `tasks/WP<!-- glossary:glossary:wp -->*.md` prompt files from the manifest in `wps.yaml`.
...
0. `## ⚡ Do This First: Load Agent Profile` — **REQUIRED. Must be the first section after the H1 title, before Objective.** Instructs the implementing agent to load the assigned profile via `/ad-hoc-profile-load` before reading anything else. Use this exact structure, substituting frontmatter values:
   ...
   Use the `/ad-hoc-profile-load` skill<!-- glossary:glossary:skill --> to load the agent profile specified in the frontmatter, and behave according to its guidance before parsing the rest of this prompt.
...
After all WP files are written and `wps.yaml` is updated, review all available doctrine<!-- glossary:glossary:doctrine -->-provided and user-created agent profiles and assign the most relevant profile to each work package.
...
> This degraded fallback can diverge because organization/project<!-- glossary:glossary:project --> overlays,
