...
- **Security checks**: Reference the `051-supply-chain-install-safety` directive<!-- glossary:glossary:directive --> and the `supply-chain-install-safety` tactic<!-- glossary:glossary:tactic -->. Planning output (Technical Context and/or `research.md`) must surface registry authenticity, package freshness, lifecycle-script discipline (deny-by-default `preinstall`/`install`/`postinstall`), and Node Active LTS awareness for any dependency decision — this mirrors the `supply_chain_security_check` step already present in the `plan` step contract<!-- glossary:glossary:step-contract -->.
