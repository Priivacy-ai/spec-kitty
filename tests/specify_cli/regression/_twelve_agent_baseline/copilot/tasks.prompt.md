...
- Understand the full scope<!-- glossary:glossary:scope --> deeply
...
```bash
# Run from project<!-- glossary:glossary:project --> root (same directory as /spec-kitty.plan):
# You should already be here if you just ran /spec-kitty.plan

# Creates:
# - kitty-specs/<mission_slug<!-- glossary:glossary:mission_slug -->>/tasks/WP01-*.md → In repository root checkout
...
# - Commits ALL to target branch<!-- glossary:glossary:target-branch -->
# - NO worktrees created
```

**Do NOT cd anywhere**. Stay in the repository root checkout.

**Worktrees created later**: After tasks are finalized, run your agent loop: `spec-kitty next --agent <agent> --mission<!-- glossary:glossary:mission --> <handle>`. Your agent will call `spec-kitty agent action implement WP<!-- glossary:glossary:wp -->## --agent <name>` for each WP. `finalize_tasks` computes the execution lanes, and each lane<!-- glossary:glossary:lane --> gets exactly one worktree.

**In repos with multiple missions, always pass `--mission <handle>` to every spec-kitty command.** The `<handle>` can be the mission's `mission_id<!-- glossary:glossary:mission_id -->` (ULID), `mid8<!-- glossary:glossary:mid8 -->` (first 8 chars of the ULID), or `mission_slug`. The resolver disambiguates by `mission_id` and returns a structured `MISSION_AMBIGUOUS_SELECTOR` error on ambiguity — there is no silent fallback.
...
- `current_branch`
- `target_branch`
- `planning_base_branch<!-- glossary:glossary:planning_base_branch -->`
- `merge_target_branch<!-- glossary:glossary:merge_target_branch -->`
...
8. **Finalize tasks with dependency parsing and commit**:
   ...
   **CRITICAL**: Run this preflight command from repo root:
   ...
   - `lanes.json` won't be available to resolve the real workspace<!-- glossary:glossary:workspace --> path/branch for each WP
...
- Phase<!-- glossary:glossary:phase --> grouping: Phase 2 WPs typically depend on Phase 1
...
title: "Build<!-- glossary:glossary:build --> API"
...
- WP01: Foundation Setup (5 subtasks, ~300 lines)
  ...
  - T002: Set up migration<!-- glossary:glossary:migration --> system
...
Resolve the feature slug<!-- glossary:glossary:feature-slug --> from explicit user direction, current branch<!-- glossary:glossary:current-branch -->, or current directory path.
...
After `finalize-tasks` completes, review all available doctrine<!-- glossary:glossary:doctrine -->-provided and user-created agent profiles and assign the most relevant profile to each work package.
...
> Only a read-only harness<!-- glossary:glossary:harness --> that cannot invoke the CLI may inspect profiles under
...
> `specializes_from` lineage, and `enhances<!-- glossary:glossary:enhances -->`/`overrides<!-- glossary:glossary:overrides -->` semantics are not applied;
