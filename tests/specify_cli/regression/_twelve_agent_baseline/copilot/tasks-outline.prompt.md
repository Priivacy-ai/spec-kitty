...
# /spec-kitty.tasks-outline - Create Work Package<!-- glossary:glossary:work-package --> Manifest

**Version**: 3.2.0

## Purpose

Create `wps.yaml` — the structured work package manifest that defines WP<!-- glossary:glossary:wp --> metadata,
...
**QUALITY OVER SPEED**: Take your time to understand the full scope<!-- glossary:glossary:scope --> deeply,
...
Treat that JSON as canonical for feature slug<!-- glossary:glossary:feature-slug -->, feature directory, and target branch<!-- glossary:glossary:target-branch -->.
...
- If the WP is cross-cutting infrastructure with no specific concern (e.g. a test
  harness<!-- glossary:glossary:harness --> setup WP), set `cross_cutting: true` and leave `plan_concern_refs` empty.
...
- Phase<!-- glossary:glossary:phase --> grouping: Phase 2 WPs typically depend on Phase 1 WPs
