...
- **Code Review Checklist** — from `.kittify/charter/charter.md` — your
  primary gate set when judging the WP<!-- glossary:glossary:wp -->.
- **Regression Vigilance** — from `.kittify/charter/charter.md` — the
  project<!-- glossary:glossary:project -->'s explicit drift guard; apply when the diff renames or introduces
...
- Every `DIRECTIVE_NNN` declared in the loaded reviewer profile's
  `directive<!-- glossary:glossary:directive -->-references` list (for example, `reviewer-renata` cites
  `DIRECTIVE_032` — Conceptual Alignment).
- Every tactic<!-- glossary:glossary:tactic -->-id declared in the loaded reviewer profile's
...
If the diff adds, upgrades, or removes a dependency (any ecosystem), verify the implementer's evidence trail against the `supply-chain-install-safety` tactic and the `051-supply-chain-install-safety` directive — this mirrors the `supply_chain_security_review` step already present in the `review` step contract<!-- glossary:glossary:step-contract -->:
...
7. **Shared-file ownership**: any file modified by this WP that is also
   modified by another WP, visible in `lanes.json`, shared lane<!-- glossary:glossary:lane --> metadata, or
...
Move the WP forward:
```bash
spec-kitty agent status emit WPxx --to approved<!-- glossary:glossary:approved --> --actor <name> --mission <handle>
```
