...
# /spec-kitty.charter<!-- glossary:glossary:charter --> - Interview + Compile Charter
...
## Repo Scope<!-- glossary:glossary:scope -->

`/spec-kitty.charter` is a repo-scoped governance flow, not a per-mission<!-- glossary:glossary:mission --> flow.

- Run it from the repository<!-- glossary:glossary:repository --> root.
...
## Skill<!-- glossary:glossary:skill --> Load

Before proceeding, load the `spec-kitty-charter-doctrine<!-- glossary:glossary:doctrine -->` skill for the charter
...
generation. Action context<!-- glossary:glossary:action-context --> is loaded iteratively at the actual action boundary.

## Doctrine Gaps

If no built-in paradigm or directive<!-- glossary:glossary:directive --> cleanly fits the user's needs:

- Do **not** force a near-match just to populate selections.
- Keep `selected_paradigms` and `selected_directives` narrow and truthful.
- Encode the missing policy as project<!-- glossary:glossary:project -->-specific charter policy in the interview
...
1. Inspect the repo quickly before asking questions. Prefer high-signal files:
   ...
   - If the repo is otherwise sparse, say so plainly. A freshly initialized
     Spec Kitty workspace<!-- glossary:glossary:workspace --> with mostly `.kittify/` metadata is a greenfield
