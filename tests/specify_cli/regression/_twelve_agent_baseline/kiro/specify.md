...
- Default planning/base branch if you create the mission right now: `planning_base_branch<!-- glossary:glossary:planning_base_branch -->`
...
2. **Check for explicit mission requests** in the user's description:
   ...
   - If user mentions "build<!-- glossary:glossary:build -->", "implement", "create feature" → use software-dev
...
Store the final mission selection in your notes and include it in the spec output. Do not pass a `--mission-type` flag to mission creation unless the user explicitly overrides<!-- glossary:glossary:overrides --> the default.
