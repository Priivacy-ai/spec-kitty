...
- `"requirement_extraction_warnings"` — soft signal: requirement-shaped tokens in
  `spec.md` that matched none of the recognized declared shapes (see below)
...
**Soft signal — `requirement_extraction_warnings` (exit code stays 0):**

If `requirement_extraction_warnings` is non-empty, `spec.md` mentions one or
more requirement-shaped tokens (`FR-###`, `NFR-###`, `C-###`) that did not
match any recognized declaring shape — a Requirements table row, a heading
naming the id, a bulleted/numbered list item, or a bold id leading a
paragraph. Those tokens are invisible to FR-coverage mapping even though the
finalize command still reports full coverage. This is **non-blocking** — it
does not fail the command or gate implementation. Read each warning: if the
tokens are this mission's own requirements, rewrite them in one of the
recognized shapes so they are picked up; if they are citations of another
mission's requirement, no action is needed.
