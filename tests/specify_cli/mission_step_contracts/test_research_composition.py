...
def _action_index_path(action: str) -> Path:
    ...
    return _repo_root() / "src" / "doctrine" / "missions" / "research" / "actions" / action / "index.yaml"
...
def test_all_research_contracts_load(action: str) -> None:
    ...
    assert contract is not None, (
        f"Missing shipped contract for research/{action}; expected a file at src/doctrine/mission_step_contracts/shipped/research-{action}.step-contract.yaml"
    )
