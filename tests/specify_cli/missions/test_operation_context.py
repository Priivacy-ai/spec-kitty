...
def test_explicit_root_does_not_expand_candidates_through_cwd(tmp_path: Path) -> None:
    ...
    context = resolve_mission_operation_context(repo, "demo-mission", cwd=caller, explicit_root=True)
...
def test_managed_lane_remains_a_split_brain_conflict_probe(tmp_path: Path) -> None:
    ...
    assert {candidate.mission_id for candidate in exc_info.value.candidates} == {MISSION_A, MISSION_B}
    assert {candidate.root for candidate in exc_info.value.candidates} == {repo.resolve(), managed.resolve()}
...
def test_split_brain_fails_closed(tmp_path: Path, selector: str) -> None:
    repo = _init_repo(tmp_path / "repo")
    caller = _add_worktree(repo, tmp_path / "caller-owned", "codex/caller")
    _write_mission(repo, "demo-mission", MISSION_A)
    _write_mission(caller, "demo-mission", MISSION_B)
    ...
    assert {candidate.mission_id for candidate in error.candidates} == {MISSION_A, MISSION_B}
    ...
    assert all(set(candidate) == {"root", "mission_id", "mission_slug"} for candidate in payload["candidates"])


def test_resolution_is_deterministic_across_repeated_calls(tmp_path: Path) -> None:
    repo = _init_repo(tmp_path / "repo")
    caller = _add_worktree(repo, tmp_path / "caller-owned", "codex/caller")
    _write_mission(caller, "demo-mission", MISSION_A)

    contexts = [resolve_mission_operation_context(repo, "demo-mission", cwd=caller) for _ in range(20)]
