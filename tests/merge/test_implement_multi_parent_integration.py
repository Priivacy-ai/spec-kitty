...
1. Create mission with WPs having multi-parent dependencies
...
@pytest.fixture
def mission_repo(tmp_path: Path) -> Path:
    """Create a repository with mission structure and multi-parent dependencies."""
    ...
    mission_dir = repo / "kitty-specs" / "010-multi-parent-test"
    mission_dir.mkdir(parents=True)

    # Create meta.json
    meta = {
        "mission_number": "010",
        "mission_slug": "010-multi-parent-test",
        "vcs": "git",
...
"""
    ...
    # Commit mission structure to main
    subprocess.run(["git", "add", "."], cwd=repo, check=True, capture_output=True)
    subprocess.run(
        ["git", "commit", "-m", "Add mission structure"],
        cwd=repo,
        check=True,
        capture_output=True,
    )
...
def test_implement_linear_dependency_chain(mission_repo: Path, monkeypatch):
    """Test implementing WP01 → WP02 → WP03 (linear chain, no auto-merge)."""
    monkeypatch.chdir(mission_repo)
    ...
    assert (mission_repo / ".worktrees" / "010-multi-parent-test-WP01").exists()

    # Verify frontmatter updated with base_branch
    wp01_file = mission_repo / "kitty-specs" / "010-multi-parent-test" / "tasks" / "WP01-database.md"
    ...
    ctx01 = load_context(mission_repo, "010-multi-parent-test-WP01")
    ...
    wp01_workspace = mission_repo / ".worktrees" / "010-multi-parent-test-WP01"
    ...
    wp02_file = mission_repo / "kitty-specs" / "010-multi-parent-test" / "tasks" / "WP02-user-api.md"
    ...
    wp02_workspace = mission_repo / ".worktrees" / "010-multi-parent-test-WP02"
    assert (wp02_workspace / "database.sql").exists()


def test_implement_multi_parent_auto_merge(mission_repo: Path, monkeypatch):
    ...
    wp03_workspace = mission_repo / ".worktrees" / "010-multi-parent-test-WP03"
    ...
    wp04_workspace = mission_repo / ".worktrees" / "010-multi-parent-test-WP04"
    assert wp04_workspace.exists()

    # Verify WP04 frontmatter has merge base branch
    wp04_file = mission_repo / "kitty-specs" / "010-multi-parent-test" / "tasks" / "WP04-admin-dashboard.md"
    ...
    ctx04 = load_context(mission_repo, "010-multi-parent-test-WP04")
...
def test_implement_multi_parent_with_conflicts(mission_repo: Path, monkeypatch):
