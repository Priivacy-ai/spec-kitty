...
class TestFeaturesRoute:
    """``GET /api/features`` is retired; must return 410 Gone."""

    def test_returns_410_gone(self, project_dir: Path) -> None:
        with _client(project_dir) as client:
        ...
        assert payload.get("error") == "endpoint_retired"
        assert payload.get("successor") == "/api/missions"


class TestKanbanRoute:
    """``GET /api/kanban/{feature_id}`` is retired; must return 410 Gone."""
    ...
    def test_returns_410_gone(self, project_dir: Path) -> None:
        ...
        payload = response.json()
        assert payload.get("error") == "endpoint_retired"
        assert payload.get("successor") == f"/api/missions/{SAMPLE_MISSION_SLUG}/status"
