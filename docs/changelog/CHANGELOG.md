...
- **`spec-kitty specify` reported success while creating a mission that was broken on arrival, on any repository with no commits yet (`#4033`; the `#3988` beginner report).** **Before:** git cannot create a branch in a repository whose HEAD is unborn, so on a freshly `git init`-ed project every coordination-bearing mission wrote a `coordination_branch` into `meta.json` naming a ref that was never minted — and creation ran to completion and exited 0 anyway. Following Getting Started on a brand-new project therefore produced missions that only `spec-kitty doctor coordination --fix` could recover, with no error at any point to act on; `doctor coordination` then diagnosed the damage correctly and also exited 0. **After:** mission creation refuses up front with a non-zero exit and names the remedy (`git commit --allow-empty -m 'Initial commit'`), before any scaffold is written — so a refused create leaves nothing behind to clean up. **Scope:** the refusal applies only to coordination-bearing topologies (`coord`, `lanes_with_coord`), which are the shapes that mint a branch; `single_branch` and `lanes` missions never needed one and are still created normally on an unborn HEAD. **Operator note:** the root cause is the unborn HEAD, not the protected-branch commit guard that `#4033` originally named — repositories with at least one commit were never affected.
