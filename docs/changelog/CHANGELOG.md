...
- **`spec-kitty upgrade` no longer leaves a freshly created root `AGENTS.md` / `GEMINI.md` dirty after its own auto-commit, and its commit-set parser no longer names the *old* path of a staged rename.** (#2491 follow-up; salvaged from #2492, superseded by #3679) **Before:** the upgrade finalizer (#3679) now runs surface repair *before* the single churn commit, but `is_upgrade_commit_eligible` still rejected every root-level file except `.gitignore` — so when surface repair auto-created a missing `AGENTS.md` (codex/opencode/antigravity/pi/vibe/letta) or `GEMINI.md` (gemini) as a `REPAIRABLE_REQUIRED` session-presence surface, the run ended with that file modified-but-uncommitted, the exact state the #2392 "every upgrade write ends in a commit" invariant forbids. Separately, `git_status_paths` read `git status --porcelain -z` rename entries with the fields backwards (real git emits `R  <new>\0<old>\0`), keeping the *old* name — its unit-test fixture encoded the same wrong order, so the test passed against the bug. **After:** a small, explicit root allowlist (`.gitignore`, `AGENTS.md`, `GEMINI.md`) — pinned by a test against the session-presence writer registry so a future root-level harness file names the missing entry — and rename parsing that keeps the destination, covered by a real-git regression test. Arbitrary operator-owned root files (`README.md`, `CLAUDE.md`, …) stay excluded, and the pre-run baseline still keeps pre-existing operator edits to the allowlisted files out of the commit.
