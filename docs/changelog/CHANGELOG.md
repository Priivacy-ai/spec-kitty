...
- **A workspace-allocation failure during `spec-kitty implement` trapped the work package in an unrecoverable `blocked` state, and clearing it demanded a fabricated review rejection (`#3937`).** **Before:** when allocation hit a dependency-lane or planning-commit merge conflict, the error handler manufactured a `planned → blocked` transition even though the work package was still `planned` and the allocator had already self-cleaned; `blocked → planned` is illegal, and the only documented escape (`move-task --to planned`) demanded a `--review-feedback-file` ("cannot be bypassed with `--force`") *before* checking transition legality — so a tooling-caused block could only be cleared by inventing a fake review finding, and the subsequent claim then ran in "Fix mode" as if a review had rejected it. The illegal-transition error also never named the legal targets. **After:** an allocation failure leaves the work package `planned` (no manufactured `blocked`) and prints the conflict's actionable resolution step, so a plain re-run works once the cause is resolved — matching the orchestrator-api path, which never emitted `blocked`. The `move-task` review-feedback demand now fires only for genuine review-family (run-affecting) rollbacks, so an illegal `blocked → planned` falls through to an honest single-stage refusal that enumerates the legal targets (`{in_progress, canceled}`), and no fabricated review artifact can launder a structurally illegal transition. The `cannot start implementation` reject names the legal recovery command. The machine-readable FSM parity fixtures are unchanged.
