...
- **A concurrent writer's status events could be destroyed by a workflow rollback truncate (`#3960`; mission-review DRIFT-2 off `#3922`).** **Before:** the two rollback truncates the `fsm-write-path-integrity` mission left outside its pipeline — the coordination fallback arm (`coordination/status_transition.py`) and its workflow-command twin (`cli/commands/agent/workflow.py`) — cut `status.events.jsonl` back to a pre-emit byte size without verifying the tail they were about to remove, so any row another writer appended between the snapshot and the rollback was silently lost. **After:** both route through one status-owned, lock-held, tail-verified helper (`specify_cli.status.rollback.rollback_events_log_tail`): it re-acquires the same per-mission `feature_status_lock` the write pipeline uses, compares the tail's event ids against a post-emit capture, and only then truncates through the new store primitive `truncate_events_log`. A tail that does not verify — foreign rows, a torn row, or a log that was rewritten in the window — is refused and left intact: stranding one already-emitted row is recoverable, destroying another writer's durable events is not. The two writes-gate ledger entries that tolerated the old blind truncates are removed, so a reintroduced lockless truncate now fails the architectural gate. The `#4072` fix round hardens the ownership record further: the workflow commit paths now capture their expected ids inside `owned_emission_window` — one lock-held window spanning the pre-emit snapshot, the emits and the capture, so a concurrent writer can never be adopted into the rollback's "expected" set — and a refused rollback now also preserves the derived `status.json` at its newer coherent state (never restoring obsolete snapshot bytes over a preserved log) while surfacing an explicit recoverable diagnostic; a log that vanishes after the pre-emit snapshot is refused rather than recreated NUL-filled. The `#4087` fix round closes the last seam: both artifacts now roll back inside ONE lock-held window (`specify_cli.status.rollback.rollback_status_artifacts`) — the rollback decision, the truncation (or its refusal/no-op) and the derived-snapshot restore share a single `feature_status_lock` hold, so a separately locked writer that appends, canonically materializes and Git-commits both artifacts can only land after the whole window, never between a successful tail cut and the snapshot restore (the schedule that left the working snapshot incoherent with both the committed snapshot and canonical replay).
