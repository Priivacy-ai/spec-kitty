...
- **`spec-kitty init` no longer gitignores all of `.cursor/` for teams that version-control their own Cursor rules.** (#2498) `GitignoreManager.protect_all_agents()` wrote a blanket `.cursor/` entry for every project, regardless of which agents were selected. **Before:** a repo that tracked `.cursor/rules/contributing.mdc` (or any other file under `.cursor/`) alongside Spec Kitty's own `.cursor/rules/spec-kitty.mdc` found the whole directory gitignored after `init`, making the team's own rules unstageable. **After:** the `cursor` entry in `AGENT_DIRECTORIES` is narrowed to only the paths Spec Kitty itself generates (`.cursor/rules/spec-kitty.mdc`, `.cursor/commands/`, `.cursor/skills/`), matching the existing `.github/copilot/` precedent; `spec-kitty upgrade` on an already-initialised project removes the stale blanket line (preserving any narrower, non-blocking `.cursor/...` patterns) and backfills the three narrow entries. `protect_selected_agents()` also had a latent bug where an agent name owning more than one registry entry silently kept only the last one — fixed alongside, since narrowing `cursor` to three rows would otherwise have surfaced it. The `3.2.6rc3_narrow_cursor_gitignore` migration removes every blanket form git treats as equivalent (`.cursor`, `.cursor/`, `/.cursor/`, `.cursor/*`, `.cursor/**`, `**/.cursor/`, …), preserves negations (`!.cursor/…`), narrower subpaths and comments, only closes the blank-line hole at the removal site, and creates `.gitignore` if absent instead of aborting the upgrade. `.cursor/skills/` stays ignored as a *declared* Spec Kitty skill root (`AGENT_SKILL_CONFIG`, not yet populated by the installer); `.cursor/hooks.json` is intentionally **not** ignored (team-owned; only written on explicit `agent config set lint_on_edit`).
