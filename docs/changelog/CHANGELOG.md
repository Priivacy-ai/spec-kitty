...
- **Legacy PR-bound missions no longer try to finalize planning artifacts onto protected `main`.** (#2938) `finalize-tasks` now recognizes only the legacy shape that conflated a protected final target with a non-protected invoking feature branch, persists that feature branch as `target_branch`, and keeps the protected destination separately as `merge_target_branch`. Linked-worktree branch context is resolved through the canonical checkout-identity seam, while `--validate-only` remains byte- and ref-stable.
