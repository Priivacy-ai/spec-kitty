...
- **`spec-kitty --version` / `-v` output is now a single copyable line (mission `copyable-version-output-01KW6N9V`).** The version callback no longer renders the ASCII-art banner before the version string — the first line of output is `spec-kitty-cli version X.Y.Z`, ready to paste into an issue report. Community contribution by @zohar.
