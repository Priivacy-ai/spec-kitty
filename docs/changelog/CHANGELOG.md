...
_The 3.2.7rc2 candidate cycle is open. Entries land here as missions merge._
...
- **`spec-kitty --version` and `-v` now include the exact 12-character source revision, so copied issue reports distinguish builds that share a semantic version (`#2268`).** Release wheels and source distributions carry the revision captured at build time, while editable source checkouts report their live Git revision; environments without either source report `build unknown` explicitly.
