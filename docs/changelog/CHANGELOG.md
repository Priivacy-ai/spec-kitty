...
- **The root command listing is alphabetically ordered, and bare `spec-kitty` now shows it (mission `alphabetical-command-listing-01KW6N8W`).** Root commands and command groups are sorted by displayed name in `--help` output, and invoking `spec-kitty` without a subcommand prints the full (sorted) help instead of only the banner and a "run --help" hint — the command surface is scannable at first contact. Community contribution by @zohar.
