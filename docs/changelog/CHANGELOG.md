...
- **Typecheck is not the test runner (DIRECTIVE_030).** Implement/review skills, the software-dev WP template, Frontend Freddy / Node Norris self-review, and the quality-gate-verification tactic now require the project's compiler typecheck (`npm run typecheck`, `tsc -b`, or `tsc --noEmit`) when TypeScript changes. Vitest/Jest/pytest passing is not that gate — they do not apply `noUnusedLocals`.
