...
- **Mission specifications can now receive an optional, explicitly consented external review through a local OpenCode CLI.** `spec-kitty spec-review` previews a digest-bound disclosure manifest before any external call, refuses routes that cannot currently prove zero cost, performs at most one model request per consent, and stores a separate advisory result without editing the specification or changing Mission lifecycle state. The requested route label does not establish provider ownership, availability, retention, privacy, or actual model identity; see [Review a Mission specification with an external model](../guides/spec-review.md).
