...
- **A malformed `drg/fragment.yaml` in one org pack of a multi-pack chain no longer drops the *other* packs' fragments.** (mission `doctrine-drg-silent-drop-boundary`; #3629 second-opinion follow-up) The mission-step executor tolerated a malformed *optional* org fragment by degrading it, but did so for the whole chain at once — one bad pack silently evicted its healthy siblings' fragments too. **After:** the degrade is **per-pack** — only the offending pack's fragment is skipped, with an operator-visible `WARNING` naming it, and the healthy siblings still compose. `strict` callers (the diagnostic APIs and the fail-loud composition path) are unchanged and still raise on any malformed pack.
