...
- **`spec-kitty upgrade`'s auto-commit no longer names the *old* path of a staged rename.** (#2491 follow-up; salvaged from #2492) `git_status_paths` read `git status --porcelain -z` rename entries with the fields backwards — real git emits `R  <new>\0<old>\0` — so a staged rename put the pre-rename path into the commit-set. The unit-test fixture encoded the same wrong order, so the test passed against the bug. Fixed to keep the destination; covered by a corrected fixture, an off-by-one test, and a real-git `git mv` regression test.
