...
- **`spec-kitty dashboard` on Windows printed success for a dashboard that silently wasn't there (`#4125`; field-verified on spec-kitty-cli 3.2.6.2 during a Windows training session).** **Before:** the dashboard server was spawned as a detached `python -c "<script>"` child, and `python -c` leaves `__main__` without a `__file__` attribute — a Windows-platform transitive import in the server import chain reads it, so the child died on import with `AttributeError: module '__main__' has no attribute '__file__'` before ever binding its port. Its stdout/stderr were both sent to `DEVNULL`, so the crash was completely invisible: the CLI reported a started dashboard that then wasn't there. **After:** the child is spawned as `python -m specify_cli.dashboard._server_main <project_dir> <port> [<token>]` — `-m` gives `__main__` a real `__file__`, and it is the stdlib-blessed way to spawn package code, so no platform bootstrap can trip on it (this is the field workaround from the training session — launching the identical server code from a real `.py` file — productized). The child's output is now piped to the spec-kitty runtime root's `dashboard-server.log` (`~/.spec-kitty/dashboard-server.log` on POSIX, `%LOCALAPPDATA%\spec-kitty\dashboard-server.log` on Windows; mirroring the old sync daemon's log pattern) instead of DEVNULL, the child detaches via `DETACHED_PROCESS` on Windows (where `start_new_session` is inert), and `start_dashboard` no longer returns on a doomed child: a readiness probe polls until the port serves *this project's* dashboard identity over `/api/health` (resolved `project_path`, plus the token when one was handed to the child — the same contract the lifecycle health check enforces), so a foreign dashboard that won the same free-port race can never stand in for the spawn; if the child exits first, the probe raises `DashboardSpawnError` (`DASHBOARD_SPAWN_FAILED`) carrying the child's exit status and log tail, so the CLI prints the actual failure and exits 1 instead of a success line. The spawn regression is now pinned end-to-end by a test that really spawns the detached child and asserts the port serves the project's health payload — including a `windows_ci` twin for the native Windows lane where the crash reproduced.
