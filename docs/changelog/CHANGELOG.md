...
- **The glossary skill now pressure-tests a contested or load-bearing domain-model term before recording it, instead of accepting the first plausible definition (`#3835`).** **Before:** `spec-kitty-glossary-context` curated terminology and checked drift, but had no step that challenged a proposed domain-model term against the actual code or a concrete edge case, and no guidance on when a term decision warranted an ADR — so a shaky model claim could be written into the glossary as if it were settled. **After:** a new "Pressure-Test Domain Model Terms" step cross-checks the claim against the real domain types, API contracts, and tests (labelling it a hypothesis when code evidence is missing), challenges it with at least one concrete edge scenario, and raises an ADR only when the decision is hard to reverse, surprising without context, and a genuine trade-off. Two triggers ("shape a domain model", "validate domain language against code") route that work in, and `spk-doctrine-glossary` now hands contested-term and domain-model work to the detailed workflow. The step points at the canonical `domain-aware-decision-interview` procedure and `adr-drafting-workflow` tactic it summarises. Instruction-only, and it skips cleanly for an ordinary canonical usage correction.
