...
- **`mission_runtime`'s root surface shrinks from 42 to 31 names and `ActionContext` is gone** (mission `dead-port-disposition-01M1TZVN`, WP03). Thirteen test-only re-exports (`ArtifactPlacementFragment`, `BranchRefFragment`, `IdentityFragment`, `MissionArtifactContext`, `StatusSurfaceFragment`, `WorkspaceFragment`, `MissionArtifactHome`, `artifact_home_for`, `ResolvedSurface`, `SurfaceLocations`, `translate_surface`, `content_present_at_primary_tip`, `get_packs_root_default`) are still defined at their modules but no longer re-exported from `mission_runtime`; the `ActionContext = MissionExecutionContext` alias and the `specify_cli.team_projection` tombstone package are deleted. `tests/architectural/test_mission_runtime_surface.py` now asserts the exact public surface.
...
- **Dependency gating for `claimed`/`in_progress` now resolves readiness once, in-lock, with a tri-state verdict** (mission `fsm-write-path-integrity-01M1TZV6`): a work package whose declared dependencies are not all `approved`/`done` is refused at claim and implement time with the blocking WP ids named; an *unresolvable* readiness (missing WP prompt file, unreadable dependency graph) fails **open** and is logged rather than blocking. The verdict lives in `specify_cli.status.dependency_verdict`.
...
- **The dead `MissionOrchestration`/`MissionStateObject`/`MissionTransition` models and the required `Mission.orchestration` field are removed from the charter offering schema** (mission `dead-port-disposition-01M1TZVN`, WP03 rider T014b); `src/charter/offering/schemas/mission.schema.yaml` is regenerated without the `orchestration` definition, and the matching inert-slot baseline rows are retired. Nothing constructed these models; the built-in packs' `states:`/`transitions:` blocks that once matched them were deleted with the DSL runtime.
- **`runtime.next.decision` no longer carries the legacy DSL readers `derive_mission_state` and `evaluate_guards`** (mission `dead-port-disposition-01M1TZVN`, WP04); `specify_cli.mission_v1.read_events` is no longer re-exported from the package root (it stays on `specify_cli.mission_v1.events`).
...
- **The mission-DSL v1 runtime and its `transitions` dependency are removed** (mission `dead-port-disposition-01M1TZVN`, WP01). `specify_cli.mission_v1.{compat,runner,guards,schema}` — the `transitions`-backed state machine, guard compiler, JSON-Schema validator and v0 wrapper — had no production caller; the surviving module `specify_cli.mission_v1.events` (`emit_event` / `read_events`, used by the `spec-kitty next` runtime loop) stays in place unchanged, and the package now re-exports exactly those two names. The orphaned `states:`/`transitions:` blocks were deleted from the three built-in `mission.yaml` packs (`software-dev`, `plan`, `research`); the keys stay tolerated in third-party and `.kittify/overrides` `mission.yaml` files via `MISSION_COMPAT_IGNORED_FIELDS`. `transitions` (and the `six` it dragged in) no longer load when a mission event is emitted; the subprocess ratchet `tests/specify_cli/mission_v1/test_import_hygiene.py` pins that invariant permanently. No version bump (the rc cycle is already open).
