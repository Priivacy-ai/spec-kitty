...
- **HTTPS doctrine bundle fetches now persist ETags separately from pack
  versions and can use explicit Artifactory `version` metadata (`#3165`).**
  Unchanged downloads reuse a validated source-matching cache validator,
  while missing or invalid Artifactory version metadata fails before archive
