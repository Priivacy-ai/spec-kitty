...
- **`spec-kitty routes` is the successor to the removed `spec-kitty sync routes` (Priivacy-ai/spec-kitty#10).** With the sync transport gone, the question `sync routes` answered ("where does my data go, which teams see it") is replaced by the ephemeral-team-status question: **which team admits this repository, and which relay do its moments reach** — or, equally valid, no team admits it, so the checkout produces nothing anywhere. `spec-kitty routes` reports exactly that, read from the same `zeitgeist_client.resolution` seam every status transition already uses (offline cache peek first — positive credential or remembered negative answers instantly without requiring auth — then a network mint only on a genuine miss); it reports honestly which of the three states it is in rather than dressing silence up as "not admitted".
- **`spec-kitty events tail` streams a mission's event log as JSON lines, so an external consumer (a dashboard, a CI watcher, a native driver) can follow status changes live and resume exactly where it left off (#3858; closes #3841).** _(Moved here from the 3.2.6 section: this landed 2026-09-05, after the 2026-09-03 `v3.2.6` tag, and belongs to 3.2.7rc1.)_ **Before:** there was no supported way to observe a mission's `status.events.jsonl` incrementally — a consumer had to re-read the whole file and diff it, and because that log is _not_ append-only (it is truncated in place on a coordination rollback) a naive tailer silently desynced when the file shrank or was rewritten. **After:** `spec-kitty events tail --mission <slug> --json` emits each event as a JSON line, injecting a `tail_offset` + `tail_invariant` cursor a consumer persists and supplies back with `--from-offset`/`--from-invariant` to resume; a mismatched resume cursor is **refused** (structured stderr, non-zero exit — never a silent reset). All three truncation shapes — a mid-append tear, a clean record-boundary shrink, and a truncate-then-regrow within one poll — are detected (size check + SHA-256 content invariant, evaluated independently every poll), so the stream stays correct across rollbacks. `--once` and `--max-events N` bound the stream for scripted use. See [CLI reference § `spec-kitty events`](../api/cli-commands.md#spec-kitty-events).
