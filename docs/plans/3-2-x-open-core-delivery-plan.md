...
> **Related domain plans (durable throughlines):** this plan is the release-scoped, PO-facing view. Its "SaaS" scope is the open-core doctrine/charter split (see the [Doctrine & Charter — Domain Plan](doctrine-charter-domain-plan.md)); the hosted product surface and its P0 cluster live in the [SaaS & Hosted Sync — Domain Plan](saas-hosted-sync-domain-plan.md), which this plan's remaining-work table does not cover.
