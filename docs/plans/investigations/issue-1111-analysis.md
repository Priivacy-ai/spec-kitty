...
| #522 | [ADR-8] Monorepo / cross-repo charter visibility | **NOT started** — ADR-8 is referenced in Mission A's spec (FR-025 org-charter, FR-029) as a future design. No implementation landed. The `charter.activation._drg_helpers._resolve_org_root` stub is *intentionally inert* (per the architecture review); org-root resolution lives in `specify_cli.doctrine.config.resolve_org_roots` and the cross-repo extension point is not yet wired. |
