...
**Since census (2026-07-05): mission `ci-topology-shrink-01KWQAVX` SHIPPED (all 6 WPs done)** — a concrete instance of #1931's standing campsite epic folding into a mission (see dependency spine, above). It de-serializes the always-on `arch-adversarial` guard off the `fast-tests-core-misc` critical path (100% of `src/`, no `needs` edge — path collapses `sum`→`max` toward the ≤13.6-min ceiling), folds the 32 previously-unmapped `src/specify_cli` dirs into six composite `dorny/paths-filter` groups, splits `fast-tests-core-misc` into two disjoint matrix shards, and fixes the pre-existing `mission-loader-coverage` C-005 coverage-drop (silently absent from `sonarcloud.needs`). Gate-coverage ratchet refreshed: `total_tests` 28573→28709, `duplicate_test_count` 3550→707, `orphan_test_count` 0. Closes **#2378** (shard-side split), **#1933** (group-side shrink), **#2383** (arch un-blind) under #1931. Draft PR (`pr/ci-topology-shrink`) prepared, not yet merged to upstream/main. *Residual follow-ups (both explicitly deferred to post-merge in the mission's own closeout notes, WP05-owned file `tests/release/ci_topology_timings_postshrink.json`):* (1) replace the committed `measured_source_run_id`/`measured_critical_path_min` (currently `null`, a labeled structural projection) with the real run-id and observed minutes from this PR's first post-shrink `ci-quality` run, reconfirming ≤13.6 min; (2) correct the escape-hatch honesty-note wording (`ci:full`/`ready-for-ci` DO exist as draft/WIP-suppression overrides, just not `run_all` full-coverage hatches). Neither blocks merge.
