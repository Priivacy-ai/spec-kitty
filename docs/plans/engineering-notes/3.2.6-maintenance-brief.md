...
- **Fix:** *immediate* — run `spec-kitty migrate backfill-runtime-state` + commit (~136 seed events + 12 `status_phase` flips; deterministic, idempotent, minutes) → green. *Durable* (follow-up) — wire the cutover into `merge` / `finalize` so new missions are born reconciled, else it re-reds on every merge wave.
- **Net:** the release gate is one mechanical backfill away from green; the durable seam is a tracked follow-up.
...
- **Release readiness for 3.2.6** = run `spec-kitty migrate backfill-runtime-state`, commit the 12 `status_phase` flips (#2917), then the normal tag flow — and schedule the durable seam (cutover in `merge`/`finalize`) so it stays green.
