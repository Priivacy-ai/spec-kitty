...
| 3 | `TestPromptReferencesAuthorityPaths::test_implement_prompt_references_glossary_path` | No `glossary/contexts/` pointer; no fetch command paired with a "when you introduce a term, consult …" rule. The implementer has no signposted way to discover the project glossary. |
| 4 | `TestPromptReferencesAuthorityPaths::test_implement_prompt_references_adr_path` | Same for `architecture/2.x/adr/`. |
...
| 3, 4 | FR — The resolved context for actions in `BOOTSTRAP_ACTIONS` MUST include a `Project authority paths:` block naming `glossary/contexts/`, `architecture/2.x/adr/`, and any project-charter-declared additional authority directories. |
