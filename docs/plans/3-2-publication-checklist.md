...
| **S2** | CLI reference parity — every visible Typer path (192 per `cli-audit-3-2.md`) is referenced or explicitly classified. | [`docs/api/cli-commands.md`](../reference/cli-commands.md) (rebuilt in WP06/WP07), [`docs/development/3-2-cli-reference-methodology.md`](3-2-cli-reference-methodology.md), parity test [`tests/architectural/test_docs_cli_reference_parity.py`](../../tests/architectural/test_docs_cli_reference_parity.py). | `SPEC_KITTY_ENABLE_SAAS_SYNC=1 PYTHONPATH=. uv run pytest tests/architectural/test_docs_cli_reference_parity.py -q` | TBD |
...
| **S4** | Divio coverage for the end-user journey — tutorial, how-to, reference, explanation for each of {install, first mission, charter, multi-harness, recovery}. | [`docs/development/3-2-information-architecture.md`](3-2-information-architecture.md) IA matrix; populated under [`docs/guides/`](../tutorials/), [`docs/guides/`](../how-to/), [`docs/api/`](../reference/), [`docs/architecture/`](../explanation/) (WP08/WP11/WP12). | `ls docs/guides docs/guides docs/api docs/architecture` and cross-reference with the IA matrix. | TBD |
| **S5** | Per-harness reachability — every first-class harness has a setup+usage page with citation to its current external docs. | [`docs/guides/harnesses/`](../how-to/harnesses/) (14 harness pages from WP11), [`docs/development/3-2-harness-research-method.md`](3-2-harness-research-method.md), harness support matrix (WP10). | Each harness page must contain at least one external doc URL: `grep -lE "^- \[.*\]\(https://" docs/guides/harnesses/*.md`. Run the freshness orchestrator with `--link-check spot` to validate URLs resolve. | TBD |
| **S6** | Cross-platform install lifecycle — pip/pipx/uv × macOS/Linux/Windows install/upgrade/uninstall, with PATH/PowerShell coverage. | [`docs/guides/install-and-upgrade.md`](../how-to/install-and-upgrade.md), [`docs/guides/install-macos.md`](../how-to/install-macos.md), [`docs/guides/install-linux.md`](../how-to/install-linux.md), [`docs/guides/install-windows.md`](../how-to/install-windows.md), [`docs/guides/upgrade-cli.md`](../how-to/upgrade-cli.md), [`docs/guides/uninstall.md`](../how-to/uninstall.md), [`docs/guides/diagnose-installation.md`](../how-to/diagnose-installation.md) (WP12). | `for f in docs/guides/install-macos.md docs/guides/install-linux.md docs/guides/install-windows.md; do grep -E "pip\|pipx\|uv" "$f" >/dev/null && echo "$f OK"; done` plus a `spec-kitty --version` smoke run on each OS surface. | TBD |
...
|---|-----------|-------------------|----------------------|--------|
| **SC1** | A new adopter on macOS runs their first mission in under 30 minutes following only the new tutorials and how-tos. | [`docs/guides/getting-started.md`](../tutorials/getting-started.md), [`docs/guides/your-first-feature.md`](../tutorials/your-first-feature.md), [`docs/guides/install-macos.md`](../how-to/install-macos.md), [`docs/guides/harnesses/claude-code.md`](../how-to/harnesses/claude-code.md). | Manual timed walk-through on macOS (§3 manual review item). | TBD |
| **SC2** | A user upgrading from 3.1 or 2.x reaches a green `spec-kitty verify-setup` (or current-3.2 equivalent) without consulting code. | [`docs/guides/upgrade-cli.md`](../how-to/upgrade-cli.md), [`docs/guides/upgrade-project.md`](../how-to/upgrade-project.md), [`docs/development/3-2-archive-migration-plan.md`](3-2-archive-migration-plan.md). | Run `spec-kitty verify-setup` (or `spec-kitty doctor`) following only the upgrade how-tos. | TBD |
| **SC3** | A CLI consumer can find an accurate reference entry for every command they invoke; missing reference entries count as zero in the freshness check. | [`docs/api/cli-commands.md`](../reference/cli-commands.md) rebuild (WP07) + parity test [`tests/architectural/test_docs_cli_reference_parity.py`](../../tests/architectural/test_docs_cli_reference_parity.py) (WP06) + [`scripts/docs/check_cli_reference_freshness.py`](../../scripts/docs/check_cli_reference_freshness.py) (`REF-*`/`HELP-*` rules). | `SPEC_KITTY_ENABLE_SAAS_SYNC=1 PYTHONPATH=. uv run python scripts/docs/check_docs_freshness.py --ci --link-check none` (zero `REF-MISSING-PATH` findings). | TBD |
...
| **SC5** | A harness user finds documentation that cites the host's own current docs and matches the files Spec Kitty installs into the host directory. | [`docs/guides/harnesses/*.md`](../how-to/harnesses/) (14 harness pages) + the agent-directory table in repo-root `CLAUDE.md` / `AGENTS.md` reflected in the per-harness pages. | Spot-check 3 random harness pages (§3 manual review item) and confirm: (a) cited external doc URL resolves, (b) page describes the directory the harness installs into. | TBD |
...
| **NFR-008** | Reference pages must be discoverable from the public URL — `docs/api/cli-commands.md` reachable at `https://docs.spec-kitty.ai/reference/cli-commands.html` after the docs build. | [`docs/toc.yml`](../toc.yml) (root nav), [`docs/api/toc.yml`](../reference/toc.yml), DocFX build via `.github/workflows/docs-pages.yml`. | Build the docs site locally (DocFX) and visit `_site/reference/cli-commands.html`; after the publish workflow runs, visit the public URL. | TBD |
...
|-----------|-----------------|
| **SC1** First mission in < 30 min on macOS | [`docs/guides/getting-started.md`](../tutorials/getting-started.md), [`docs/guides/your-first-feature.md`](../tutorials/your-first-feature.md), [`docs/guides/install-macos.md`](../how-to/install-macos.md), claude-code harness page. |
| **SC2** Upgrader reaches green verify-setup | [`docs/guides/upgrade-cli.md`](../how-to/upgrade-cli.md), [`docs/guides/upgrade-project.md`](../how-to/upgrade-project.md). |
| **SC3** CLI consumer finds accurate reference | [`docs/api/cli-commands.md`](../reference/cli-commands.md) + parity test. |
...
| **SC5** Harness user finds host-matching docs | [`docs/guides/harnesses/*.md`](../how-to/harnesses/). |
...
|         | **macOS**                                                                                 | **Linux**                                                                                 | **Windows**                                                                                  |
|---------|-------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------|
| **pip** | [`docs/guides/install-macos.md`](../how-to/install-macos.md) §pip                          | [`docs/guides/install-linux.md`](../how-to/install-linux.md) §pip                          | [`docs/guides/install-windows.md`](../how-to/install-windows.md) §pip (PowerShell + PATH)     |
| **pipx**| [`docs/guides/install-macos.md`](../how-to/install-macos.md) §pipx                         | [`docs/guides/install-linux.md`](../how-to/install-linux.md) §pipx                         | [`docs/guides/install-windows.md`](../how-to/install-windows.md) §pipx (PowerShell + PATH)    |
| **uv**  | [`docs/guides/install-macos.md`](../how-to/install-macos.md) §uv                           | [`docs/guides/install-linux.md`](../how-to/install-linux.md) §uv                           | [`docs/guides/install-windows.md`](../how-to/install-windows.md) §uv (PowerShell + py-launcher)|

Cross-cutting: [`docs/guides/install-and-upgrade.md`](../how-to/install-and-upgrade.md), [`docs/guides/upgrade-cli.md`](../how-to/upgrade-cli.md), [`docs/guides/uninstall.md`](../how-to/uninstall.md), [`docs/guides/diagnose-installation.md`](../how-to/diagnose-installation.md). Each cell must have a `spec-kitty --version` verification step (NFR-003). Manual review step §3.4 enforces this.
