...
| 16 | top-level `spec-kitty implement` vs `agent action implement` | both exist | Top-level `implement` ARGS=`[wp_id]` OPTS=`--mission, --feature, --auto-commit, --json, --recover, --base, --acknowledge-not-bulk-edit, --actor` — **has `--json`** and is the internal allocator. The canonical agent surface (`agent action implement`) does not. This is the split #2007 names: JSON lives on the internal surface, not the canonical one. |
