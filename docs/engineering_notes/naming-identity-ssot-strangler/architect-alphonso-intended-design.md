...
| Aspect | Target |
|--------|--------|
| **SSOT** | `missions/_read_path_resolver.py::resolve_mission_read_path` — the ONE read primitive (C-005). All topology priority (coord worktree → primary checkout, fail-closed on stale-primary) lives here. `feature_dir_resolver.py` is now a thin re-export shim (C-004 strangler). |
| **Public contract** | `resolve_mission_read_path(repo_root, slug, mid8, *, require_exists)`; `candidate_feature_dir_for_mission` (never-raise candidate); `primary_feature_dir_for_mission` (deliberately topology-BLIND, for meta.json reads); `resolve_status_surface_with_anchor` (status-emitter surface, anchored on meta-derived mid8). Ambiguous handle → `MissionSelectorAmbiguous` (no silent fallback, C-CTX-4/C-009). Stale-primary-under-coord → `StatusReadPathNotFound` (#1718). |
