...
|---|----------|-------------------|----------------------|--------|
| **S1** | Version separation at the entry page — 3.2 lands as `current`; 1.x/2.x is archive/migration. | [`docs/3x/index.md`](../3x/index.md) (3.2 landing page from WP04), [`docs/development/3-2-version-taxonomy.md`](3-2-version-taxonomy.md), [`docs/development/3-2-navigation-plan.md`](3-2-navigation-plan.md), archive-notice banners enforced by [`scripts/docs/version_leakage_check.py`](../../scripts/docs/version_leakage_check.py) (`LEAK-*` rules). | `SPEC_KITTY_ENABLE_SAAS_SYNC=1 SPEC_KITTY_NO_UPGRADE_CHECK=1 PYTHONPATH=. uv run python scripts/docs/check_docs_freshness.py --ci --link-check none` | TBD |
| **S2** | CLI reference parity — every visible Typer path (192 per `cli-audit-3-2.md`) is referenced or explicitly classified. | [`docs/reference/cli-commands.md`](../reference/cli-commands.md) (rebuilt in WP06/WP07), [`docs/development/3-2-cli-reference-methodology.md`](3-2-cli-reference-methodology.md), parity test [`tests/architectural/test_docs_cli_reference_parity.py`](../../tests/architectural/test_docs_cli_reference_parity.py). | `SPEC_KITTY_ENABLE_SAAS_SYNC=1 PYTHONPATH=. uv run pytest tests/architectural/test_docs_cli_reference_parity.py -q` | TBD |
| **S3** | CLI help truthfulness — every help/code/test mismatch lands in `3-2-cli-reference-audit-meta-issues.md`, never silently in docs. | [`docs/development/3-2-cli-reference-audit-meta-issues.md`](3-2-cli-reference-audit-meta-issues.md) (6 seeded rows, schema enforced by [`kitty-specs/spec-kitty-3-2-docs-01KS4KSZ/data-model.md`](../../kitty-specs/spec-kitty-3-2-docs-01KS4KSZ/data-model.md#metaissueentry)). | Grep the docs tree for changes that could have masked a meta-issue: `git log --oneline docs/reference/ docs/how-to/ docs/tutorials/ | head` then cross-check that every change has a row here. | TBD |
| **S4** | Divio coverage for the end-user journey — tutorial, how-to, reference, explanation for each of {install, first mission, charter, multi-harness, recovery}. | [`docs/development/3-2-information-architecture.md`](3-2-information-architecture.md) IA matrix; populated under [`docs/tutorials/`](../tutorials/), [`docs/how-to/`](../how-to/), [`docs/reference/`](../reference/), [`docs/explanation/`](../explanation/) (WP08/WP11/WP12). | `ls docs/tutorials docs/how-to docs/reference docs/explanation` and cross-reference with the IA matrix. | TBD |
| **S5** | Per-harness reachability — every first-class harness has a setup+usage page with citation to its current external docs. | [`docs/how-to/harnesses/`](../how-to/harnesses/) (14 harness pages from WP11), [`docs/development/3-2-harness-research-method.md`](3-2-harness-research-method.md), harness support matrix (WP10). | Each harness page must contain at least one external doc URL: `grep -lE "^- \[.*\]\(https://" docs/how-to/harnesses/*.md`. Run the freshness orchestrator with `--link-check spot` to validate URLs resolve. | TBD |
| **S6** | Cross-platform install lifecycle — pip/pipx/uv × macOS/Linux/Windows install/upgrade/uninstall, with PATH/PowerShell coverage. | [`docs/how-to/install-and-upgrade.md`](../how-to/install-and-upgrade.md), [`docs/how-to/install-macos.md`](../how-to/install-macos.md), [`docs/how-to/install-linux.md`](../how-to/install-linux.md), [`docs/how-to/install-windows.md`](../how-to/install-windows.md), [`docs/how-to/upgrade-cli.md`](../how-to/upgrade-cli.md), [`docs/how-to/uninstall.md`](../how-to/uninstall.md), [`docs/how-to/diagnose-installation.md`](../how-to/diagnose-installation.md) (WP12). | `for f in docs/how-to/install-macos.md docs/how-to/install-linux.md docs/how-to/install-windows.md; do grep -E "pip\|pipx\|uv" "$f" >/dev/null && echo "$f OK"; done` plus a `spec-kitty --version` smoke run on each OS surface. | TBD |
| **S7** | No mixed-version leakage in current pages — pages tagged `current` never link to `archival` without a banner. | [`docs/development/3-2-page-inventory.yaml`](3-2-page-inventory.yaml), [`docs/development/3-2-archive-migration-plan.md`](3-2-archive-migration-plan.md), enforced by `scripts/docs/version_leakage_check.py` (`LEAK-CURRENT-TO-ARCHIVAL`, `LEAK-MISSING-BANNER`). | `SPEC_KITTY_ENABLE_SAAS_SYNC=1 PYTHONPATH=. uv run python scripts/docs/check_docs_freshness.py --ci --link-check none` (zero `LEAK-*` findings required). | TBD |
...
|---|-----------|-------------------|----------------------|--------|
| **SC1** | A new adopter on macOS runs their first mission in under 30 minutes following only the new tutorials and how-tos. | [`docs/tutorials/getting-started.md`](../tutorials/getting-started.md), [`docs/tutorials/your-first-feature.md`](../tutorials/your-first-feature.md), [`docs/how-to/install-macos.md`](../how-to/install-macos.md), [`docs/how-to/harnesses/claude-code.md`](../how-to/harnesses/claude-code.md). | Manual timed walk-through on macOS (§3 manual review item). | TBD |
| **SC2** | A user upgrading from 3.1 or 2.x reaches a green `spec-kitty verify-setup` (or current-3.2 equivalent) without consulting code. | [`docs/how-to/upgrade-cli.md`](../how-to/upgrade-cli.md), [`docs/how-to/upgrade-project.md`](../how-to/upgrade-project.md), [`docs/development/3-2-archive-migration-plan.md`](3-2-archive-migration-plan.md). | Run `spec-kitty verify-setup` (or `spec-kitty doctor`) following only the upgrade how-tos. | TBD |
| **SC3** | A CLI consumer can find an accurate reference entry for every command they invoke; missing reference entries count as zero in the freshness check. | [`docs/reference/cli-commands.md`](../reference/cli-commands.md) rebuild (WP07) + parity test [`tests/architectural/test_docs_cli_reference_parity.py`](../../tests/architectural/test_docs_cli_reference_parity.py) (WP06) + [`scripts/docs/check_cli_reference_freshness.py`](../../scripts/docs/check_cli_reference_freshness.py) (`REF-*`/`HELP-*` rules). | `SPEC_KITTY_ENABLE_SAAS_SYNC=1 PYTHONPATH=. uv run python scripts/docs/check_docs_freshness.py --ci --link-check none` (zero `REF-MISSING-PATH` findings). | TBD |
| **SC4** | A docs reviewer confirms version-leakage compliance in under 10 minutes using the FR-005 validation mechanism plus this checklist. | [`scripts/docs/version_leakage_check.py`](../../scripts/docs/version_leakage_check.py), [`docs/development/3-2-page-inventory.yaml`](3-2-page-inventory.yaml). | Same orchestrator command as S7; output must be a single PASS line for the reviewer. | TBD |
| **SC5** | A harness user finds documentation that cites the host's own current docs and matches the files Spec Kitty installs into the host directory. | [`docs/how-to/harnesses/*.md`](../how-to/harnesses/) (14 harness pages) + the agent-directory table in repo-root `CLAUDE.md` / `AGENTS.md` reflected in the per-harness pages. | Spot-check 3 random harness pages (§3 manual review item) and confirm: (a) cited external doc URL resolves, (b) page describes the directory the harness installs into. | TBD |
| **SC6** | After publication, the only place CLI/help mismatches live is `3-2-cli-reference-audit-meta-issues.md`; no silent doc-side fixes exist. | Meta-issue file plus the test [`tests/architectural/test_docs_cli_reference_parity.py`](../../tests/architectural/test_docs_cli_reference_parity.py) which fails if a doc fix masks a help mismatch. | Cross-check meta-issue rows against `docs/reference/cli-commands.md` revision diffs; the test should pass. | TBD |
...
| **NFR-007** | Operator readability of the harness support matrix — single page, 5 tiers, every harness from FR-014 listed. | [`docs/development/3-2-harness-research-method.md`](3-2-harness-research-method.md) + harness support matrix page (WP10). | Open the harness support matrix, count tiers (5 expected), and confirm every harness from `docs/how-to/harnesses/` appears in exactly one tier. | TBD |
| **NFR-008** | Reference pages must be discoverable from the public URL — `docs/reference/cli-commands.md` reachable at `https://docs.spec-kitty.ai/reference/cli-commands.html` after the docs build. | [`docs/toc.yml`](../toc.yml) (root nav), [`docs/reference/toc.yml`](../reference/toc.yml), DocFX build via `.github/workflows/docs-pages.yml`. | Build the docs site locally (DocFX) and visit `_site/reference/cli-commands.html`; after the publish workflow runs, visit the public URL. | TBD |
...
| **`tests/architectural/test_docs_cli_reference_parity.py`** (from WP06) | Part of the architectural-tests run inside `ci-quality.yml` and pre-merge `pytest` | Confirms every visible Typer path (with SaaS sync on) appears in `docs/reference/cli-commands.md`, and every reference entry is either visible or classified in the meta-issue file. | `SPEC_KITTY_ENABLE_SAAS_SYNC=1 PYTHONPATH=. uv run pytest tests/architectural/test_docs_cli_reference_parity.py -q` |
...
2. **3.2 landing page link from root README (S1, S4).** Open the repo-root `README.md`. Confirm a link to the 3.2 landing page (e.g., `docs/3x/index.md` or its rendered URL `https://docs.spec-kitty.ai/3x/`) appears prominently. If absent, file a doc fix before publishing.
3. **CLI reference cross-link reachable (S2, NFR-008).** From the 3.2 landing page, navigate to `docs/reference/cli-commands.md` in two clicks or fewer. Then visit the rendered URL `https://docs.spec-kitty.ai/reference/cli-commands.html` after the publish workflow runs.
4. **`spec-kitty --version` in install how-tos (S6, NFR-003).** Run `spec-kitty --version` locally and confirm the output (e.g., `spec-kitty, version 3.2.0rc22`) is shown verbatim or as a placeholder pattern in `docs/how-to/install-macos.md`, `install-linux.md`, and `install-windows.md`. Verify each install how-to documents the `--version` check as the post-install smoke step.
...
6. **Random harness external doc URLs still resolve (S5, NFR-004).** Open 3 random files under `docs/how-to/harnesses/`. For each, click every external URL in the `References` section and confirm the page loads (HTTP 200). If any URL 404s, file a meta-issue row before publishing.
...
| 2 | `spec-kitty agent context update-context` | `docs` | Remove every doc reference and replace with `agent context resolve`; add a migration note for operators on older tutorials. | Owner: maintainers (docs). Track as an in-mission fix during WP07 follow-up or a fast-follow docs sweep. Resolution: confirm `docs/reference/agent-subcommands.md`, `docs/tutorials/missions-overview.md`, and `docs/tutorials/claude-code-workflow.md` no longer reference `update-context`; flip to `resolved`. |
| 3 | `spec-kitty agent workflow implement` | `docs` | Rewrite affected how-tos and skill prompts to use `spec-kitty agent action implement`; optionally add a deprecation alias if external automation depends on the old name. | Owner: maintainers (docs + skills). Track as an upstream issue if the alias decision lands in code; otherwise track as a docs fast-follow. Resolution: grep `docs/` and `.claude/`, `.codex/`, etc., for `agent workflow implement`; replace; flip to `resolved`. |
...
|-----------|-----------------|
| **SC1** First mission in < 30 min on macOS | [`docs/tutorials/getting-started.md`](../tutorials/getting-started.md), [`docs/tutorials/your-first-feature.md`](../tutorials/your-first-feature.md), [`docs/how-to/install-macos.md`](../how-to/install-macos.md), claude-code harness page. |
| **SC2** Upgrader reaches green verify-setup | [`docs/how-to/upgrade-cli.md`](../how-to/upgrade-cli.md), [`docs/how-to/upgrade-project.md`](../how-to/upgrade-project.md). |
| **SC3** CLI consumer finds accurate reference | [`docs/reference/cli-commands.md`](../reference/cli-commands.md) + parity test. |
...
| **SC5** Harness user finds host-matching docs | [`docs/how-to/harnesses/*.md`](../how-to/harnesses/). |
...
|         | **macOS**                                                                                 | **Linux**                                                                                 | **Windows**                                                                                  |
|---------|-------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------|
| **pip** | [`docs/how-to/install-macos.md`](../how-to/install-macos.md) §pip                          | [`docs/how-to/install-linux.md`](../how-to/install-linux.md) §pip                          | [`docs/how-to/install-windows.md`](../how-to/install-windows.md) §pip (PowerShell + PATH)     |
| **pipx**| [`docs/how-to/install-macos.md`](../how-to/install-macos.md) §pipx                         | [`docs/how-to/install-linux.md`](../how-to/install-linux.md) §pipx                         | [`docs/how-to/install-windows.md`](../how-to/install-windows.md) §pipx (PowerShell + PATH)    |
| **uv**  | [`docs/how-to/install-macos.md`](../how-to/install-macos.md) §uv                           | [`docs/how-to/install-linux.md`](../how-to/install-linux.md) §uv                           | [`docs/how-to/install-windows.md`](../how-to/install-windows.md) §uv (PowerShell + py-launcher)|

Cross-cutting: [`docs/how-to/install-and-upgrade.md`](../how-to/install-and-upgrade.md), [`docs/how-to/upgrade-cli.md`](../how-to/upgrade-cli.md), [`docs/how-to/uninstall.md`](../how-to/uninstall.md), [`docs/how-to/diagnose-installation.md`](../how-to/diagnose-installation.md). Each cell must have a `spec-kitty --version` verification step (NFR-003). Manual review step §3.4 enforces this.

### §6d. Harness support matrix (WP10)

The harness support matrix from WP10 lists every harness in `docs/how-to/harnesses/`:
...
The 14 pages under `docs/how-to/harnesses/` cover: Claude Code, Codex, OpenCode,
