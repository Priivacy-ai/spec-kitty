...
| 2. Plug into spec-kitty | Drop into `.kittify/doctrine/styleguides/` (project layer) OR bundle into a pack | Project-layer drop works (Mission A's three-layer resolver); pack form works (`spec-kitty doctrine pack validate` + `assemble`) | No `spec-kitty doctrine plug <path>` gesture; user must read source docs to learn the destination |
