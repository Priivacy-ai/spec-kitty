...
| `tests/sync/test_orphan_sweep.py` and the rest of `tests/_real_port_suites.py`'s `FIXED_RANGE_SUITES` (real fixed ports 9400–9449, daemons — 5 files, not just `test_orphan_sweep.py`; see [testing-parallel.md → The serial daemon pass](testing-parallel.md#the-serial-daemon-pass)) | 3 | **Resolved — already serialised in CI.** Each runs in a dedicated `-n0` serial job, excluded from the parallel pool via explicit `--ignore`s in `.github/workflows/ci-quality.yml` (`tests/architectural/test_serial_port_preservation.py` / `test_workflow_dist_lint.py` enforce the YAML stays that way). No quarantine needed — but this exclusion is CI-workflow-level, not a pytest-collection-level guard, so a bare local `pytest tests/sync -n <N>` (or `tests/ -n <N>`) with no `--ignore`s can still hit a real port-contention flake here (e.g. `RuntimeError: daemon on port 9401 (version=None) never published health identity`); that failure is this known, already-disposed-of surface, not a new regression — rerun with the five files isolated. |
...
| `tests/sync/test_daemon_self_retirement.py` (4 failures reported against the M0 baseline and the pre-`cff82ad84` M2-canonical-integration baseline; `docs/bootstrap/M2-CANONICAL-INTEGRATION.json`) | 2 | **Retired — this surface no longer exists.** The sync transport that owned it was deleted (issue #5); `tests/sync/test_daemon_self_retirement.py` is gone from the repo. |
