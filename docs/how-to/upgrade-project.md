...
3. **Updates agent command directories** for configured agents only (`.claude/commands/`, `.agents/skills/spec-kitty.*/`, `.amazonq/prompts/`, etc.). Agents you removed stay removed — migrations never recreate them.
...
```bash
git add .kittify/ .claude/ .codex/    # whichever directories changed
...
```bash
git restore .kittify/ .claude/ .codex/
```
