...
> **Note**: This tutorial works with both git and jujutsu (jj) backends. Spec Kitty abstracts the VCS operations, so the workflow is the same. For jj-specific features like auto-rebase and non-blocking conflicts, see [Jujutsu Workflow Tutorial](jujutsu-workflow.md).
