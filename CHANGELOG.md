...
- **Branch-strategy recommendation in `/specify` (issue #765):** `spec-kitty agent mission branch-context`
  ...
  **proactively** recommend starting on a dedicated feature branch — and to create/switch to it before any
  mission artifacts are written — when the operator is on the primary branch and expects a later PR; staying
  on the current branch remains an explicit, supported choice (wiring the previously-dormant `--pr-bound`
