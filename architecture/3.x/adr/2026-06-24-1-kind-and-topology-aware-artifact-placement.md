# ADR: Kind- and Topology-Aware Artifact Placement — One Partition, Read/Write Symmetry

**Date**: 2026-06-24
**Status**: Accepted — implemented by mission `write-surface-coherence-01KVTVZS` (PR [#2106](https://github.com/Priivacy-ai/spec-kitty/pull/2106)). The `MissionArtifactKind` read model shipped earlier with mission `single-authority-topology-cleanup` (PR [#2099](https://github.com/Priivacy-ai/spec-kitty/pull/2099)); this ADR records the write-side twin that closes the read/write asymmetry.
**Mission**: `write-surface-coherence-01KVTVZS`
**Requirement**: FR-001..FR-010, NFR-001..NFR-004, C-001..C-006 (binding)
**Module**: `src/mission_runtime/artifacts.py` (`MissionArtifactKind`, `_PRIMARY_ARTIFACT_KINDS`, `_PLACEMENT_ARTIFACT_KINDS`, `artifact_home_for`, `is_coordination_artifact_residue_path`, `is_primary_artifact_kind`, `kind_for_mission_file`), `src/mission_runtime/resolution.py` (`resolve_placement_only`), `src/specify_cli/coordination/commit_router.py` (`commit_for_mission`), `src/specify_cli/cli/commands/agent/mission.py` (`_planning_commit_worktree`)
**Design driver**: [#2090](https://github.com/Priivacy-ai/spec-kitty/issues/2090)
**Predecessor**: [ADR 2026-06-22-1 — MissionTopology SSOT](2026-06-22-1-mission-topology-ssot.md), [ADR 2026-06-19-1 — Coord-Empty Surface Policy](2026-06-19-1-coord-empty-surface-fallback.md), [ADR 2026-06-03-2 — ExecutionContext Owner and CommitTarget](2026-06-03-2-executioncontext-owner-and-committarget.md)
