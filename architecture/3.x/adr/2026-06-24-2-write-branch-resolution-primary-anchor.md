# ADR: Write-Branch Resolution Anchors `meta.json` on the PRIMARY Surface (Write-Surface Twin)

**Date**: 2026-06-24
**Status**: Accepted — implemented by mission `gate-read-surface-completion-01KVW9B0` (draft PR [#2113](https://github.com/Priivacy-ai/spec-kitty/pull/2113)). Records the write-side twin of the kind-aware read partition: the read side resolves planning artifacts via the kind-aware seam; the write side resolves a planning artifact's commit/branch from the mission `target_branch` on the **primary** surface, never the topology candidate.
**Mission**: `gate-read-surface-completion-01KVW9B0`
**Requirement**: FR-004, FR-009(e), FR-010, NFR-001 (binding); contract `contracts/gate-read-seam.md` §"Seam contract — WRITE side" / G-6
**Module**: `src/specify_cli/core/paths.py` (`get_feature_target_branch`, `resolve_merge_target_branch`), `src/specify_cli/core/git_ops.py` (`resolve_target_branch`), `src/specify_cli/cli/commands/agent/mission.py` (`finalize_tasks` commit-branch resolution), `src/specify_cli/cli/commands/research.py` (`research` — planning read + scaffold write via the seam), `src/specify_cli/missions/_read_path_resolver.py` (`primary_feature_dir_for_mission`, `resolve_planning_read_dir`), enforced by `tests/architectural/test_gate_read_literal_ban.py`
**Design driver**: [#2107](https://github.com/Priivacy-ai/spec-kitty/issues/2107)
**Predecessor**: [ADR 2026-06-24-1 — Kind- and Topology-Aware Artifact Placement](2026-06-24-1-kind-and-topology-aware-artifact-placement.md), [ADR 2026-06-22-1 — MissionTopology SSOT](2026-06-22-1-mission-topology-ssot.md)
